...
using namespace SD;
BOOLEAN ReAuthAnswer::isPresent(int tag){
...
ReAuthAnswer::ReAuthAnswer()
{
	...
	mLoadGrpList = NULL;
	mTagsPresent.reset();
	mMandatoryAvpCheckVal= bitset<16>(string("0110001001111101"));
...
OctetString* ReAuthAnswer::setSessionId(){
	mMandatoryAvpSetVal [ReAuthAnswer_SESSIONID] = 1;
	if(mSessionId == NULL)
	{
		mSessionId = new OctetString();
		mTagsPresent[ReAuthAnswer_SESSIONID] = 1;
		return mSessionId;
	}
	return mSessionId;
}

void ReAuthAnswer::resetSessionId(){
	mTagsPresent[ReAuthAnswer_SESSIONID] = 0;
	if(mSessionId != NULL)
	{
		delete mSessionId; 
		mSessionId = NULL;
...
DRMP* ReAuthAnswer::setDRMP(){
	if(mDRMP == NULL)
	{
		mDRMP = new DRMP();
		mTagsPresent[ReAuthAnswer_DRMP] = 1;
		return mDRMP;
	}
	return mDRMP;
}

void ReAuthAnswer::resetDRMP(){
	mTagsPresent[ReAuthAnswer_DRMP] = 0;
	if(mDRMP != NULL)
	{
		delete mDRMP; 
		mDRMP = NULL;
...
OctetString* ReAuthAnswer::setOriginHost(){
	mMandatoryAvpSetVal [ReAuthAnswer_ORIGINHOST] = 1;
	if(mOriginHost == NULL)
	{
		mOriginHost = new OctetString();
		mTagsPresent[ReAuthAnswer_ORIGINHOST] = 1;
		return mOriginHost;
	}
	return mOriginHost;
}

void ReAuthAnswer::resetOriginHost(){
	mTagsPresent[ReAuthAnswer_ORIGINHOST] = 0;
	if(mOriginHost != NULL)
	{
		delete mOriginHost; 
		mOriginHost = NULL;
...
OctetString* ReAuthAnswer::setOriginRealm(){
	mMandatoryAvpSetVal [ReAuthAnswer_ORIGINREALM] = 1;
	if(mOriginRealm == NULL)
	{
		mOriginRealm = new OctetString();
		mTagsPresent[ReAuthAnswer_ORIGINREALM] = 1;
		return mOriginRealm;
	}
	return mOriginRealm;
}

void ReAuthAnswer::resetOriginRealm(){
	mTagsPresent[ReAuthAnswer_ORIGINREALM] = 0;
	if(mOriginRealm != NULL)
	{
		delete mOriginRealm; 
		mOriginRealm = NULL;
...
Unsigned32* ReAuthAnswer::setResultCode(){
	mMandatoryAvpSetVal [ReAuthAnswer_RESULTCODE] = 1;
	if(mResultCode == NULL)
	{
		mResultCode = new Unsigned32();
		mTagsPresent[ReAuthAnswer_RESULTCODE] = 1;
		return mResultCode;
	}
	return mResultCode;
}

void ReAuthAnswer::resetResultCode(){
	mTagsPresent[ReAuthAnswer_RESULTCODE] = 0;
	if(mResultCode != NULL)
	{
		delete mResultCode; 
		mResultCode = NULL;
...
Unsigned32* ReAuthAnswer::setOriginStateId(){
	mMandatoryAvpSetVal [ReAuthAnswer_ORIGINSTATEID] = 1;
	if(mOriginStateId == NULL)
	{
		mOriginStateId = new Unsigned32();
		mTagsPresent[ReAuthAnswer_ORIGINSTATEID] = 1;
		return mOriginStateId;
	}
	return mOriginStateId;
}

void ReAuthAnswer::resetOriginStateId(){
	mTagsPresent[ReAuthAnswer_ORIGINSTATEID] = 0;
	if(mOriginStateId != NULL)
	{
		delete mOriginStateId; 
		mOriginStateId = NULL;
...
OctetString* ReAuthAnswer::setErrorMessage(){
	if(mErrorMessage == NULL)
	{
		mErrorMessage = new OctetString();
		mTagsPresent[ReAuthAnswer_ERRORMESSAGE] = 1;
		return mErrorMessage;
	}
	return mErrorMessage;
}

void ReAuthAnswer::resetErrorMessage(){
	mTagsPresent[ReAuthAnswer_ERRORMESSAGE] = 0;
	if(mErrorMessage != NULL)
	{
		delete mErrorMessage; 
		mErrorMessage = NULL;
...
OctetString* ReAuthAnswer::setErrorReportingHost(){
	if(mErrorReportingHost == NULL)
	{
		mErrorReportingHost = new OctetString();
		mTagsPresent[ReAuthAnswer_ERRORREPORTINGHOST] = 1;
		return mErrorReportingHost;
	}
	return mErrorReportingHost;
}

void ReAuthAnswer::resetErrorReportingHost(){
	mTagsPresent[ReAuthAnswer_ERRORREPORTINGHOST] = 0;
	if(mErrorReportingHost != NULL)
	{
		delete mErrorReportingHost; 
		mErrorReportingHost = NULL;
...
OctetString* ReAuthAnswer::getSessionId(){
	if(NULL == mSessionId) {
...
OctetString* ReAuthAnswer::getOriginHost(){
	if(NULL == mOriginHost) {
...
OctetString* ReAuthAnswer::getOriginRealm(){
...
Unsigned32* ReAuthAnswer::getResultCode(){
	if(NULL == mResultCode) {
...
ExperimentalResult* ReAuthAnswer::getExperimentalResult()
{
	if(mExperimentalResultGrp == NULL)
	{
...
ExperimentalResult* ReAuthAnswer::setExperimentalResult(){
	if(mExperimentalResultGrp == NULL)
		mExperimentalResultGrp = new ExperimentalResult();
	mTagsPresent[ReAuthAnswer_EXPERIMENTALRESULT] = 1;
	mMandatoryAvpSetVal[ReAuthAnswer_EXPERIMENTALRESULT] = 1;
	return mExperimentalResultGrp;
}

void ReAuthAnswer::resetExperimentalResult(){
	mTagsPresent[ReAuthAnswer_EXPERIMENTALRESULT] = 0;
	if(mExperimentalResultGrp != NULL)
		delete mExperimentalResultGrp;
}

Unsigned32* ReAuthAnswer::getOriginStateId(){
...
OCSupportedFeatures* ReAuthAnswer::getOCSupportedFeatures()
{
	if(mOCSupportedFeaturesGrp == NULL)
	{
...
OCSupportedFeatures* ReAuthAnswer::setOCSupportedFeatures(){
	if(mOCSupportedFeaturesGrp == NULL)
		mOCSupportedFeaturesGrp = new OCSupportedFeatures();
	mTagsPresent[ReAuthAnswer_OCSUPPORTEDFEATURES] = 1;
	return mOCSupportedFeaturesGrp;
}

void ReAuthAnswer::resetOCSupportedFeatures(){
	mTagsPresent[ReAuthAnswer_OCSUPPORTEDFEATURES] = 0;
	if(mOCSupportedFeaturesGrp != NULL)
		delete mOCSupportedFeaturesGrp;
}

OCOLR* ReAuthAnswer::getOCOLR()
{
	if(mOCOLRGrp == NULL)
	{
...
OCOLR* ReAuthAnswer::setOCOLR(){
	if(mOCOLRGrp == NULL)
		mOCOLRGrp = new OCOLR();
	mTagsPresent[ReAuthAnswer_OCOLR] = 1;
	return mOCOLRGrp;
}

void ReAuthAnswer::resetOCOLR(){
	mTagsPresent[ReAuthAnswer_OCOLR] = 0;
	if(mOCOLRGrp != NULL)
		delete mOCOLRGrp;
}

list<ADCRuleReport*>* ReAuthAnswer::getADCRuleReportGrpList()
{
	if(mADCRuleReportGrpList == NULL)
	{
	...
	for(auto itr : *mADCRuleReportGrpList)
	{
		...
		{
			mTagsPresent[ReAuthAnswer_ADCRULEREPORT] = 0;
			mADCRuleReportGrpList = NULL;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "ADCRuleReport is Null");
			throw lTssDiaMsgException;
		}
...
list<ADCRuleReport*>* ReAuthAnswer::setADCRuleReportGrpList(){
	mTagsPresent[ReAuthAnswer_ADCRULEREPORT] = 1;
	if(mADCRuleReportGrpList == NULL)
		mADCRuleReportGrpList = new list<ADCRuleReport*>;
	mMandatoryAvpSetVal[ReAuthAnswer_ADCRULEREPORT] = 1;
	mADCRuleReportGrpList->clear();
	return mADCRuleReportGrpList;
}

void ReAuthAnswer::resetADCRuleReportGrpList(){
	mTagsPresent[ReAuthAnswer_ADCRULEREPORT] = 0;
	if(mADCRuleReportGrpList != NULL)
		delete mADCRuleReportGrpList;
}

EventReportIndication* ReAuthAnswer::getEventReportIndication()
{
	if(mEventReportIndicationGrp == NULL)
	{
...
EventReportIndication* ReAuthAnswer::setEventReportIndication(){
	if(mEventReportIndicationGrp == NULL)
		mEventReportIndicationGrp = new EventReportIndication();
	mTagsPresent[ReAuthAnswer_EVENTREPORTINDICATION] = 1;
	return mEventReportIndicationGrp;
}

void ReAuthAnswer::resetEventReportIndication(){
	mTagsPresent[ReAuthAnswer_EVENTREPORTINDICATION] = 0;
	if(mEventReportIndicationGrp != NULL)
		delete mEventReportIndicationGrp;
}

OctetString* ReAuthAnswer::getErrorMessage(){
...
OctetString* ReAuthAnswer::getErrorReportingHost(){
...
FailedAVP* ReAuthAnswer::getFailedAVP()
{
	if(mFailedAVPGrp == NULL)
	{
...
FailedAVP* ReAuthAnswer::setFailedAVP(){
	if(mFailedAVPGrp == NULL)
		mFailedAVPGrp = new FailedAVP();
	mTagsPresent[ReAuthAnswer_FAILEDAVP] = 1;
	mMandatoryAvpSetVal[ReAuthAnswer_FAILEDAVP] = 1;
	return mFailedAVPGrp;
}

void ReAuthAnswer::resetFailedAVP(){
	mTagsPresent[ReAuthAnswer_FAILEDAVP] = 0;
	if(mFailedAVPGrp != NULL)
		delete mFailedAVPGrp;
}

list<SD::ProxyInfo*>* ReAuthAnswer::getProxyInfoGrpList()
{
	if(mProxyInfoGrpList == NULL)
	{
	...
	for(auto itr : *mProxyInfoGrpList)
	{
		...
		{
			mTagsPresent[ReAuthAnswer_PROXYINFO] = 0;
			mProxyInfoGrpList = NULL;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "ProxyInfo is Null");
			throw lTssDiaMsgException;
		}
...
list<SD::ProxyInfo*>* ReAuthAnswer::setProxyInfoGrpList(){
	mTagsPresent[ReAuthAnswer_PROXYINFO] = 1;
	if(mProxyInfoGrpList == NULL)
		mProxyInfoGrpList = new list<SD::ProxyInfo*>;
	mMandatoryAvpSetVal[ReAuthAnswer_PROXYINFO] = 1;
	mProxyInfoGrpList->clear();
	return mProxyInfoGrpList;
}

void ReAuthAnswer::resetProxyInfoGrpList(){
	mTagsPresent[ReAuthAnswer_PROXYINFO] = 0;
	if(mProxyInfoGrpList != NULL)
		delete mProxyInfoGrpList;
}

list<Load*>* ReAuthAnswer::getLoadGrpList()
{
	if(mLoadGrpList == NULL)
	{
	...
	for(auto itr : *mLoadGrpList)
	{
		...
		{
			mTagsPresent[ReAuthAnswer_LOAD] = 0;
			mLoadGrpList = NULL;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "Load is Null");
			throw lTssDiaMsgException;
		}
...
list<Load*>* ReAuthAnswer::setLoadGrpList(){
	mTagsPresent[ReAuthAnswer_LOAD] = 1;
	if(mLoadGrpList == NULL)
...
void ReAuthAnswer::resetLoadGrpList(){
	mTagsPresent[ReAuthAnswer_LOAD] = 0;
	if(mLoadGrpList != NULL)
		delete mLoadGrpList;
}
...
int ReAuthAnswer::Encode(byteArray *array)
{
	...
	for(unsigned int i = 0 ; i < mTagsPresent.size() ; ++i )
	{
		...
		switch(i)
		{
			...
			case ReAuthAnswer_RESULTCODE:
			{
				...
				break;
			case ReAuthAnswer_EXPERIMENTALRESULT:
			{
				array->size+=4;
				...
				break;
			case ReAuthAnswer_ORIGINSTATEID:
			{
					array->size+=4;
				...
				break;
			case ReAuthAnswer_OCSUPPORTEDFEATURES:
			{
				array->size+=4;
			...
			case ReAuthAnswer_OCOLR:
			{
				...
				break;
			case ReAuthAnswer_ADCRULEREPORT:
			{
				if( mADCRuleReportGrpList->size() <= 0) {
					break;
				}
				...
				break;
			case ReAuthAnswer_EVENTREPORTINDICATION:
			{
				array->size+=4;
				...
				break;
			case ReAuthAnswer_ERRORMESSAGE:
			{
					array->size+=4;
				...
				break;
			case ReAuthAnswer_ERRORREPORTINGHOST:
			{
					array->size+=4;
...
int ReAuthAnswer::Decode(byteArray *array, INT16 lMLen, UINT16 *parsedBytes)
{
...
DiameterBaseInterface* ReAuthAnswer::Find(byteArray *array, UINT32 avpCode)
{
...
BOOLEAN ReAuthAnswer::FindAll(byteArray *array, UINT32 avpCode, list<DiameterBaseInterface*>& pBaseInterface,BOOLEAN deepInspect)
{
		if(!mDecodeFlag && array != NULL)
			this->Decode(array);
...
std::shared_ptr<byteArray> ReAuthAnswer::GetByteArray()
{
	return mspByteArray;
}


BOOLEAN ReAuthAnswer::FindAll(byteArray *array, std::map<int,UINT32> &avpCodeMap, int &size, int key, list<DiameterBaseInterface*>& pBaseInterface,BOOLEAN deepInspect)
{
		if(!mDecodeFlag && array != NULL)
			this->Decode(array);
...
void ReAuthAnswer::Display(UINT32 tab)
{
...
BOOLEAN ReAuthAnswer::GetMissingAvpInfo(TEXT *lMissingAvpInfo)
{
