#include "base/include/ReAuthRequest.h"
...
ReAuthRequest::ReAuthRequest()
{
	...
	mCCRequestType = NULL;
	mTagsPresent.reset();
	mMandatoryAvpCheckVal= bitset<12>(string("111111111111"));
...
OctetString* ReAuthRequest::setUserName(){
	mMandatoryAvpSetVal [ReAuthRequest_USERNAME] = 1;
	if(mUserName == NULL)
	{
		mUserName = new OctetString();
		mTagsPresent[ReAuthRequest_USERNAME] = 1;
		return mUserName;
	}
	return mUserName;
}

void ReAuthRequest::resetUserName(){
	mTagsPresent[ReAuthRequest_USERNAME] = 0;
	if(mUserName != NULL)
	{
		delete mUserName; 
		mUserName = NULL;
...
CCRequestType* ReAuthRequest::setCCRequestType(){
	mMandatoryAvpSetVal [ReAuthRequest_CCREQUESTTYPE] = 1;
	if(mCCRequestType == NULL)
	{
		mCCRequestType = new CCRequestType();
		mTagsPresent[ReAuthRequest_CCREQUESTTYPE] = 1;
		return mCCRequestType;
	}
	return mCCRequestType;
}

void ReAuthRequest::resetCCRequestType(){
	mTagsPresent[ReAuthRequest_CCREQUESTTYPE] = 0;
	if(mCCRequestType != NULL)
	{
		delete mCCRequestType; 
		mCCRequestType = NULL;
...
OctetString* ReAuthRequest::getUserName(){
	if(NULL == mUserName) {
...
list<ProxyInfo*>* ReAuthRequest::getProxyInfoGrpList()
{
	if(mProxyInfoGrpList == NULL)
	{
...
list<ProxyInfo*>* ReAuthRequest::setProxyInfoGrpList(){
...
CCRequestType* ReAuthRequest::getCCRequestType(){
	...
	for(unsigned int i = 0 ; i < mTagsPresent.size() ; ++i )
	{
		...
		switch(i)
		{
			...
			case ReAuthRequest_ROUTERECORD:
			{
				...
				break;
			case ReAuthRequest_CCREQUESTTYPE:
			{
					array->size+=4;
