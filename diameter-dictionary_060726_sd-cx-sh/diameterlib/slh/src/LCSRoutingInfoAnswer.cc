...
LCSRoutingInfoAnswer::LCSRoutingInfoAnswer()
{
	...
	mTagsPresent.reset();
	mMandatoryAvpCheckVal= bitset<19>(string("1110111111101101111"));
...
list<OctetString*>* LCSRoutingInfoAnswer::getGMLCAddressList()
{
	if(mGMLCAddressList == NULL)
	{
...
list<OctetString*>* LCSRoutingInfoAnswer::setGMLCAddressList() {
...
list<OctetString*>* LCSRoutingInfoAnswer::getPPRAddressList()
{
	if(mPPRAddressList == NULL)
	{
...
list<OctetString*>* LCSRoutingInfoAnswer::setPPRAddressList() {
	mTagsPresent[LCSRoutingInfoAnswer_PPRADDRESS] = 1;
	if(mPPRAddressList == NULL)
		mPPRAddressList = new list<OctetString*>;
...
list<SLH::ProxyInfo*>* LCSRoutingInfoAnswer::getProxyInfoGrpList()
{
	if(mProxyInfoGrpList == NULL)
	{
	...
	for(auto itr : *mProxyInfoGrpList)
	{
		...
		{
			mTagsPresent[LCSRoutingInfoAnswer_PROXYINFO] = 0;
			mProxyInfoGrpList = NULL;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "ProxyInfo is Null");
			throw lTssDiaMsgException;
		}
...
list<SLH::ProxyInfo*>* LCSRoutingInfoAnswer::setProxyInfoGrpList(){
	mTagsPresent[LCSRoutingInfoAnswer_PROXYINFO] = 1;
	if(mProxyInfoGrpList == NULL)
		mProxyInfoGrpList = new list<SLH::ProxyInfo*>;
	mMandatoryAvpSetVal[LCSRoutingInfoAnswer_PROXYINFO] = 1;
	mProxyInfoGrpList->clear();
	return mProxyInfoGrpList;
}

void LCSRoutingInfoAnswer::resetProxyInfoGrpList(){
	mTagsPresent[LCSRoutingInfoAnswer_PROXYINFO] = 0;
	if(mProxyInfoGrpList != NULL)
		delete mProxyInfoGrpList;
}
	...
	for(unsigned int i = 0 ; i < mTagsPresent.size() ; ++i )
	{
		...
		switch(i)
		{
			...
			case LCSRoutingInfoAnswer_FAILEDAVP:
			{
				...
				break;
			case LCSRoutingInfoAnswer_PROXYINFO:
			{
				if( mProxyInfoGrpList->size() <= 0) {
					break;
				}
			...
			case 2407:
				{
						mMandatoryAvpSetVal [LCSRoutingInfoAnswer_PPRADDRESS] = 1;
					mTagsPresent[LCSRoutingInfoAnswer_PPRADDRESS] = 1;
					OctetString *lPPRAddress = new OctetString();
					if( lPPRAddress == NULL ){
						mDecodeFlag = false;
						TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_DECODE,"Error decoding PPRAddress");
						throw lTssDiaMsgException;
					...
					lPPRAddress->SetDataType((DiameterDataType)5);
