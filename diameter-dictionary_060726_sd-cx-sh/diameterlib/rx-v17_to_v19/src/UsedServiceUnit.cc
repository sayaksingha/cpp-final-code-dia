...
TariffTimeChange* UsedServiceUnit::setTariffTimeChange(){
	mMandatoryAvpSetVal [UsedServiceUnit_TARIFFTIMECHANGE] = 1;
	if(mTariffTimeChange == NULL)
	{
		mTariffTimeChange = new TariffTimeChange();
		mTagsPresent[UsedServiceUnit_TARIFFTIMECHANGE] = 1;
		return mTariffTimeChange;
	}
	return mTariffTimeChange;
}

void UsedServiceUnit::resetTariffTimeChange(){
	mTagsPresent[UsedServiceUnit_TARIFFTIMECHANGE] = 0;
	if(mTariffTimeChange != NULL)
	{
		delete mTariffTimeChange; 
		mTariffTimeChange = NULL;
...
Unsigned64* UsedServiceUnit::setCCServiceSpecificUnits(){
	mMandatoryAvpSetVal [UsedServiceUnit_CCSERVICESPECIFICUNITS] = 1;
	if(mCCServiceSpecificUnits == NULL)
	{
		mCCServiceSpecificUnits = new Unsigned64();
		mTagsPresent[UsedServiceUnit_CCSERVICESPECIFICUNITS] = 1;
		return mCCServiceSpecificUnits;
	}
	return mCCServiceSpecificUnits;
}

void UsedServiceUnit::resetCCServiceSpecificUnits(){
	mTagsPresent[UsedServiceUnit_CCSERVICESPECIFICUNITS] = 0;
	if(mCCServiceSpecificUnits != NULL)
	{
		delete mCCServiceSpecificUnits; 
		mCCServiceSpecificUnits = NULL;
...
TariffTimeChange* UsedServiceUnit::getTariffTimeChange(){
...
list<CCMoney*>* UsedServiceUnit::getCCMoneyGrpList()
{
	if(mCCMoneyGrpList == NULL)
	{
	...
	for(auto itr : *mCCMoneyGrpList)
	{
		...
		{
			mTagsPresent[UsedServiceUnit_CCMONEY] = 0;
			mCCMoneyGrpList = NULL;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "CCMoney is Null");
			throw lTssDiaMsgException;
		}
...
list<CCMoney*>* UsedServiceUnit::setCCMoneyGrpList(){
	mTagsPresent[UsedServiceUnit_CCMONEY] = 1;
	if(mCCMoneyGrpList == NULL)
		mCCMoneyGrpList = new list<CCMoney*>;
	mMandatoryAvpSetVal[UsedServiceUnit_CCMONEY] = 1;
	mCCMoneyGrpList->clear();
	return mCCMoneyGrpList;
}

void UsedServiceUnit::resetCCMoneyGrpList(){
	mTagsPresent[UsedServiceUnit_CCMONEY] = 0;
	if(mCCMoneyGrpList != NULL)
		delete mCCMoneyGrpList;
}
...
Unsigned64* UsedServiceUnit::getCCServiceSpecificUnits(){
...
int UsedServiceUnit::Encode(byteArray *array)
{
	...
	for(unsigned int i = 0 ; i < mTagsPresent.size() ; ++i )
	{
		...
		switch(i)
		{
			case UsedServiceUnit_TARIFFTIMECHANGE:
			{
					array->size+=4;
			...
			case UsedServiceUnit_CCOUTPUTOCTETS:
			{
				...
				break;
			case UsedServiceUnit_CCSERVICESPECIFICUNITS:
			{
					array->size+=4;
