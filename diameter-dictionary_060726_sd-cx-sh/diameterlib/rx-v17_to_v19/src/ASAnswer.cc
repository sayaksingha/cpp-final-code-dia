...
using namespace RX;
BOOLEAN ASAnswer::isPresent(int tag){
...
ASAnswer::ASAnswer()
{
	...
	mTagsPresent.reset();
	mMandatoryAvpCheckVal= bitset<15>(string("111111010011101"));
...
OctetString* ASAnswer::setSessionId(){
	mMandatoryAvpSetVal [ASAnswer_SESSIONID] = 1;
	if(mSessionId == NULL)
	{
		mSessionId = new OctetString();
		mTagsPresent[ASAnswer_SESSIONID] = 1;
		return mSessionId;
	}
	return mSessionId;
}

void ASAnswer::resetSessionId(){
	mTagsPresent[ASAnswer_SESSIONID] = 0;
	if(mSessionId != NULL)
	{
		delete mSessionId; 
		mSessionId = NULL;
...
DRMP* ASAnswer::setDRMP(){
	if(mDRMP == NULL)
	{
		mDRMP = new DRMP();
		mTagsPresent[ASAnswer_DRMP] = 1;
		return mDRMP;
	}
	return mDRMP;
}

void ASAnswer::resetDRMP(){
	mTagsPresent[ASAnswer_DRMP] = 0;
	if(mDRMP != NULL)
	{
		delete mDRMP; 
		mDRMP = NULL;
...
OctetString* ASAnswer::setOriginHost(){
	mMandatoryAvpSetVal [ASAnswer_ORIGINHOST] = 1;
	if(mOriginHost == NULL)
	{
		mOriginHost = new OctetString();
		mTagsPresent[ASAnswer_ORIGINHOST] = 1;
		return mOriginHost;
	}
	return mOriginHost;
}

void ASAnswer::resetOriginHost(){
	mTagsPresent[ASAnswer_ORIGINHOST] = 0;
	if(mOriginHost != NULL)
	{
		delete mOriginHost; 
		mOriginHost = NULL;
...
OctetString* ASAnswer::setOriginRealm(){
	mMandatoryAvpSetVal [ASAnswer_ORIGINREALM] = 1;
	if(mOriginRealm == NULL)
	{
		mOriginRealm = new OctetString();
		mTagsPresent[ASAnswer_ORIGINREALM] = 1;
		return mOriginRealm;
	}
	return mOriginRealm;
}

void ASAnswer::resetOriginRealm(){
	mTagsPresent[ASAnswer_ORIGINREALM] = 0;
	if(mOriginRealm != NULL)
	{
		delete mOriginRealm; 
		mOriginRealm = NULL;
...
Unsigned32* ASAnswer::setResultCode(){
	mMandatoryAvpSetVal [ASAnswer_RESULTCODE] = 1;
	if(mResultCode == NULL)
	{
		mResultCode = new Unsigned32();
		mTagsPresent[ASAnswer_RESULTCODE] = 1;
		return mResultCode;
	}
	return mResultCode;
}

void ASAnswer::resetResultCode(){
	mTagsPresent[ASAnswer_RESULTCODE] = 0;
	if(mResultCode != NULL)
	{
		delete mResultCode; 
		mResultCode = NULL;
...
Unsigned32* ASAnswer::setOriginStateId(){
	mMandatoryAvpSetVal [ASAnswer_ORIGINSTATEID] = 1;
	if(mOriginStateId == NULL)
	{
		mOriginStateId = new Unsigned32();
		mTagsPresent[ASAnswer_ORIGINSTATEID] = 1;
		return mOriginStateId;
	}
	return mOriginStateId;
}

void ASAnswer::resetOriginStateId(){
	mTagsPresent[ASAnswer_ORIGINSTATEID] = 0;
	if(mOriginStateId != NULL)
	{
		delete mOriginStateId; 
		mOriginStateId = NULL;
...
OctetString* ASAnswer::setErrorMessage(){
	if(mErrorMessage == NULL)
	{
		mErrorMessage = new OctetString();
		mTagsPresent[ASAnswer_ERRORMESSAGE] = 1;
		return mErrorMessage;
	}
	return mErrorMessage;
}

void ASAnswer::resetErrorMessage(){
	mTagsPresent[ASAnswer_ERRORMESSAGE] = 0;
	if(mErrorMessage != NULL)
	{
		delete mErrorMessage; 
		mErrorMessage = NULL;
...
OctetString* ASAnswer::setErrorReportingHost(){
	mMandatoryAvpSetVal [ASAnswer_ERRORREPORTINGHOST] = 1;
	if(mErrorReportingHost == NULL)
	{
		mErrorReportingHost = new OctetString();
		mTagsPresent[ASAnswer_ERRORREPORTINGHOST] = 1;
		return mErrorReportingHost;
	}
	return mErrorReportingHost;
}

void ASAnswer::resetErrorReportingHost(){
	mTagsPresent[ASAnswer_ERRORREPORTINGHOST] = 0;
	if(mErrorReportingHost != NULL)
	{
		delete mErrorReportingHost; 
		mErrorReportingHost = NULL;
...
RedirectHostUsage* ASAnswer::setRedirectHostUsage(){
	mMandatoryAvpSetVal [ASAnswer_REDIRECTHOSTUSAGE] = 1;
	if(mRedirectHostUsage == NULL)
	{
		mRedirectHostUsage = new RedirectHostUsage();
		mTagsPresent[ASAnswer_REDIRECTHOSTUSAGE] = 1;
		return mRedirectHostUsage;
	}
	return mRedirectHostUsage;
}

void ASAnswer::resetRedirectHostUsage(){
	mTagsPresent[ASAnswer_REDIRECTHOSTUSAGE] = 0;
	if(mRedirectHostUsage != NULL)
	{
		delete mRedirectHostUsage; 
		mRedirectHostUsage = NULL;
...
Unsigned32* ASAnswer::setRedirectMaxCacheTime(){
	mMandatoryAvpSetVal [ASAnswer_REDIRECTMAXCACHETIME] = 1;
	if(mRedirectMaxCacheTime == NULL)
	{
		mRedirectMaxCacheTime = new Unsigned32();
		mTagsPresent[ASAnswer_REDIRECTMAXCACHETIME] = 1;
		return mRedirectMaxCacheTime;
	}
	return mRedirectMaxCacheTime;
}

void ASAnswer::resetRedirectMaxCacheTime(){
	mTagsPresent[ASAnswer_REDIRECTMAXCACHETIME] = 0;
	if(mRedirectMaxCacheTime != NULL)
	{
		delete mRedirectMaxCacheTime; 
		mRedirectMaxCacheTime = NULL;
...
OctetString* ASAnswer::getSessionId(){
	if(NULL == mSessionId) {
...
OctetString* ASAnswer::getOriginHost(){
	if(NULL == mOriginHost) {
...
OctetString* ASAnswer::getOriginRealm(){
...
Unsigned32* ASAnswer::getResultCode(){
	if(NULL == mResultCode) {
...
OCSupportedFeatures* ASAnswer::getOCSupportedFeatures()
{
	if(mOCSupportedFeaturesGrp == NULL)
	{
...
OCSupportedFeatures* ASAnswer::setOCSupportedFeatures(){
	if(mOCSupportedFeaturesGrp == NULL)
		mOCSupportedFeaturesGrp = new OCSupportedFeatures();
	mTagsPresent[ASAnswer_OCSUPPORTEDFEATURES] = 1;
	return mOCSupportedFeaturesGrp;
}

void ASAnswer::resetOCSupportedFeatures(){
	mTagsPresent[ASAnswer_OCSUPPORTEDFEATURES] = 0;
	if(mOCSupportedFeaturesGrp != NULL)
		delete mOCSupportedFeaturesGrp;
}

OCOLR* ASAnswer::getOCOLR()
{
	if(mOCOLRGrp == NULL)
	{
...
OCOLR* ASAnswer::setOCOLR(){
	if(mOCOLRGrp == NULL)
		mOCOLRGrp = new OCOLR();
	mTagsPresent[ASAnswer_OCOLR] = 1;
	return mOCOLRGrp;
}

void ASAnswer::resetOCOLR(){
	mTagsPresent[ASAnswer_OCOLR] = 0;
	if(mOCOLRGrp != NULL)
		delete mOCOLRGrp;
}

Unsigned32* ASAnswer::getOriginStateId(){
...
OctetString* ASAnswer::getErrorMessage(){
...
OctetString* ASAnswer::getErrorReportingHost(){
...
list<FailedAVP*>* ASAnswer::getFailedAVPGrpList()
{
	if(mFailedAVPGrpList == NULL)
	{
	...
	for(auto itr : *mFailedAVPGrpList)
	{
		...
		{
			mTagsPresent[ASAnswer_FAILEDAVP] = 0;
			mFailedAVPGrpList = NULL;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "FailedAVP is Null");
			throw lTssDiaMsgException;
		}
...
list<FailedAVP*>* ASAnswer::setFailedAVPGrpList(){
	mTagsPresent[ASAnswer_FAILEDAVP] = 1;
	if(mFailedAVPGrpList == NULL)
		mFailedAVPGrpList = new list<FailedAVP*>;
	mMandatoryAvpSetVal[ASAnswer_FAILEDAVP] = 1;
	mFailedAVPGrpList->clear();
	return mFailedAVPGrpList;
}

void ASAnswer::resetFailedAVPGrpList(){
	mTagsPresent[ASAnswer_FAILEDAVP] = 0;
	if(mFailedAVPGrpList != NULL)
		delete mFailedAVPGrpList;
}

list<OctetString*>* ASAnswer::getRedirectHostList()
{
	if(mRedirectHostList == NULL)
	{
...
list<OctetString*>* ASAnswer::setRedirectHostList() {
	mTagsPresent[ASAnswer_REDIRECTHOST] = 1;
	if(mRedirectHostList == NULL)
		mRedirectHostList = new list<OctetString*>;
	mMandatoryAvpSetVal [ASAnswer_REDIRECTHOST] = 1;
	mRedirectHostList->clear();
	return mRedirectHostList;
}

void ASAnswer::resetRedirectHostList(){
	mTagsPresent[ASAnswer_REDIRECTHOST] = 0;
	if(mRedirectHostList != NULL)
		delete mRedirectHostList;
}

RedirectHostUsage* ASAnswer::getRedirectHostUsage(){
...
Unsigned32* ASAnswer::getRedirectMaxCacheTime(){
...
list<RX::ProxyInfo*>* ASAnswer::getProxyInfoGrpList()
{
	if(mProxyInfoGrpList == NULL)
	{
	...
	for(auto itr : *mProxyInfoGrpList)
	{
		...
		{
			mTagsPresent[ASAnswer_PROXYINFO] = 0;
			mProxyInfoGrpList = NULL;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "ProxyInfo is Null");
			throw lTssDiaMsgException;
		}
...
list<RX::ProxyInfo*>* ASAnswer::setProxyInfoGrpList(){
	mTagsPresent[ASAnswer_PROXYINFO] = 1;
	if(mProxyInfoGrpList == NULL)
		mProxyInfoGrpList = new list<RX::ProxyInfo*>;
	mMandatoryAvpSetVal[ASAnswer_PROXYINFO] = 1;
	mProxyInfoGrpList->clear();
	return mProxyInfoGrpList;
}

void ASAnswer::resetProxyInfoGrpList(){
	mTagsPresent[ASAnswer_PROXYINFO] = 0;
	if(mProxyInfoGrpList != NULL)
		delete mProxyInfoGrpList;
}
...
int ASAnswer::Encode(byteArray *array)
{
	...
	for(unsigned int i = 0 ; i < mTagsPresent.size() ; ++i )
	{
		...
		switch(i)
		{
			...
			case ASAnswer_RESULTCODE:
			{
				...
				break;
			case ASAnswer_OCSUPPORTEDFEATURES:
			{
				array->size+=4;
			...
			case ASAnswer_ERRORMESSAGE:
			{
				...
				break;
			case ASAnswer_ERRORREPORTINGHOST:
			{
					array->size+=4;
			...
			case ASAnswer_REDIRECTHOST:
			{
				...
				break;
			case ASAnswer_REDIRECTHOSTUSAGE:
			{
					array->size+=4;
				...
				break;
			case ASAnswer_REDIRECTMAXCACHETIME:
			{
					array->size+=4;
...
int ASAnswer::Decode(byteArray *array, INT16 lMLen, UINT16 *parsedBytes)
{
...
DiameterBaseInterface* ASAnswer::Find(byteArray *array, UINT32 avpCode)
{
...
BOOLEAN ASAnswer::FindAll(byteArray *array, UINT32 avpCode, list<DiameterBaseInterface*>& pBaseInterface,BOOLEAN deepInspect)
{
		if(!mDecodeFlag && array != NULL)
			this->Decode(array);
...
std::shared_ptr<byteArray> ASAnswer::GetByteArray()
{
	return mspByteArray;
}


BOOLEAN ASAnswer::FindAll(byteArray *array, std::map<int,UINT32> &avpCodeMap, int &size, int key, list<DiameterBaseInterface*>& pBaseInterface,BOOLEAN deepInspect)
{
		if(!mDecodeFlag && array != NULL)
			this->Decode(array);
...
void ASAnswer::Display(UINT32 tab)
{
...
BOOLEAN ASAnswer::GetMissingAvpInfo(TEXT *lMissingAvpInfo)
{
