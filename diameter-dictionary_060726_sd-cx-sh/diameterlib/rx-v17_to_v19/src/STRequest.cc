...
using namespace RX;
BOOLEAN STRequest::isPresent(int tag){
...
STRequest::STRequest()
{
	...
	mTagsPresent.reset();
	mMandatoryAvpCheckVal= bitset<14>(string("11110011111101"));
...
STRequest::~STRequest()
{
	...
	if(mTerminationCause != NULL)
		delete mTerminationCause;
...
OctetString* STRequest::setSessionId(){
	mMandatoryAvpSetVal [STRequest_SESSIONID] = 1;
	if(mSessionId == NULL)
	{
		mSessionId = new OctetString();
		mTagsPresent[STRequest_SESSIONID] = 1;
		return mSessionId;
	}
	return mSessionId;
}

void STRequest::resetSessionId(){
	mTagsPresent[STRequest_SESSIONID] = 0;
	if(mSessionId != NULL)
	{
		delete mSessionId; 
		mSessionId = NULL;
...
DRMP* STRequest::setDRMP(){
	if(mDRMP == NULL)
	{
		mDRMP = new DRMP();
		mTagsPresent[STRequest_DRMP] = 1;
		return mDRMP;
	}
	return mDRMP;
}

void STRequest::resetDRMP(){
	mTagsPresent[STRequest_DRMP] = 0;
	if(mDRMP != NULL)
	{
		delete mDRMP; 
		mDRMP = NULL;
...
OctetString* STRequest::setOriginHost(){
	mMandatoryAvpSetVal [STRequest_ORIGINHOST] = 1;
	if(mOriginHost == NULL)
	{
		mOriginHost = new OctetString();
		mTagsPresent[STRequest_ORIGINHOST] = 1;
		return mOriginHost;
	}
	return mOriginHost;
}

void STRequest::resetOriginHost(){
	mTagsPresent[STRequest_ORIGINHOST] = 0;
	if(mOriginHost != NULL)
	{
		delete mOriginHost; 
		mOriginHost = NULL;
...
OctetString* STRequest::setOriginRealm(){
	mMandatoryAvpSetVal [STRequest_ORIGINREALM] = 1;
	if(mOriginRealm == NULL)
	{
		mOriginRealm = new OctetString();
		mTagsPresent[STRequest_ORIGINREALM] = 1;
		return mOriginRealm;
	}
	return mOriginRealm;
}

void STRequest::resetOriginRealm(){
	mTagsPresent[STRequest_ORIGINREALM] = 0;
	if(mOriginRealm != NULL)
	{
		delete mOriginRealm; 
		mOriginRealm = NULL;
...
OctetString* STRequest::setDestinationRealm(){
	mMandatoryAvpSetVal [STRequest_DESTINATIONREALM] = 1;
	if(mDestinationRealm == NULL)
	{
		mDestinationRealm = new OctetString();
		mTagsPresent[STRequest_DESTINATIONREALM] = 1;
		return mDestinationRealm;
	}
	return mDestinationRealm;
}

void STRequest::resetDestinationRealm(){
	mTagsPresent[STRequest_DESTINATIONREALM] = 0;
	if(mDestinationRealm != NULL)
	{
		delete mDestinationRealm; 
		mDestinationRealm = NULL;
...
Integer32* STRequest::setAuthApplicationId(){
	mMandatoryAvpSetVal [STRequest_AUTHAPPLICATIONID] = 1;
	if(mAuthApplicationId == NULL)
	{
		mAuthApplicationId = new Integer32();
		mTagsPresent[STRequest_AUTHAPPLICATIONID] = 1;
		return mAuthApplicationId;
	}
	return mAuthApplicationId;
}

void STRequest::resetAuthApplicationId(){
	mTagsPresent[STRequest_AUTHAPPLICATIONID] = 0;
	if(mAuthApplicationId != NULL)
	{
		delete mAuthApplicationId; 
		mAuthApplicationId = NULL;
...
TerminationCause* STRequest::setTerminationCause(){
	mMandatoryAvpSetVal [STRequest_TERMINATIONCAUSE] = 1;
	if(mTerminationCause == NULL)
	{
		mTerminationCause = new TerminationCause();
		mTagsPresent[STRequest_TERMINATIONCAUSE] = 1;
		return mTerminationCause;
	}
	return mTerminationCause;
}

void STRequest::resetTerminationCause(){
	mTagsPresent[STRequest_TERMINATIONCAUSE] = 0;
	if(mTerminationCause != NULL)
	{
		delete mTerminationCause; 
		mTerminationCause = NULL;
...
OctetString* STRequest::setDestinationHost(){
	mMandatoryAvpSetVal [STRequest_DESTINATIONHOST] = 1;
	if(mDestinationHost == NULL)
	{
		mDestinationHost = new OctetString();
		mTagsPresent[STRequest_DESTINATIONHOST] = 1;
		return mDestinationHost;
	}
	return mDestinationHost;
}

void STRequest::resetDestinationHost(){
	mTagsPresent[STRequest_DESTINATIONHOST] = 0;
	if(mDestinationHost != NULL)
	{
		delete mDestinationHost; 
		mDestinationHost = NULL;
...
OctetString* STRequest::setClass(){
	mMandatoryAvpSetVal [STRequest_CLASS] = 1;
	if(mClass == NULL)
	{
		mClass = new OctetString();
		mTagsPresent[STRequest_CLASS] = 1;
		return mClass;
	}
	return mClass;
}

void STRequest::resetClass(){
	mTagsPresent[STRequest_CLASS] = 0;
	if(mClass != NULL)
	{
		delete mClass; 
		mClass = NULL;
...
Unsigned32* STRequest::setOriginStateId(){
	mMandatoryAvpSetVal [STRequest_ORIGINSTATEID] = 1;
	if(mOriginStateId == NULL)
	{
		mOriginStateId = new Unsigned32();
		mTagsPresent[STRequest_ORIGINSTATEID] = 1;
		return mOriginStateId;
	}
	return mOriginStateId;
}

void STRequest::resetOriginStateId(){
	mTagsPresent[STRequest_ORIGINSTATEID] = 0;
	if(mOriginStateId != NULL)
	{
		delete mOriginStateId; 
		mOriginStateId = NULL;
...
OctetString* STRequest::getSessionId(){
	if(NULL == mSessionId) {
...
OctetString* STRequest::getOriginHost(){
	if(NULL == mOriginHost) {
...
OctetString* STRequest::getOriginRealm(){
...
OctetString* STRequest::getDestinationRealm(){
...
Integer32* STRequest::getAuthApplicationId(){
...
TerminationCause* STRequest::getTerminationCause(){
	if(NULL == mTerminationCause) {
		TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "TerminationCause is Null");
		throw lTssDiaMsgException;
	}
	if(!mTerminationCause->mAvpDecodedFlag)
	{
			if(mspByteArray.get() == NULL)
			{
			mTerminationCause->mAvpDecodedFlag = true;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "TerminationCause is Null");
			throw lTssDiaMsgException;
			}
		mspByteArray.get()->offset = 0;
		mspByteArray.get()->offset = mTerminationCause->getAvpOffsetLen().first;
		UINT16 lBytesDecoded = 0;
		if(mTerminationCause->Decode(mspByteArray.get(), mTerminationCause->getAvpOffsetLen().second, &lBytesDecoded)< 0)
		{
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_DECODE,"Error decoding TerminationCause")     ;
			throw lTssDiaMsgException;
		}
		mTerminationCause->mAvpDecodedFlag = true;
	}
...
OctetString* STRequest::getDestinationHost(){
...
OCSupportedFeatures* STRequest::getOCSupportedFeatures()
{
	if(mOCSupportedFeaturesGrp == NULL)
	{
...
OCSupportedFeatures* STRequest::setOCSupportedFeatures(){
	if(mOCSupportedFeaturesGrp == NULL)
		mOCSupportedFeaturesGrp = new OCSupportedFeatures();
	mTagsPresent[STRequest_OCSUPPORTEDFEATURES] = 1;
	return mOCSupportedFeaturesGrp;
}

void STRequest::resetOCSupportedFeatures(){
	mTagsPresent[STRequest_OCSUPPORTEDFEATURES] = 0;
	if(mOCSupportedFeaturesGrp != NULL)
		delete mOCSupportedFeaturesGrp;
}

list<OctetString*>* STRequest::getRequiredAccessInfoList()
{
	if(mRequiredAccessInfoList == NULL)
	{
...
list<OctetString*>* STRequest::setRequiredAccessInfoList() {
	mTagsPresent[STRequest_REQUIREDACCESSINFO] = 1;
	if(mRequiredAccessInfoList == NULL)
...
void STRequest::resetRequiredAccessInfoList(){
	mTagsPresent[STRequest_REQUIREDACCESSINFO] = 0;
	if(mRequiredAccessInfoList != NULL)
		delete mRequiredAccessInfoList;
}

OctetString* STRequest::getClass(){
	if(NULL == mClass) {
...
Unsigned32* STRequest::getOriginStateId(){
...
list<RX::ProxyInfo*>* STRequest::getProxyInfoGrpList()
{
	if(mProxyInfoGrpList == NULL)
	{
	...
	for(auto itr : *mProxyInfoGrpList)
	{
		...
		{
			mTagsPresent[STRequest_PROXYINFO] = 0;
			mProxyInfoGrpList = NULL;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "ProxyInfo is Null");
			throw lTssDiaMsgException;
		}
...
list<RX::ProxyInfo*>* STRequest::setProxyInfoGrpList(){
	mTagsPresent[STRequest_PROXYINFO] = 1;
	if(mProxyInfoGrpList == NULL)
		mProxyInfoGrpList = new list<RX::ProxyInfo*>;
	mMandatoryAvpSetVal[STRequest_PROXYINFO] = 1;
	mProxyInfoGrpList->clear();
	return mProxyInfoGrpList;
}

void STRequest::resetProxyInfoGrpList(){
	mTagsPresent[STRequest_PROXYINFO] = 0;
	if(mProxyInfoGrpList != NULL)
		delete mProxyInfoGrpList;
}

list<OctetString*>* STRequest::getRouteRecordList()
{
	if(mRouteRecordList == NULL)
	{
...
list<OctetString*>* STRequest::setRouteRecordList() {
	mTagsPresent[STRequest_ROUTERECORD] = 1;
	if(mRouteRecordList == NULL)
		mRouteRecordList = new list<OctetString*>;
	mMandatoryAvpSetVal [STRequest_ROUTERECORD] = 1;
	mRouteRecordList->clear();
	return mRouteRecordList;
}

void STRequest::resetRouteRecordList(){
	mTagsPresent[STRequest_ROUTERECORD] = 0;
	if(mRouteRecordList != NULL)
		delete mRouteRecordList;
}
...
int STRequest::Encode(byteArray *array)
{
	...
	for(unsigned int i = 0 ; i < mTagsPresent.size() ; ++i )
	{
		...
		switch(i)
		{
			...
			case STRequest_ORIGINREALM:
			{
				...
				break;
			case STRequest_DESTINATIONREALM:
			{
					array->size+=4;
				...
				break;
			case STRequest_AUTHAPPLICATIONID:
			{
					array->size+=4;
				...
				break;
			case STRequest_TERMINATIONCAUSE:
			{
					array->size+=4;
					lTempHtonlVar =  htonl(295);
					memcpy(array->byte+array->offset, &lTempHtonlVar, DIA_AVP_CODE_SIZE);
					array->offset += DIA_AVP_CODE_SIZE;
					array->size++;
					array->byte[array->offset++] = 64;
					...
					if(!mTerminationCause->mAvpDecodedFlag)
					{
						memcpy(array->byte+array->offset, &mspByteArray.get()->byte[mTerminationCause->getAvpOffsetLen().first], mTerminationCause->getAvpOffsetLen().second);
						array->offset += mTerminationCause->getAvpOffsetLen().second;
						array->size += mTerminationCause->getAvpOffsetLen().second;
						if(false == DiameterTLVUtil::avp_length_encode(lengthOffset, array)){
							TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_ENCODE,"Error encoding TerminationCause");
							throw lTssDiaMsgException;
						}
						continue;
					}
					if((mTerminationCause->Encode(array) < 0) || 
						(DiameterTLVUtil::avp_length_encode(lengthOffset, array) == false)){ 
						TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_ENCODE,"Error encoding TerminationCause");
						throw lTssDiaMsgException;
					}
				...
				break;
			case STRequest_DESTINATIONHOST:
			{
					array->size+=4;
				...
				break;
			case STRequest_OCSUPPORTEDFEATURES:
			{
				array->size+=4;
				...
				break;
			case STRequest_REQUIREDACCESSINFO:
			{
...
int STRequest::Decode(byteArray *array, INT16 lMLen, UINT16 *parsedBytes)
{
	...
	while( lMsgLen > 0)
	{
		...
		switch(lAvpCode)
		{
			...
			case 295:
				{
						mMandatoryAvpSetVal [STRequest_TERMINATIONCAUSE] = 1;
					if( NULL == this->setTerminationCause())
					{
						mDecodeFlag = false;
						TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_DECODE,"Error decoding TerminationCause");
						throw lTssDiaMsgException;
					}
					mTerminationCause->setAvpCode(295);
					mTerminationCause->setAvpVendorId(lVenderId);
					mTerminationCause->SetDataType((DiameterDataType)7);
					mTerminationCause->mspByteArray = mspByteArray;
					mTerminationCause->mAvpDecodedFlag = false;
					mTerminationCause->setAvpOffsetLen(array->offset,lAvpDataLen);
					array->offset+=lAvpDataLen;
...
DiameterBaseInterface* STRequest::Find(byteArray *array, UINT32 avpCode)
{
		...
		switch(avpCode)
		{
			...
			{
				if(mTerminationCause != NULL)
					return mTerminationCause->Find(array, avpCode);
				else
					return NULL;
			}
			case 293:
...
BOOLEAN STRequest::FindAll(byteArray *array, UINT32 avpCode, list<DiameterBaseInterface*>& pBaseInterface,BOOLEAN deepInspect)
{
		if(!mDecodeFlag && array != NULL)
			this->Decode(array);
		...
		switch(avpCode)
		{
			...
			case 295:
			{
				if(mTerminationCause != NULL)
				{
					DiameterBaseInterface* inf =  mTerminationCause->Find(this->mspByteArray.get(), avpCode);
					if(inf != NULL)
					{
						pBaseInterface.push_back(inf);
					}
...
std::shared_ptr<byteArray> STRequest::GetByteArray()
{
	return mspByteArray;
}


BOOLEAN STRequest::FindAll(byteArray *array, std::map<int,UINT32> &avpCodeMap, int &size, int key, list<DiameterBaseInterface*>& pBaseInterface,BOOLEAN deepInspect)
{
		if(!mDecodeFlag && array != NULL)
			this->Decode(array);
...
void STRequest::Display(UINT32 tab)
{
	...
	for(int i = 0; i < mTagsPresent.size() ; ++i)
	{
		...
		switch(i)
		{
			...
			case STRequest_TERMINATIONCAUSE:
			{
				cout << setw(tempTab) << "" << "|"  << left <<  setw(12) << "AvpCode" << ":" << setw(15) << "295" << "|" << setw(12)  << "AvpType" << ":" << setw(15) << "SINGLE" << "|" << "\n"; 
				cout << setw(tempTab) << "" << "|"  << left <<  setw(12) << "AvpName" << ":" << setw(44) << "TerminationCause" << "\n"; 
				TEXT lAvpFlg[100];
				//Hardcoded value.;
				sprintf( lAvpFlg, "%s %s %s", ((DiameterTLVUtil::isVenderSpecific(64)==true)?"Vendor":""), ((DiameterTLVUtil::isMandatoryBitSet(64)==true)?"Mandatory":""), ((DiameterTLVUtil::isProtectedBitSet(64)==true)?"Protected":""));
				this->getTerminationCause()->Display(tab+1);
			}
				break;
			case STRequest_DESTINATIONHOST:
			{
...
BOOLEAN STRequest::GetMissingAvpInfo(TEXT *lMissingAvpInfo)
{
	bitset<14> lMissingAvpTag(mMandatoryAvpSetVal ^ mMandatoryAvpCheckVal);
	...
	for(unsigned int i = 0 ; i < lMissingAvpTag.size(); ++i)
	{
		...
		switch(i)
		{
			...
			case STRequest_TERMINATIONCAUSE:
			{
				...
				{
					lFirstRec = false;
					sprintf(lMissingAvpInfo, "Missing Avps List: 295");
				}
				else
					sprintf(lMissingAvpInfo, "%s,295", lMissingAvpInfo);
			}
				break;
			case STRequest_DESTINATIONHOST:
			{
