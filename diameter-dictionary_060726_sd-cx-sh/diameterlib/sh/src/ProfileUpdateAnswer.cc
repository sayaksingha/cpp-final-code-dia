...
using namespace SH;
BOOLEAN ProfileUpdateAnswer::isPresent(int tag){
	return (mTagsPresent[tag])?true:false;
}

ProfileUpdateAnswer::ProfileUpdateAnswer()
{
	mSessionId = NULL;
	mDRMP = NULL;
	...
	mTagsPresent.reset();
	mMandatoryAvpCheckVal= bitset<19>(string("1110000100011111101"));
...
ProfileUpdateAnswer::~ProfileUpdateAnswer()
{
	...
	if(mRepositoryDataIDGrp != NULL)
		delete mRepositoryDataIDGrp;
	if(mDataReference != NULL)
		delete mDataReference;
...
OctetString* ProfileUpdateAnswer::setSessionId(){
	mMandatoryAvpSetVal [ProfileUpdateAnswer_SESSIONID] = 1;
	if(mSessionId == NULL)
	{
		mSessionId = new OctetString();
		mTagsPresent[ProfileUpdateAnswer_SESSIONID] = 1;
		return mSessionId;
	}
	return mSessionId;
}

void ProfileUpdateAnswer::resetSessionId(){
	mTagsPresent[ProfileUpdateAnswer_SESSIONID] = 0;
	if(mSessionId != NULL)
	{
		delete mSessionId; 
		mSessionId = NULL;
...
DRMP* ProfileUpdateAnswer::setDRMP(){
	if(mDRMP == NULL)
	{
		mDRMP = new DRMP();
		mTagsPresent[ProfileUpdateAnswer_DRMP] = 1;
		return mDRMP;
	}
	return mDRMP;
}

void ProfileUpdateAnswer::resetDRMP(){
	mTagsPresent[ProfileUpdateAnswer_DRMP] = 0;
	if(mDRMP != NULL)
	{
		delete mDRMP; 
		mDRMP = NULL;
...
Unsigned32* ProfileUpdateAnswer::setResultCode(){
	mMandatoryAvpSetVal [ProfileUpdateAnswer_RESULTCODE] = 1;
	if(mResultCode == NULL)
	{
		mResultCode = new Unsigned32();
		mTagsPresent[ProfileUpdateAnswer_RESULTCODE] = 1;
		return mResultCode;
	}
	return mResultCode;
}

void ProfileUpdateAnswer::resetResultCode(){
	mTagsPresent[ProfileUpdateAnswer_RESULTCODE] = 0;
	if(mResultCode != NULL)
	{
		delete mResultCode; 
		mResultCode = NULL;
...
AuthSessionState* ProfileUpdateAnswer::setAuthSessionState(){
	mMandatoryAvpSetVal [ProfileUpdateAnswer_AUTHSESSIONSTATE] = 1;
	if(mAuthSessionState == NULL)
	{
		mAuthSessionState = new AuthSessionState();
		mTagsPresent[ProfileUpdateAnswer_AUTHSESSIONSTATE] = 1;
		return mAuthSessionState;
	}
	return mAuthSessionState;
}

void ProfileUpdateAnswer::resetAuthSessionState(){
	mTagsPresent[ProfileUpdateAnswer_AUTHSESSIONSTATE] = 0;
	if(mAuthSessionState != NULL)
	{
		delete mAuthSessionState; 
		mAuthSessionState = NULL;
...
OctetString* ProfileUpdateAnswer::setOriginHost(){
	mMandatoryAvpSetVal [ProfileUpdateAnswer_ORIGINHOST] = 1;
	if(mOriginHost == NULL)
	{
		mOriginHost = new OctetString();
		mTagsPresent[ProfileUpdateAnswer_ORIGINHOST] = 1;
		return mOriginHost;
	}
	return mOriginHost;
}

void ProfileUpdateAnswer::resetOriginHost(){
	mTagsPresent[ProfileUpdateAnswer_ORIGINHOST] = 0;
	if(mOriginHost != NULL)
	{
		delete mOriginHost; 
		mOriginHost = NULL;
...
OctetString* ProfileUpdateAnswer::setOriginRealm(){
	mMandatoryAvpSetVal [ProfileUpdateAnswer_ORIGINREALM] = 1;
	if(mOriginRealm == NULL)
	{
		mOriginRealm = new OctetString();
		mTagsPresent[ProfileUpdateAnswer_ORIGINREALM] = 1;
		return mOriginRealm;
	}
	return mOriginRealm;
}

void ProfileUpdateAnswer::resetOriginRealm(){
	mTagsPresent[ProfileUpdateAnswer_ORIGINREALM] = 0;
	if(mOriginRealm != NULL)
	{
		delete mOriginRealm; 
		mOriginRealm = NULL;
...
OctetString* ProfileUpdateAnswer::setWildcardedPublicIdentity(){
	if(mWildcardedPublicIdentity == NULL)
	{
		mWildcardedPublicIdentity = new OctetString();
		mTagsPresent[ProfileUpdateAnswer_WILDCARDEDPUBLICIDENTITY] = 1;
		return mWildcardedPublicIdentity;
	}
	return mWildcardedPublicIdentity;
}

void ProfileUpdateAnswer::resetWildcardedPublicIdentity(){
	mTagsPresent[ProfileUpdateAnswer_WILDCARDEDPUBLICIDENTITY] = 0;
	if(mWildcardedPublicIdentity != NULL)
	{
		delete mWildcardedPublicIdentity; 
		mWildcardedPublicIdentity = NULL;
...
OctetString* ProfileUpdateAnswer::setWildcardedIMPU(){
	if(mWildcardedIMPU == NULL)
	{
		mWildcardedIMPU = new OctetString();
		mTagsPresent[ProfileUpdateAnswer_WILDCARDEDIMPU] = 1;
		return mWildcardedIMPU;
	}
	return mWildcardedIMPU;
}

void ProfileUpdateAnswer::resetWildcardedIMPU(){
	mTagsPresent[ProfileUpdateAnswer_WILDCARDEDIMPU] = 0;
	if(mWildcardedIMPU != NULL)
	{
		delete mWildcardedIMPU; 
		mWildcardedIMPU = NULL;
...
DataReference* ProfileUpdateAnswer::setDataReference(){
	mMandatoryAvpSetVal [ProfileUpdateAnswer_DATAREFERENCE] = 1;
	if(mDataReference == NULL)
	{
		mDataReference = new DataReference();
		mTagsPresent[ProfileUpdateAnswer_DATAREFERENCE] = 1;
		return mDataReference;
	}
	return mDataReference;
}

void ProfileUpdateAnswer::resetDataReference(){
	mTagsPresent[ProfileUpdateAnswer_DATAREFERENCE] = 0;
	if(mDataReference != NULL)
	{
		delete mDataReference; 
		mDataReference = NULL;
...
OctetString* ProfileUpdateAnswer::getSessionId(){
	if(NULL == mSessionId) {
...
DRMP* ProfileUpdateAnswer::getDRMP(){
	if(NULL == mDRMP) {
...
SH::VendorSpecificApplicationId* ProfileUpdateAnswer::getVendorSpecificApplicationId()
{
	if(mVendorSpecificApplicationIdGrp == NULL)
	{
...
SH::VendorSpecificApplicationId* ProfileUpdateAnswer::setVendorSpecificApplicationId(){
	if(mVendorSpecificApplicationIdGrp == NULL)
		mVendorSpecificApplicationIdGrp = new VendorSpecificApplicationId();
	mTagsPresent[ProfileUpdateAnswer_VENDORSPECIFICAPPLICATIONID] = 1;
	mMandatoryAvpSetVal[ProfileUpdateAnswer_VENDORSPECIFICAPPLICATIONID] = 1;
	return mVendorSpecificApplicationIdGrp;
}

void ProfileUpdateAnswer::resetVendorSpecificApplicationId(){
	mTagsPresent[ProfileUpdateAnswer_VENDORSPECIFICAPPLICATIONID] = 0;
	if(mVendorSpecificApplicationIdGrp != NULL)
		delete mVendorSpecificApplicationIdGrp;
}

Unsigned32* ProfileUpdateAnswer::getResultCode(){
	if(NULL == mResultCode) {
...
ExperimentalResult* ProfileUpdateAnswer::getExperimentalResult()
{
	if(mExperimentalResultGrp == NULL)
	{
...
ExperimentalResult* ProfileUpdateAnswer::setExperimentalResult(){
	if(mExperimentalResultGrp == NULL)
		mExperimentalResultGrp = new ExperimentalResult();
	mTagsPresent[ProfileUpdateAnswer_EXPERIMENTALRESULT] = 1;
	mMandatoryAvpSetVal[ProfileUpdateAnswer_EXPERIMENTALRESULT] = 1;
	return mExperimentalResultGrp;
}

void ProfileUpdateAnswer::resetExperimentalResult(){
	mTagsPresent[ProfileUpdateAnswer_EXPERIMENTALRESULT] = 0;
	if(mExperimentalResultGrp != NULL)
		delete mExperimentalResultGrp;
}

AuthSessionState* ProfileUpdateAnswer::getAuthSessionState(){
...
OctetString* ProfileUpdateAnswer::getOriginHost(){
	if(NULL == mOriginHost) {
...
OctetString* ProfileUpdateAnswer::getOriginRealm(){
...
OctetString* ProfileUpdateAnswer::getWildcardedPublicIdentity(){
...
OctetString* ProfileUpdateAnswer::getWildcardedIMPU(){
...
RepositoryDataID* ProfileUpdateAnswer::getRepositoryDataID()
{
	if(mRepositoryDataIDGrp == NULL)
	{
		TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "RepositoryDataID is Null");
		throw lTssDiaMsgException;
	}
	if(!mRepositoryDataIDGrp->mAvpDecodedFlag)
	{
			if(mspByteArray.get() == NULL)
			{
			mRepositoryDataIDGrp->mAvpDecodedFlag = true;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "RepositoryDataID is Null");
			throw lTssDiaMsgException;
			}
		mspByteArray.get()->offset = 0;
		mspByteArray.get()->offset = mRepositoryDataIDGrp->getAvpOffsetLen().first;
		UINT16 lBytesDecoded = 0;
		if(mRepositoryDataIDGrp->Decode(mspByteArray.get(), mRepositoryDataIDGrp->getAvpOffsetLen().second, &lBytesDecoded)< 0)
		{
	...
	return mRepositoryDataIDGrp;
}

RepositoryDataID* ProfileUpdateAnswer::setRepositoryDataID(){
	if(mRepositoryDataIDGrp == NULL)
		mRepositoryDataIDGrp = new RepositoryDataID();
	mTagsPresent[ProfileUpdateAnswer_REPOSITORYDATAID] = 1;
	return mRepositoryDataIDGrp;
}

void ProfileUpdateAnswer::resetRepositoryDataID(){
	mTagsPresent[ProfileUpdateAnswer_REPOSITORYDATAID] = 0;
	if(mRepositoryDataIDGrp != NULL)
		delete mRepositoryDataIDGrp;
}

DataReference* ProfileUpdateAnswer::getDataReference(){
	if(NULL == mDataReference) {
		TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "DataReference is Null");
		throw lTssDiaMsgException;
	}
	if(!mDataReference->mAvpDecodedFlag)
	{
			if(mspByteArray.get() == NULL)
			{
			mDataReference->mAvpDecodedFlag = true;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "DataReference is Null");
			throw lTssDiaMsgException;
			}
		mspByteArray.get()->offset = 0;
		mspByteArray.get()->offset = mDataReference->getAvpOffsetLen().first;
		UINT16 lBytesDecoded = 0;
		if(mDataReference->Decode(mspByteArray.get(), mDataReference->getAvpOffsetLen().second, &lBytesDecoded)< 0)
		{
...
list<SupportedFeatures*>* ProfileUpdateAnswer::getSupportedFeaturesGrpList()
{
	if(mSupportedFeaturesGrpList == NULL)
	{
	...
	for(auto itr : *mSupportedFeaturesGrpList)
	{
		...
		{
			mTagsPresent[ProfileUpdateAnswer_SUPPORTEDFEATURES] = 0;
			mSupportedFeaturesGrpList = NULL;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "SupportedFeatures is Null");
			throw lTssDiaMsgException;
		}
...
list<SupportedFeatures*>* ProfileUpdateAnswer::setSupportedFeaturesGrpList(){
	mTagsPresent[ProfileUpdateAnswer_SUPPORTEDFEATURES] = 1;
	if(mSupportedFeaturesGrpList == NULL)
...
void ProfileUpdateAnswer::resetSupportedFeaturesGrpList(){
	mTagsPresent[ProfileUpdateAnswer_SUPPORTEDFEATURES] = 0;
	if(mSupportedFeaturesGrpList != NULL)
		delete mSupportedFeaturesGrpList;
}

OCSupportedFeatures* ProfileUpdateAnswer::getOCSupportedFeatures()
{
	if(mOCSupportedFeaturesGrp == NULL)
	{
...
OCSupportedFeatures* ProfileUpdateAnswer::setOCSupportedFeatures(){
	if(mOCSupportedFeaturesGrp == NULL)
		mOCSupportedFeaturesGrp = new OCSupportedFeatures();
	mTagsPresent[ProfileUpdateAnswer_OCSUPPORTEDFEATURES] = 1;
	return mOCSupportedFeaturesGrp;
}

void ProfileUpdateAnswer::resetOCSupportedFeatures(){
	mTagsPresent[ProfileUpdateAnswer_OCSUPPORTEDFEATURES] = 0;
	if(mOCSupportedFeaturesGrp != NULL)
		delete mOCSupportedFeaturesGrp;
}

OCOLR* ProfileUpdateAnswer::getOCOLR()
{
	if(mOCOLRGrp == NULL)
	{
...
OCOLR* ProfileUpdateAnswer::setOCOLR(){
	if(mOCOLRGrp == NULL)
		mOCOLRGrp = new OCOLR();
	mTagsPresent[ProfileUpdateAnswer_OCOLR] = 1;
	return mOCOLRGrp;
}

void ProfileUpdateAnswer::resetOCOLR(){
	mTagsPresent[ProfileUpdateAnswer_OCOLR] = 0;
	if(mOCOLRGrp != NULL)
		delete mOCOLRGrp;
}

list<Load*>* ProfileUpdateAnswer::getLoadGrpList()
{
	if(mLoadGrpList == NULL)
	{
	...
	for(auto itr : *mLoadGrpList)
	{
		...
		{
			mTagsPresent[ProfileUpdateAnswer_LOAD] = 0;
			mLoadGrpList = NULL;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "Load is Null");
			throw lTssDiaMsgException;
		}
...
list<Load*>* ProfileUpdateAnswer::setLoadGrpList(){
	mTagsPresent[ProfileUpdateAnswer_LOAD] = 1;
	if(mLoadGrpList == NULL)
...
void ProfileUpdateAnswer::resetLoadGrpList(){
	mTagsPresent[ProfileUpdateAnswer_LOAD] = 0;
	if(mLoadGrpList != NULL)
		delete mLoadGrpList;
}

FailedAVP* ProfileUpdateAnswer::getFailedAVP()
{
	if(mFailedAVPGrp == NULL)
	{
...
FailedAVP* ProfileUpdateAnswer::setFailedAVP(){
	if(mFailedAVPGrp == NULL)
		mFailedAVPGrp = new FailedAVP();
	mTagsPresent[ProfileUpdateAnswer_FAILEDAVP] = 1;
	mMandatoryAvpSetVal[ProfileUpdateAnswer_FAILEDAVP] = 1;
	return mFailedAVPGrp;
}

void ProfileUpdateAnswer::resetFailedAVP(){
	mTagsPresent[ProfileUpdateAnswer_FAILEDAVP] = 0;
	if(mFailedAVPGrp != NULL)
		delete mFailedAVPGrp;
}

list<SH::ProxyInfo*>* ProfileUpdateAnswer::getProxyInfoGrpList()
{
	if(mProxyInfoGrpList == NULL)
	{
	...
	for(auto itr : *mProxyInfoGrpList)
	{
		...
		{
			mTagsPresent[ProfileUpdateAnswer_PROXYINFO] = 0;
			mProxyInfoGrpList = NULL;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "ProxyInfo is Null");
			throw lTssDiaMsgException;
		}
...
list<SH::ProxyInfo*>* ProfileUpdateAnswer::setProxyInfoGrpList(){
	mTagsPresent[ProfileUpdateAnswer_PROXYINFO] = 1;
	if(mProxyInfoGrpList == NULL)
		mProxyInfoGrpList = new list<SH::ProxyInfo*>;
	mMandatoryAvpSetVal[ProfileUpdateAnswer_PROXYINFO] = 1;
	mProxyInfoGrpList->clear();
	return mProxyInfoGrpList;
}

void ProfileUpdateAnswer::resetProxyInfoGrpList(){
	mTagsPresent[ProfileUpdateAnswer_PROXYINFO] = 0;
	if(mProxyInfoGrpList != NULL)
		delete mProxyInfoGrpList;
}

list<OctetString*>* ProfileUpdateAnswer::getRouteRecordList()
{
	if(mRouteRecordList == NULL)
	{
...
list<OctetString*>* ProfileUpdateAnswer::setRouteRecordList() {
	mTagsPresent[ProfileUpdateAnswer_ROUTERECORD] = 1;
	if(mRouteRecordList == NULL)
		mRouteRecordList = new list<OctetString*>;
	mMandatoryAvpSetVal [ProfileUpdateAnswer_ROUTERECORD] = 1;
	mRouteRecordList->clear();
	return mRouteRecordList;
}

void ProfileUpdateAnswer::resetRouteRecordList(){
	mTagsPresent[ProfileUpdateAnswer_ROUTERECORD] = 0;
	if(mRouteRecordList != NULL)
		delete mRouteRecordList;
}
...
int ProfileUpdateAnswer::Encode(byteArray *array)
{
	...
	array->byte[array->offset++] = (307>>16) & 0xFF;
	array->byte[array->offset++] = (307>>8) & 0xFF;
	array->byte[array->offset++] = (307) & 0xFF;
	...
	for(unsigned int i = 0 ; i < mTagsPresent.size() ; ++i )
	{
		...
		switch(i)
		{
			case ProfileUpdateAnswer_SESSIONID:
			{
					array->size+=4;
			...
			case ProfileUpdateAnswer_DRMP:
			{
				...
				break;
			case ProfileUpdateAnswer_VENDORSPECIFICAPPLICATIONID:
			{
				array->size+=4;
				...
				break;
			case ProfileUpdateAnswer_RESULTCODE:
			{
					array->size+=4;
				...
				break;
			case ProfileUpdateAnswer_EXPERIMENTALRESULT:
			{
				array->size+=4;
				...
				break;
			case ProfileUpdateAnswer_AUTHSESSIONSTATE:
			{
					array->size+=4;
				...
				break;
			case ProfileUpdateAnswer_ORIGINHOST:
			{
					array->size+=4;
				...
				break;
			case ProfileUpdateAnswer_ORIGINREALM:
			{
					array->size+=4;
				...
				break;
			case ProfileUpdateAnswer_WILDCARDEDPUBLICIDENTITY:
			{
					array->size+=4;
				...
				break;
			case ProfileUpdateAnswer_WILDCARDEDIMPU:
			{
					array->size+=4;
				...
				break;
			case ProfileUpdateAnswer_REPOSITORYDATAID:
			{
				array->size+=4;
					lTempHtonlVar =  htonl(715);
					memcpy(array->byte+array->offset, &lTempHtonlVar, DIA_AVP_CODE_SIZE);
					array->offset += DIA_AVP_CODE_SIZE;
				array->size++;
				array->byte[array->offset++] = 128;
				...
				if((this->getRepositoryDataID()->Encode(array) < 0) || 
						(DiameterTLVUtil::avp_length_encode(lengthOffset, array) == false)) {
					TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_ENCODE,"Error encoding RepositoryDataID");
					throw lTssDiaMsgException;
					}
				}
				break;
			case ProfileUpdateAnswer_DATAREFERENCE:
			{
					array->size+=4;
					...
					if(!mDataReference->mAvpDecodedFlag)
					{
						memcpy(array->byte+array->offset, &mspByteArray.get()->byte[mDataReference->getAvpOffsetLen().first], mDataReference->getAvpOffsetLen().second);
						array->offset += mDataReference->getAvpOffsetLen().second;
						array->size += mDataReference->getAvpOffsetLen().second;
						if(false == DiameterTLVUtil::avp_length_encode(lengthOffset, array)){
					...
					if((mDataReference->Encode(array) < 0) || 
						(DiameterTLVUtil::avp_length_encode(lengthOffset, array) == false)){ 
						TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_ENCODE,"Error encoding DataReference");
						throw lTssDiaMsgException;
					}
				...
				break;
			case ProfileUpdateAnswer_SUPPORTEDFEATURES:
			{
				if( mSupportedFeaturesGrpList->size() <= 0) {
					break;
				}
				...
				break;
			case ProfileUpdateAnswer_OCSUPPORTEDFEATURES:
			{
				array->size+=4;
				...
				break;
			case ProfileUpdateAnswer_OCOLR:
			{
				array->size+=4;
			...
			case ProfileUpdateAnswer_LOAD:
			{
				...
				break;
			case ProfileUpdateAnswer_FAILEDAVP:
			{
				array->size+=4;
				...
				break;
			case ProfileUpdateAnswer_PROXYINFO:
			{
				if( mProxyInfoGrpList->size() <= 0) {
					break;
				}
				...
				break;
			case ProfileUpdateAnswer_ROUTERECORD:
			{
...
int ProfileUpdateAnswer::Decode(byteArray *array, INT16 lMLen, UINT16 *parsedBytes)
{
	...
	while( lMsgLen > 0)
	{
		...
		switch(lAvpCode)
		{
			...
			case 715:
				{
					mTagsPresent[ProfileUpdateAnswer_REPOSITORYDATAID] = 1;
						if( NULL == this->setRepositoryDataID()){
						mDecodeFlag = false;
						TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_DECODE,"Error decoding RepositoryDataID");
						throw lTssDiaMsgException;
						}
					mRepositoryDataIDGrp->setAvpCode(715);
					mRepositoryDataIDGrp->SetDataType((DiameterDataType)6);
					mRepositoryDataIDGrp->mspByteArray = mspByteArray;
					mRepositoryDataIDGrp->mAvpDecodedFlag = false;
					mRepositoryDataIDGrp->setAvpOffsetLen(array->offset,lAvpDataLen);
					mBaseInterfaceList.push_back(mRepositoryDataIDGrp);
					array->offset+=lAvpDataLen;
			...
			case 703:
				{
						mMandatoryAvpSetVal [ProfileUpdateAnswer_DATAREFERENCE] = 1;
					if( NULL == this->setDataReference())
					{
						mDecodeFlag = false;
						TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_DECODE,"Error decoding DataReference");
						throw lTssDiaMsgException;
					}
					mDataReference->setAvpCode(703);
					mDataReference->setAvpVendorId(lVenderId);
					mDataReference->SetDataType((DiameterDataType)7);
					mDataReference->mspByteArray = mspByteArray;
					mDataReference->mAvpDecodedFlag = false;
					mDataReference->setAvpOffsetLen(array->offset,lAvpDataLen);
					array->offset+=lAvpDataLen;
...
DiameterBaseInterface* ProfileUpdateAnswer::Find(byteArray *array, UINT32 avpCode)
{
		...
		switch(avpCode)
		{
			...
			{
				if(mDataReference != NULL)
					return mDataReference->Find(array, avpCode);
				else
					return NULL;
			}
			case 282:
...
BOOLEAN ProfileUpdateAnswer::FindAll(byteArray *array, UINT32 avpCode, list<DiameterBaseInterface*>& pBaseInterface,BOOLEAN deepInspect)
{
		if(!mDecodeFlag && array != NULL)
			this->Decode(array);
		...
		switch(avpCode)
		{
			...
			case 715:
			{
				if(mRepositoryDataIDGrp != NULL)
				{
						pBaseInterface.push_back(mRepositoryDataIDGrp);
				}
				if(deepInspect)
				{
					for(auto itr = mBaseInterfaceList.begin(); itr != mBaseInterfaceList.end(); ++itr)
			...
			case 703:
			{
				if(mDataReference != NULL)
				{
					DiameterBaseInterface* inf =  mDataReference->Find(this->mspByteArray.get(), avpCode);
					if(inf != NULL)
					{
						pBaseInterface.push_back(inf);
					}
...
std::shared_ptr<byteArray> ProfileUpdateAnswer::GetByteArray()
{
	return mspByteArray;
}


BOOLEAN ProfileUpdateAnswer::FindAll(byteArray *array, std::map<int,UINT32> &avpCodeMap, int &size, int key, list<DiameterBaseInterface*>& pBaseInterface,BOOLEAN deepInspect)
{
		if(!mDecodeFlag && array != NULL)
			this->Decode(array);
		...
		switch(avpCode)
		{
			...
			case 715:
			{
				...
				if(deepInspect)
				{
					for(auto itr = mBaseInterfaceList.begin(); itr != mBaseInterfaceList.end(); ++itr)
					{
						mRepositoryDataIDGrp->FindAll(this->mspByteArray.get(), avpCodeMap, size, (key+1),  pBaseInterface,deepInspect);
					}
				}
				}
				break;
...
void ProfileUpdateAnswer::Display(UINT32 tab)
{
	...
	for(int i = 0; i < mTagsPresent.size() ; ++i)
	{
		...
		switch(i)
		{
			...
			case ProfileUpdateAnswer_REPOSITORYDATAID:
			{
				cout << setw(tempTab) << "" << "|"  << left <<  setw(12) << "AvpCode" << ":" << setw(15) << "715" << "|" << setw(12)  << "AvpType" << ":" << setw(15) << "GROUPED" << "|" << "\n"; 
				cout << setw(tempTab) << "" << "|"  << left <<  setw(12) << "AvpName" << ":" << setw(44) << "RepositoryDataID" << "\n"; 
				TEXT lAvpFlg[100];
				//Hardcoded value.;
				sprintf( lAvpFlg, "%s %s %s", ((DiameterTLVUtil::isVenderSpecific(128)==true)?"Vendor":""), ((DiameterTLVUtil::isMandatoryBitSet(128)==true)?"Mandatory":""), ((DiameterTLVUtil::isProtectedBitSet(128)==true)?"Protected":""));
				this->getRepositoryDataID()->Display(tab+1);
			}
				break;
			case ProfileUpdateAnswer_DATAREFERENCE:
			{
				...
				this->getDataReference()->Display(tab+1);
			}
				break;
			case ProfileUpdateAnswer_SUPPORTEDFEATURES:
			{
...
BOOLEAN ProfileUpdateAnswer::GetMissingAvpInfo(TEXT *lMissingAvpInfo)
{
	...
	for(unsigned int i = 0 ; i < lMissingAvpTag.size(); ++i)
	{
		...
		switch(i)
		{
			...
			case ProfileUpdateAnswer_REPOSITORYDATAID:
			{
				...
				{
					lFirstRec = false;
					sprintf(lMissingAvpInfo, "Missing Avps List: 715");
				}
				else
					sprintf(lMissingAvpInfo, "%s,715", lMissingAvpInfo);
			}
				break;
			case ProfileUpdateAnswer_DATAREFERENCE:
			{
