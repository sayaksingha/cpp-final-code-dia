...
using namespace CX;
BOOLEAN SubscriptionInfo::isPresent(int tag){
...
OctetString* SubscriptionInfo::setCallIDSIPHeader(){
	if(mCallIDSIPHeader == NULL)
	{
		mCallIDSIPHeader = new OctetString();
		mTagsPresent[SubscriptionInfo_CALLIDSIPHEADER] = 1;
		return mCallIDSIPHeader;
	}
	return mCallIDSIPHeader;
}

void SubscriptionInfo::resetCallIDSIPHeader(){
	mTagsPresent[SubscriptionInfo_CALLIDSIPHEADER] = 0;
	if(mCallIDSIPHeader != NULL)
	{
		delete mCallIDSIPHeader; 
		mCallIDSIPHeader = NULL;
...
OctetString* SubscriptionInfo::setFromSIPHeader(){
	if(mFromSIPHeader == NULL)
	{
		mFromSIPHeader = new OctetString();
		mTagsPresent[SubscriptionInfo_FROMSIPHEADER] = 1;
		return mFromSIPHeader;
	}
	return mFromSIPHeader;
}

void SubscriptionInfo::resetFromSIPHeader(){
	mTagsPresent[SubscriptionInfo_FROMSIPHEADER] = 0;
	if(mFromSIPHeader != NULL)
	{
		delete mFromSIPHeader; 
		mFromSIPHeader = NULL;
...
OctetString* SubscriptionInfo::setToSIPHeader(){
	if(mToSIPHeader == NULL)
	{
		mToSIPHeader = new OctetString();
		mTagsPresent[SubscriptionInfo_TOSIPHEADER] = 1;
		return mToSIPHeader;
	}
	return mToSIPHeader;
}

void SubscriptionInfo::resetToSIPHeader(){
	mTagsPresent[SubscriptionInfo_TOSIPHEADER] = 0;
	if(mToSIPHeader != NULL)
	{
		delete mToSIPHeader; 
		mToSIPHeader = NULL;
...
OctetString* SubscriptionInfo::setRecordRoute(){
	if(mRecordRoute == NULL)
	{
		mRecordRoute = new OctetString();
		mTagsPresent[SubscriptionInfo_RECORDROUTE] = 1;
		return mRecordRoute;
	}
	return mRecordRoute;
}

void SubscriptionInfo::resetRecordRoute(){
	mTagsPresent[SubscriptionInfo_RECORDROUTE] = 0;
	if(mRecordRoute != NULL)
	{
		delete mRecordRoute; 
		mRecordRoute = NULL;
...
OctetString* SubscriptionInfo::setContact(){
	if(mContact == NULL)
	{
		mContact = new OctetString();
		mTagsPresent[SubscriptionInfo_CONTACT] = 1;
		return mContact;
	}
	return mContact;
}

void SubscriptionInfo::resetContact(){
	mTagsPresent[SubscriptionInfo_CONTACT] = 0;
	if(mContact != NULL)
	{
		delete mContact; 
		mContact = NULL;
...
OctetString* SubscriptionInfo::getCallIDSIPHeader(){
...
OctetString* SubscriptionInfo::getFromSIPHeader(){
...
OctetString* SubscriptionInfo::getToSIPHeader(){
...
OctetString* SubscriptionInfo::getRecordRoute(){
	if(NULL == mRecordRoute) {
		TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "RecordRoute is Null");
		throw lTssDiaMsgException;
	}
	if(!mRecordRoute->mAvpDecodedFlag)
	{
			if(mspByteArray.get() == NULL)
			{
			mRecordRoute->mAvpDecodedFlag = true;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "RecordRoute is Null");
			throw lTssDiaMsgException;
			}
		mspByteArray.get()->offset = 0;
		mspByteArray.get()->offset = mRecordRoute->getAvpOffsetLen().first;
		UINT16 lBytesDecoded = 0;
		if(mRecordRoute->Decode(mspByteArray.get(), mRecordRoute->getAvpOffsetLen().second, &lBytesDecoded)< 0)
		{
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_DECODE,"Error decoding RecordRoute")     ;
			throw lTssDiaMsgException;
		}
		mRecordRoute->mAvpDecodedFlag = true;
	}
...
OctetString* SubscriptionInfo::getContact(){
	if(NULL == mContact) {
...
int SubscriptionInfo::Encode(byteArray *array)
{
	...
	for(unsigned int i = 0 ; i < mTagsPresent.size() ; ++i )
	{
		...
		switch(i)
		{
			case SubscriptionInfo_CALLIDSIPHEADER:
			{
					array->size+=4;
				...
				break;
			case SubscriptionInfo_FROMSIPHEADER:
			{
					array->size+=4;
				...
				break;
			case SubscriptionInfo_TOSIPHEADER:
			{
					array->size+=4;
				...
				break;
			case SubscriptionInfo_RECORDROUTE:
			{
					array->size+=4;
					lTempHtonlVar =  htonl(646);
					memcpy(array->byte+array->offset, &lTempHtonlVar, DIA_AVP_CODE_SIZE);
					array->offset += DIA_AVP_CODE_SIZE;
					array->size++;
					array->byte[array->offset++] = 128;
					...
					if(!mRecordRoute->mAvpDecodedFlag)
					{
						memcpy(array->byte+array->offset, &mspByteArray.get()->byte[mRecordRoute->getAvpOffsetLen().first], mRecordRoute->getAvpOffsetLen().second);
						array->offset += mRecordRoute->getAvpOffsetLen().second;
						array->size += mRecordRoute->getAvpOffsetLen().second;
						UINT8 lPaddingLen = 0;
						if ( mRecordRoute->getAvpOffsetLen().second%4 != 0)
						{
							lPaddingLen = (4 -(mRecordRoute->getAvpOffsetLen().second%4));
						}
						if(false == DiameterTLVUtil::avp_length_encode(lengthOffset, array)){
							TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_ENCODE,"Error encoding RecordRoute");
							throw lTssDiaMsgException;
						}
						array->offset += lPaddingLen;
						array->size += lPaddingLen;
					...
					if((mRecordRoute->Encode(array) < 0) || 
							(DiameterTLVUtil::avp_length_encode(lengthOffset, array, mRecordRoute->getLength()) == false)){ 
						TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_ENCODE,"Error encoding RecordRoute");
						throw lTssDiaMsgException;
					}
				}
				break;
			case SubscriptionInfo_CONTACT:
			{
					array->size+=4;
...
int SubscriptionInfo::Decode(byteArray *array, INT16 lMsgLen, UINT16 *parsedBytes)
{
	mDecodeFlag = true;
	while( lMsgLen > 0)
	{
		...
		switch(lAvpCode)
		{
			...
			case 646:
				{
					if( NULL == this->setRecordRoute())
					{
						mDecodeFlag = false;
						TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_DECODE,"Error decoding RecordRoute");
						throw lTssDiaMsgException;
					}
					mRecordRoute->setAvpCode(646);
					mRecordRoute->setAvpVendorId(lVenderId);
					mRecordRoute->SetDataType((DiameterDataType)5);
					mRecordRoute->mspByteArray = mspByteArray;
					mRecordRoute->mAvpDecodedFlag = false;
					mRecordRoute->setAvpOffsetLen(array->offset,lAvpDataLen);
					array->offset+=lAvpDataLen;
...
DiameterBaseInterface* SubscriptionInfo::Find(byteArray *array, UINT32 avpCode)
{
	if(!this->mAvpDecodedFlag)
	{
		...
		switch(avpCode)
		{
			...
			{
				if(mRecordRoute != NULL)
					return mRecordRoute->Find(array, avpCode);
				else
					return NULL;
			}
			case 641:
...
BOOLEAN SubscriptionInfo::FindAll(byteArray *array, UINT32 avpCode, list<DiameterBaseInterface*>& pBaseInterface,BOOLEAN deepInspect)
{
	if(!this->mAvpDecodedFlag)
	{
		this->mspByteArray.get()->offset = 0;
		...
		switch(avpCode)
		{
			...
			case 646:
			{
				if(mRecordRoute != NULL)
				{
					DiameterBaseInterface* inf =  mRecordRoute->Find(this->mspByteArray.get(), avpCode);
					if(inf != NULL)
					{
						pBaseInterface.push_back(inf);
					}
...
std::shared_ptr<byteArray> SubscriptionInfo::GetByteArray()
{
	return mspByteArray;
}


BOOLEAN SubscriptionInfo::FindAll(byteArray *array, std::map<int,UINT32> &avpCodeMap, int &size, int key, list<DiameterBaseInterface*>& pBaseInterface,BOOLEAN deepInspect)
{
	if(!this->mAvpDecodedFlag)
	{
		this->mspByteArray.get()->offset = 0;
...
INT32 SubscriptionInfo::setAvpCode(UINT32 avpCode)
{
	mAvpCode = avpCode;
	return 0;
}
UINT32 SubscriptionInfo::getAvpCode()
{
	return mAvpCode;
}
INT32 SubscriptionInfo::setAvpVendorId(UINT32 avpVendorId)
{
	mAvpVendorId = avpVendorId;
	return 0;
}
UINT32 SubscriptionInfo::getAvpVendorId()
{
	return mAvpVendorId;
}
INT32 SubscriptionInfo::setAvpOffsetLen(INT16 offset, INT16 length)
{
	mAvpOffsetLen = make_pair(offset,length);
	return 0;
}
pair<INT16,INT16> SubscriptionInfo::getAvpOffsetLen()
{
...
void SubscriptionInfo::Display(UINT32 tab)
{
	int tempTab = tab * 3; 
	...
	for(int i = 0; i < mTagsPresent.size() ; ++i)
	{
		...
		switch(i)
		{
			...
			case SubscriptionInfo_RECORDROUTE:
			{
				cout << setw(tempTab) << "" << "|"  << left <<  setw(12) << "AvpCode" << ":" << setw(15) << "646" << "|" << setw(12)  << "AvpType" << ":" << setw(15) << "SINGLE" << "|" << "\n"; 
				cout << setw(tempTab) << "" << "|"  << left <<  setw(12) << "AvpName" << ":" << setw(44) << "RecordRoute" << "\n"; 
				TEXT lAvpFlg[100];
				//Hardcoded value.;
				sprintf( lAvpFlg, "%s %s %s", ((DiameterTLVUtil::isVenderSpecific(128)==true)?"Vendor":""), ((DiameterTLVUtil::isMandatoryBitSet(128)==true)?"Mandatory":""), ((DiameterTLVUtil::isProtectedBitSet(128)==true)?"Protected":""));
				this->getRecordRoute()->Display(tab+1);
			}
				break;
			case SubscriptionInfo_CONTACT:
			{
...
BOOLEAN SubscriptionInfo::GetMissingAvpInfo(TEXT *lMissingAvpInfo)
{
	...
	for(unsigned int i = 0 ; i < lMissingAvpTag.size(); ++i)
	{
		...
		switch(i)
		{
			...
			case SubscriptionInfo_RECORDROUTE:
			{
				...
				{
					lFirstRec = false;
					sprintf(lMissingAvpInfo, "Missing Avps List: 646");
				}
				else
					sprintf(lMissingAvpInfo, "%s,646", lMissingAvpInfo);
			}
				break;
			case SubscriptionInfo_CONTACT:
			{
