...
using namespace CX;
BOOLEAN FailedAVP::isPresent(int tag){
...
OctetString* FailedAVP::setErrorMessage(){
	if(mErrorMessage == NULL)
	{
		mErrorMessage = new OctetString();
		mTagsPresent[FailedAVP_ERRORMESSAGE] = 1;
		return mErrorMessage;
	}
	return mErrorMessage;
}

void FailedAVP::resetErrorMessage(){
	mTagsPresent[FailedAVP_ERRORMESSAGE] = 0;
	if(mErrorMessage != NULL)
	{
		delete mErrorMessage; 
		mErrorMessage = NULL;
...
OctetString* FailedAVP::getErrorMessage(){
...
int FailedAVP::Encode(byteArray *array)
{
...
int FailedAVP::Decode(byteArray *array, INT16 lMsgLen, UINT16 *parsedBytes)
{
	mDecodeFlag = true;
	while( lMsgLen > 0)
	{
...
DiameterBaseInterface* FailedAVP::Find(byteArray *array, UINT32 avpCode)
{
...
BOOLEAN FailedAVP::FindAll(byteArray *array, UINT32 avpCode, list<DiameterBaseInterface*>& pBaseInterface,BOOLEAN deepInspect)
{
	if(!this->mAvpDecodedFlag)
	{
		this->mspByteArray.get()->offset = 0;
...
std::shared_ptr<byteArray> FailedAVP::GetByteArray()
{
	return mspByteArray;
}


BOOLEAN FailedAVP::FindAll(byteArray *array, std::map<int,UINT32> &avpCodeMap, int &size, int key, list<DiameterBaseInterface*>& pBaseInterface,BOOLEAN deepInspect)
{
	if(!this->mAvpDecodedFlag)
	{
		this->mspByteArray.get()->offset = 0;
...
INT32 FailedAVP::setAvpCode(UINT32 avpCode)
{
	mAvpCode = avpCode;
	return 0;
}
UINT32 FailedAVP::getAvpCode()
{
	return mAvpCode;
}
INT32 FailedAVP::setAvpVendorId(UINT32 avpVendorId)
{
	mAvpVendorId = avpVendorId;
	return 0;
}
UINT32 FailedAVP::getAvpVendorId()
{
	return mAvpVendorId;
}
INT32 FailedAVP::setAvpOffsetLen(INT16 offset, INT16 length)
{
	mAvpOffsetLen = make_pair(offset,length);
	return 0;
}
pair<INT16,INT16> FailedAVP::getAvpOffsetLen()
{
...
void FailedAVP::Display(UINT32 tab)
{
	int tempTab = tab * 3; 
...
BOOLEAN FailedAVP::GetMissingAvpInfo(TEXT *lMissingAvpInfo)
{
