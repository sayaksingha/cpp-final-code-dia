...
using namespace CX;
BOOLEAN ServerAssignmentAnswer::isPresent(int tag){
...
ServerAssignmentAnswer::ServerAssignmentAnswer()
{
	...
	mLooseRouteIndication = NULL;
	mSCSCFRestorationInfoGrpList = NULL;
	mAssociatedRegisteredIdentitiesGrp = NULL;
	mServerName = NULL;
	mWildcardedPublicIdentity = NULL;
	mPriviledgedSenderIndication = NULL;
	mAllowedWAFWWSFIdentitiesGrp = NULL;
	mErrorMessage = NULL;
	mFailedAVPGrp = NULL;
	mProxyInfoGrpList = NULL;
	mRouteRecordList = NULL;
	mTagsPresent.reset();
	mMandatoryAvpCheckVal= bitset<27>(string("111000010000110011111111101"));
...
ServerAssignmentAnswer::~ServerAssignmentAnswer()
{
	...
	if(mLooseRouteIndication != NULL)
		delete mLooseRouteIndication;
	if(mSCSCFRestorationInfoGrpList != NULL){
		mSCSCFRestorationInfoGrpIterator = mSCSCFRestorationInfoGrpList->begin();
		while(mSCSCFRestorationInfoGrpIterator != mSCSCFRestorationInfoGrpList->end()){
	if(*mSCSCFRestorationInfoGrpIterator != NULL)
			delete (*mSCSCFRestorationInfoGrpIterator);
			mSCSCFRestorationInfoGrpIterator++;
		}
		delete mSCSCFRestorationInfoGrpList;
	}
	if(mAssociatedRegisteredIdentitiesGrp != NULL)
		delete mAssociatedRegisteredIdentitiesGrp;
	if(mServerName != NULL)
		delete mServerName;
	if(mWildcardedPublicIdentity != NULL)
		delete mWildcardedPublicIdentity;
	if(mPriviledgedSenderIndication != NULL)
		delete mPriviledgedSenderIndication;
	if(mAllowedWAFWWSFIdentitiesGrp != NULL)
		delete mAllowedWAFWWSFIdentitiesGrp;
...
OctetString* ServerAssignmentAnswer::setSessionId(){
	mMandatoryAvpSetVal [ServerAssignmentAnswer_SESSIONID] = 1;
	if(mSessionId == NULL)
	{
		mSessionId = new OctetString();
		mTagsPresent[ServerAssignmentAnswer_SESSIONID] = 1;
		return mSessionId;
	}
	return mSessionId;
}

void ServerAssignmentAnswer::resetSessionId(){
	mTagsPresent[ServerAssignmentAnswer_SESSIONID] = 0;
	if(mSessionId != NULL)
	{
		delete mSessionId; 
		mSessionId = NULL;
...
DRMP* ServerAssignmentAnswer::setDRMP(){
	if(mDRMP == NULL)
	{
		mDRMP = new DRMP();
		mTagsPresent[ServerAssignmentAnswer_DRMP] = 1;
		return mDRMP;
	}
	return mDRMP;
}

void ServerAssignmentAnswer::resetDRMP(){
	mTagsPresent[ServerAssignmentAnswer_DRMP] = 0;
	if(mDRMP != NULL)
	{
		delete mDRMP; 
		mDRMP = NULL;
...
Unsigned32* ServerAssignmentAnswer::setResultCode(){
	mMandatoryAvpSetVal [ServerAssignmentAnswer_RESULTCODE] = 1;
	if(mResultCode == NULL)
	{
		mResultCode = new Unsigned32();
		mTagsPresent[ServerAssignmentAnswer_RESULTCODE] = 1;
		return mResultCode;
	}
	return mResultCode;
}

void ServerAssignmentAnswer::resetResultCode(){
	mTagsPresent[ServerAssignmentAnswer_RESULTCODE] = 0;
	if(mResultCode != NULL)
	{
		delete mResultCode; 
		mResultCode = NULL;
...
AuthSessionState* ServerAssignmentAnswer::setAuthSessionState(){
	mMandatoryAvpSetVal [ServerAssignmentAnswer_AUTHSESSIONSTATE] = 1;
	if(mAuthSessionState == NULL)
	{
		mAuthSessionState = new AuthSessionState();
		mTagsPresent[ServerAssignmentAnswer_AUTHSESSIONSTATE] = 1;
		return mAuthSessionState;
	}
	return mAuthSessionState;
}

void ServerAssignmentAnswer::resetAuthSessionState(){
	mTagsPresent[ServerAssignmentAnswer_AUTHSESSIONSTATE] = 0;
	if(mAuthSessionState != NULL)
	{
		delete mAuthSessionState; 
		mAuthSessionState = NULL;
...
OctetString* ServerAssignmentAnswer::setOriginHost(){
	mMandatoryAvpSetVal [ServerAssignmentAnswer_ORIGINHOST] = 1;
	if(mOriginHost == NULL)
	{
		mOriginHost = new OctetString();
		mTagsPresent[ServerAssignmentAnswer_ORIGINHOST] = 1;
		return mOriginHost;
	}
	return mOriginHost;
}

void ServerAssignmentAnswer::resetOriginHost(){
	mTagsPresent[ServerAssignmentAnswer_ORIGINHOST] = 0;
	if(mOriginHost != NULL)
	{
		delete mOriginHost; 
		mOriginHost = NULL;
...
OctetString* ServerAssignmentAnswer::setOriginRealm(){
	mMandatoryAvpSetVal [ServerAssignmentAnswer_ORIGINREALM] = 1;
	if(mOriginRealm == NULL)
	{
		mOriginRealm = new OctetString();
		mTagsPresent[ServerAssignmentAnswer_ORIGINREALM] = 1;
		return mOriginRealm;
	}
	return mOriginRealm;
}

void ServerAssignmentAnswer::resetOriginRealm(){
	mTagsPresent[ServerAssignmentAnswer_ORIGINREALM] = 0;
	if(mOriginRealm != NULL)
	{
		delete mOriginRealm; 
		mOriginRealm = NULL;
...
OctetString* ServerAssignmentAnswer::setUserName(){
	mMandatoryAvpSetVal [ServerAssignmentAnswer_USERNAME] = 1;
	if(mUserName == NULL)
	{
		mUserName = new OctetString();
		mTagsPresent[ServerAssignmentAnswer_USERNAME] = 1;
		return mUserName;
	}
	return mUserName;
}

void ServerAssignmentAnswer::resetUserName(){
	mTagsPresent[ServerAssignmentAnswer_USERNAME] = 0;
	if(mUserName != NULL)
	{
		delete mUserName; 
		mUserName = NULL;
...
OctetString* ServerAssignmentAnswer::setUserData(){
	mMandatoryAvpSetVal [ServerAssignmentAnswer_USERDATA] = 1;
	if(mUserData == NULL)
	{
		mUserData = new OctetString();
		mTagsPresent[ServerAssignmentAnswer_USERDATA] = 1;
		return mUserData;
	}
	return mUserData;
}

void ServerAssignmentAnswer::resetUserData(){
	mTagsPresent[ServerAssignmentAnswer_USERDATA] = 0;
	if(mUserData != NULL)
	{
		delete mUserData; 
		mUserData = NULL;
...
LooseRouteIndication* ServerAssignmentAnswer::setLooseRouteIndication(){
	if(mLooseRouteIndication == NULL)
	{
		mLooseRouteIndication = new LooseRouteIndication();
		mTagsPresent[ServerAssignmentAnswer_LOOSEROUTEINDICATION] = 1;
		return mLooseRouteIndication;
	}
	return mLooseRouteIndication;
}

void ServerAssignmentAnswer::resetLooseRouteIndication(){
	mTagsPresent[ServerAssignmentAnswer_LOOSEROUTEINDICATION] = 0;
	if(mLooseRouteIndication != NULL)
	{
		delete mLooseRouteIndication; 
		mLooseRouteIndication = NULL;
...
OctetString* ServerAssignmentAnswer::setServerName(){
	mMandatoryAvpSetVal [ServerAssignmentAnswer_SERVERNAME] = 1;
	if(mServerName == NULL)
	{
		mServerName = new OctetString();
		mTagsPresent[ServerAssignmentAnswer_SERVERNAME] = 1;
		return mServerName;
	}
	return mServerName;
}

void ServerAssignmentAnswer::resetServerName(){
	mTagsPresent[ServerAssignmentAnswer_SERVERNAME] = 0;
	if(mServerName != NULL)
	{
		delete mServerName; 
		mServerName = NULL;
...
OctetString* ServerAssignmentAnswer::setWildcardedPublicIdentity(){
	if(mWildcardedPublicIdentity == NULL)
	{
		mWildcardedPublicIdentity = new OctetString();
		mTagsPresent[ServerAssignmentAnswer_WILDCARDEDPUBLICIDENTITY] = 1;
		return mWildcardedPublicIdentity;
	}
	return mWildcardedPublicIdentity;
}

void ServerAssignmentAnswer::resetWildcardedPublicIdentity(){
	mTagsPresent[ServerAssignmentAnswer_WILDCARDEDPUBLICIDENTITY] = 0;
	if(mWildcardedPublicIdentity != NULL)
	{
		delete mWildcardedPublicIdentity; 
		mWildcardedPublicIdentity = NULL;
...
PriviledgedSenderIndication* ServerAssignmentAnswer::setPriviledgedSenderIndication(){
	if(mPriviledgedSenderIndication == NULL)
	{
		mPriviledgedSenderIndication = new PriviledgedSenderIndication();
		mTagsPresent[ServerAssignmentAnswer_PRIVILEDGEDSENDERINDICATION] = 1;
		return mPriviledgedSenderIndication;
	}
	return mPriviledgedSenderIndication;
}

void ServerAssignmentAnswer::resetPriviledgedSenderIndication(){
	mTagsPresent[ServerAssignmentAnswer_PRIVILEDGEDSENDERINDICATION] = 0;
	if(mPriviledgedSenderIndication != NULL)
	{
		delete mPriviledgedSenderIndication; 
		mPriviledgedSenderIndication = NULL;
...
OctetString* ServerAssignmentAnswer::setErrorMessage(){
	if(mErrorMessage == NULL)
	{
		mErrorMessage = new OctetString();
		mTagsPresent[ServerAssignmentAnswer_ERRORMESSAGE] = 1;
		return mErrorMessage;
	}
	return mErrorMessage;
}

void ServerAssignmentAnswer::resetErrorMessage(){
	mTagsPresent[ServerAssignmentAnswer_ERRORMESSAGE] = 0;
	if(mErrorMessage != NULL)
	{
		delete mErrorMessage; 
		mErrorMessage = NULL;
...
OctetString* ServerAssignmentAnswer::getSessionId(){
	if(NULL == mSessionId) {
...
DRMP* ServerAssignmentAnswer::getDRMP(){
	if(NULL == mDRMP) {
...
CX::VendorSpecificApplicationId* ServerAssignmentAnswer::getVendorSpecificApplicationId()
{
	if(mVendorSpecificApplicationIdGrp == NULL)
	{
...
CX::VendorSpecificApplicationId* ServerAssignmentAnswer::setVendorSpecificApplicationId(){
	if(mVendorSpecificApplicationIdGrp == NULL)
		mVendorSpecificApplicationIdGrp = new VendorSpecificApplicationId();
	mTagsPresent[ServerAssignmentAnswer_VENDORSPECIFICAPPLICATIONID] = 1;
	mMandatoryAvpSetVal[ServerAssignmentAnswer_VENDORSPECIFICAPPLICATIONID] = 1;
	return mVendorSpecificApplicationIdGrp;
}

void ServerAssignmentAnswer::resetVendorSpecificApplicationId(){
	mTagsPresent[ServerAssignmentAnswer_VENDORSPECIFICAPPLICATIONID] = 0;
	if(mVendorSpecificApplicationIdGrp != NULL)
		delete mVendorSpecificApplicationIdGrp;
}

Unsigned32* ServerAssignmentAnswer::getResultCode(){
	if(NULL == mResultCode) {
...
ExperimentalResult* ServerAssignmentAnswer::getExperimentalResult()
{
	if(mExperimentalResultGrp == NULL)
	{
...
ExperimentalResult* ServerAssignmentAnswer::setExperimentalResult(){
	if(mExperimentalResultGrp == NULL)
		mExperimentalResultGrp = new ExperimentalResult();
	mTagsPresent[ServerAssignmentAnswer_EXPERIMENTALRESULT] = 1;
	mMandatoryAvpSetVal[ServerAssignmentAnswer_EXPERIMENTALRESULT] = 1;
	return mExperimentalResultGrp;
}

void ServerAssignmentAnswer::resetExperimentalResult(){
	mTagsPresent[ServerAssignmentAnswer_EXPERIMENTALRESULT] = 0;
	if(mExperimentalResultGrp != NULL)
		delete mExperimentalResultGrp;
}

AuthSessionState* ServerAssignmentAnswer::getAuthSessionState(){
...
OctetString* ServerAssignmentAnswer::getOriginHost(){
	if(NULL == mOriginHost) {
...
OctetString* ServerAssignmentAnswer::getOriginRealm(){
...
OctetString* ServerAssignmentAnswer::getUserName(){
	if(NULL == mUserName) {
...
OCSupportedFeatures* ServerAssignmentAnswer::getOCSupportedFeatures()
{
	if(mOCSupportedFeaturesGrp == NULL)
	{
...
OCSupportedFeatures* ServerAssignmentAnswer::setOCSupportedFeatures(){
	if(mOCSupportedFeaturesGrp == NULL)
		mOCSupportedFeaturesGrp = new OCSupportedFeatures();
	mTagsPresent[ServerAssignmentAnswer_OCSUPPORTEDFEATURES] = 1;
	mMandatoryAvpSetVal[ServerAssignmentAnswer_OCSUPPORTEDFEATURES] = 1;
	return mOCSupportedFeaturesGrp;
}

void ServerAssignmentAnswer::resetOCSupportedFeatures(){
	mTagsPresent[ServerAssignmentAnswer_OCSUPPORTEDFEATURES] = 0;
	if(mOCSupportedFeaturesGrp != NULL)
		delete mOCSupportedFeaturesGrp;
}

OCOLR* ServerAssignmentAnswer::getOCOLR()
{
	if(mOCOLRGrp == NULL)
	{
...
OCOLR* ServerAssignmentAnswer::setOCOLR(){
	if(mOCOLRGrp == NULL)
		mOCOLRGrp = new OCOLR();
	mTagsPresent[ServerAssignmentAnswer_OCOLR] = 1;
	mMandatoryAvpSetVal[ServerAssignmentAnswer_OCOLR] = 1;
	return mOCOLRGrp;
}

void ServerAssignmentAnswer::resetOCOLR(){
	mTagsPresent[ServerAssignmentAnswer_OCOLR] = 0;
	if(mOCOLRGrp != NULL)
		delete mOCOLRGrp;
}

list<Load*>* ServerAssignmentAnswer::getLoadGrpList()
{
	if(mLoadGrpList == NULL)
	{
	...
	for(auto itr : *mLoadGrpList)
	{
		...
		{
			mTagsPresent[ServerAssignmentAnswer_LOAD] = 0;
			mLoadGrpList = NULL;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "Load is Null");
			throw lTssDiaMsgException;
		}
...
list<Load*>* ServerAssignmentAnswer::setLoadGrpList(){
	mTagsPresent[ServerAssignmentAnswer_LOAD] = 1;
	if(mLoadGrpList == NULL)
...
void ServerAssignmentAnswer::resetLoadGrpList(){
	mTagsPresent[ServerAssignmentAnswer_LOAD] = 0;
	if(mLoadGrpList != NULL)
		delete mLoadGrpList;
}

list<SupportedFeatures*>* ServerAssignmentAnswer::getSupportedFeaturesGrpList()
{
	if(mSupportedFeaturesGrpList == NULL)
	{
	...
	for(auto itr : *mSupportedFeaturesGrpList)
	{
		...
		{
			mTagsPresent[ServerAssignmentAnswer_SUPPORTEDFEATURES] = 0;
			mSupportedFeaturesGrpList = NULL;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "SupportedFeatures is Null");
			throw lTssDiaMsgException;
		}
...
list<SupportedFeatures*>* ServerAssignmentAnswer::setSupportedFeaturesGrpList(){
	mTagsPresent[ServerAssignmentAnswer_SUPPORTEDFEATURES] = 1;
	if(mSupportedFeaturesGrpList == NULL)
...
void ServerAssignmentAnswer::resetSupportedFeaturesGrpList(){
	mTagsPresent[ServerAssignmentAnswer_SUPPORTEDFEATURES] = 0;
	if(mSupportedFeaturesGrpList != NULL)
		delete mSupportedFeaturesGrpList;
}

OctetString* ServerAssignmentAnswer::getUserData(){
	if(NULL == mUserData) {
...
ChargingInformation* ServerAssignmentAnswer::getChargingInformation()
{
	if(mChargingInformationGrp == NULL)
	{
...
ChargingInformation* ServerAssignmentAnswer::setChargingInformation(){
	if(mChargingInformationGrp == NULL)
		mChargingInformationGrp = new ChargingInformation();
	mTagsPresent[ServerAssignmentAnswer_CHARGINGINFORMATION] = 1;
	mMandatoryAvpSetVal[ServerAssignmentAnswer_CHARGINGINFORMATION] = 1;
	return mChargingInformationGrp;
}

void ServerAssignmentAnswer::resetChargingInformation(){
	mTagsPresent[ServerAssignmentAnswer_CHARGINGINFORMATION] = 0;
	if(mChargingInformationGrp != NULL)
		delete mChargingInformationGrp;
}

AssociatedIdentities* ServerAssignmentAnswer::getAssociatedIdentities()
{
	if(mAssociatedIdentitiesGrp == NULL)
	{
...
AssociatedIdentities* ServerAssignmentAnswer::setAssociatedIdentities(){
	if(mAssociatedIdentitiesGrp == NULL)
		mAssociatedIdentitiesGrp = new AssociatedIdentities();
	mTagsPresent[ServerAssignmentAnswer_ASSOCIATEDIDENTITIES] = 1;
	return mAssociatedIdentitiesGrp;
}

void ServerAssignmentAnswer::resetAssociatedIdentities(){
	mTagsPresent[ServerAssignmentAnswer_ASSOCIATEDIDENTITIES] = 0;
	if(mAssociatedIdentitiesGrp != NULL)
		delete mAssociatedIdentitiesGrp;
}

LooseRouteIndication* ServerAssignmentAnswer::getLooseRouteIndication(){
	if(NULL == mLooseRouteIndication) {
		TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "LooseRouteIndication is Null");
		throw lTssDiaMsgException;
	}
	if(!mLooseRouteIndication->mAvpDecodedFlag)
	{
			if(mspByteArray.get() == NULL)
			{
			mLooseRouteIndication->mAvpDecodedFlag = true;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "LooseRouteIndication is Null");
			throw lTssDiaMsgException;
			}
		mspByteArray.get()->offset = 0;
		mspByteArray.get()->offset = mLooseRouteIndication->getAvpOffsetLen().first;
		UINT16 lBytesDecoded = 0;
		if(mLooseRouteIndication->Decode(mspByteArray.get(), mLooseRouteIndication->getAvpOffsetLen().second, &lBytesDecoded)< 0)
		{
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_DECODE,"Error decoding LooseRouteIndication")     ;
			throw lTssDiaMsgException;
		}
		mLooseRouteIndication->mAvpDecodedFlag = true;
	}
...
list<SCSCFRestorationInfo*>* ServerAssignmentAnswer::getSCSCFRestorationInfoGrpList()
{
	if(mSCSCFRestorationInfoGrpList == NULL)
	{
	...
	for(auto itr : *mSCSCFRestorationInfoGrpList)
	{
		if(mSCSCFRestorationInfoGrpList != NULL)
		{
		if(mSCSCFRestorationInfoGrpList->size() == 0)
		{
			mTagsPresent[ServerAssignmentAnswer_SCSCFRESTORATIONINFO] = 0;
			mSCSCFRestorationInfoGrpList = NULL;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "SCSCFRestorationInfo is Null");
			throw lTssDiaMsgException;
		}
	...
	return mSCSCFRestorationInfoGrpList;
}

list<SCSCFRestorationInfo*>* ServerAssignmentAnswer::setSCSCFRestorationInfoGrpList(){
	mTagsPresent[ServerAssignmentAnswer_SCSCFRESTORATIONINFO] = 1;
	if(mSCSCFRestorationInfoGrpList == NULL)
		mSCSCFRestorationInfoGrpList = new list<SCSCFRestorationInfo*>;
	mSCSCFRestorationInfoGrpList->clear();
	return mSCSCFRestorationInfoGrpList;
}

void ServerAssignmentAnswer::resetSCSCFRestorationInfoGrpList(){
	mTagsPresent[ServerAssignmentAnswer_SCSCFRESTORATIONINFO] = 0;
	if(mSCSCFRestorationInfoGrpList != NULL)
		delete mSCSCFRestorationInfoGrpList;
}

AssociatedRegisteredIdentities* ServerAssignmentAnswer::getAssociatedRegisteredIdentities()
{
	if(mAssociatedRegisteredIdentitiesGrp == NULL)
	{
		TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "AssociatedRegisteredIdentities is Null");
		throw lTssDiaMsgException;
	}
	if(!mAssociatedRegisteredIdentitiesGrp->mAvpDecodedFlag)
	{
			if(mspByteArray.get() == NULL)
			{
			mAssociatedRegisteredIdentitiesGrp->mAvpDecodedFlag = true;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "AssociatedRegisteredIdentities is Null");
			throw lTssDiaMsgException;
			}
		mspByteArray.get()->offset = 0;
		mspByteArray.get()->offset = mAssociatedRegisteredIdentitiesGrp->getAvpOffsetLen().first;
		UINT16 lBytesDecoded = 0;
		if(mAssociatedRegisteredIdentitiesGrp->Decode(mspByteArray.get(), mAssociatedRegisteredIdentitiesGrp->getAvpOffsetLen().second, &lBytesDecoded)< 0)
		{
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_DECODE,"Error decoding AssociatedRegisteredIdentities")     ;
			throw lTssDiaMsgException;
		}
		mAssociatedRegisteredIdentitiesGrp->mAvpDecodedFlag = true;
	}
	return mAssociatedRegisteredIdentitiesGrp;
}

AssociatedRegisteredIdentities* ServerAssignmentAnswer::setAssociatedRegisteredIdentities(){
	if(mAssociatedRegisteredIdentitiesGrp == NULL)
		mAssociatedRegisteredIdentitiesGrp = new AssociatedRegisteredIdentities();
	mTagsPresent[ServerAssignmentAnswer_ASSOCIATEDREGISTEREDIDENTITIES] = 1;
	return mAssociatedRegisteredIdentitiesGrp;
}

void ServerAssignmentAnswer::resetAssociatedRegisteredIdentities(){
	mTagsPresent[ServerAssignmentAnswer_ASSOCIATEDREGISTEREDIDENTITIES] = 0;
	if(mAssociatedRegisteredIdentitiesGrp != NULL)
		delete mAssociatedRegisteredIdentitiesGrp;
}

OctetString* ServerAssignmentAnswer::getServerName(){
	if(NULL == mServerName) {
...
OctetString* ServerAssignmentAnswer::getWildcardedPublicIdentity(){
...
PriviledgedSenderIndication* ServerAssignmentAnswer::getPriviledgedSenderIndication(){
	if(NULL == mPriviledgedSenderIndication) {
		TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "PriviledgedSenderIndication is Null");
		throw lTssDiaMsgException;
	}
	if(!mPriviledgedSenderIndication->mAvpDecodedFlag)
	{
			if(mspByteArray.get() == NULL)
			{
			mPriviledgedSenderIndication->mAvpDecodedFlag = true;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "PriviledgedSenderIndication is Null");
			throw lTssDiaMsgException;
			}
		mspByteArray.get()->offset = 0;
		mspByteArray.get()->offset = mPriviledgedSenderIndication->getAvpOffsetLen().first;
		UINT16 lBytesDecoded = 0;
		if(mPriviledgedSenderIndication->Decode(mspByteArray.get(), mPriviledgedSenderIndication->getAvpOffsetLen().second, &lBytesDecoded)< 0)
		{
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_DECODE,"Error decoding PriviledgedSenderIndication")     ;
			throw lTssDiaMsgException;
		}
		mPriviledgedSenderIndication->mAvpDecodedFlag = true;
	}
...
AllowedWAFWWSFIdentities* ServerAssignmentAnswer::getAllowedWAFWWSFIdentities()
{
	if(mAllowedWAFWWSFIdentitiesGrp == NULL)
	{
		TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "AllowedWAFWWSFIdentities is Null");
		throw lTssDiaMsgException;
	}
	if(!mAllowedWAFWWSFIdentitiesGrp->mAvpDecodedFlag)
	{
			if(mspByteArray.get() == NULL)
			{
			mAllowedWAFWWSFIdentitiesGrp->mAvpDecodedFlag = true;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "AllowedWAFWWSFIdentities is Null");
			throw lTssDiaMsgException;
			}
		mspByteArray.get()->offset = 0;
		mspByteArray.get()->offset = mAllowedWAFWWSFIdentitiesGrp->getAvpOffsetLen().first;
		UINT16 lBytesDecoded = 0;
		if(mAllowedWAFWWSFIdentitiesGrp->Decode(mspByteArray.get(), mAllowedWAFWWSFIdentitiesGrp->getAvpOffsetLen().second, &lBytesDecoded)< 0)
		{
	...
	return mAllowedWAFWWSFIdentitiesGrp;
}

AllowedWAFWWSFIdentities* ServerAssignmentAnswer::setAllowedWAFWWSFIdentities(){
	if(mAllowedWAFWWSFIdentitiesGrp == NULL)
		mAllowedWAFWWSFIdentitiesGrp = new AllowedWAFWWSFIdentities();
	mTagsPresent[ServerAssignmentAnswer_ALLOWEDWAFWWSFIDENTITIES] = 1;
	return mAllowedWAFWWSFIdentitiesGrp;
}

void ServerAssignmentAnswer::resetAllowedWAFWWSFIdentities(){
	mTagsPresent[ServerAssignmentAnswer_ALLOWEDWAFWWSFIDENTITIES] = 0;
	if(mAllowedWAFWWSFIdentitiesGrp != NULL)
		delete mAllowedWAFWWSFIdentitiesGrp;
}

OctetString* ServerAssignmentAnswer::getErrorMessage(){
...
FailedAVP* ServerAssignmentAnswer::getFailedAVP()
{
	if(mFailedAVPGrp == NULL)
	{
...
FailedAVP* ServerAssignmentAnswer::setFailedAVP(){
	if(mFailedAVPGrp == NULL)
		mFailedAVPGrp = new FailedAVP();
	mTagsPresent[ServerAssignmentAnswer_FAILEDAVP] = 1;
	mMandatoryAvpSetVal[ServerAssignmentAnswer_FAILEDAVP] = 1;
	return mFailedAVPGrp;
}

void ServerAssignmentAnswer::resetFailedAVP(){
	mTagsPresent[ServerAssignmentAnswer_FAILEDAVP] = 0;
	if(mFailedAVPGrp != NULL)
		delete mFailedAVPGrp;
}

list<CX::ProxyInfo*>* ServerAssignmentAnswer::getProxyInfoGrpList()
{
	if(mProxyInfoGrpList == NULL)
	{
	...
	for(auto itr : *mProxyInfoGrpList)
	{
		...
		{
			mTagsPresent[ServerAssignmentAnswer_PROXYINFO] = 0;
			mProxyInfoGrpList = NULL;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "ProxyInfo is Null");
			throw lTssDiaMsgException;
		}
...
list<CX::ProxyInfo*>* ServerAssignmentAnswer::setProxyInfoGrpList(){
	mTagsPresent[ServerAssignmentAnswer_PROXYINFO] = 1;
	if(mProxyInfoGrpList == NULL)
		mProxyInfoGrpList = new list<CX::ProxyInfo*>;
	mMandatoryAvpSetVal[ServerAssignmentAnswer_PROXYINFO] = 1;
	mProxyInfoGrpList->clear();
	return mProxyInfoGrpList;
}

void ServerAssignmentAnswer::resetProxyInfoGrpList(){
	mTagsPresent[ServerAssignmentAnswer_PROXYINFO] = 0;
	if(mProxyInfoGrpList != NULL)
		delete mProxyInfoGrpList;
}

list<OctetString*>* ServerAssignmentAnswer::getRouteRecordList()
{
	if(mRouteRecordList == NULL)
	{
...
list<OctetString*>* ServerAssignmentAnswer::setRouteRecordList() {
	mTagsPresent[ServerAssignmentAnswer_ROUTERECORD] = 1;
	if(mRouteRecordList == NULL)
		mRouteRecordList = new list<OctetString*>;
	mMandatoryAvpSetVal [ServerAssignmentAnswer_ROUTERECORD] = 1;
	mRouteRecordList->clear();
	return mRouteRecordList;
}

void ServerAssignmentAnswer::resetRouteRecordList(){
	mTagsPresent[ServerAssignmentAnswer_ROUTERECORD] = 0;
	if(mRouteRecordList != NULL)
		delete mRouteRecordList;
}
...
int ServerAssignmentAnswer::Encode(byteArray *array)
{
	...
	for(unsigned int i = 0 ; i < mTagsPresent.size() ; ++i )
	{
		...
		switch(i)
		{
			case ServerAssignmentAnswer_SESSIONID:
			{
					array->size+=4;
				...
				break;
			case ServerAssignmentAnswer_DRMP:
			{
					array->size+=4;
				...
				break;
			case ServerAssignmentAnswer_VENDORSPECIFICAPPLICATIONID:
			{
				array->size+=4;
				...
				break;
			case ServerAssignmentAnswer_RESULTCODE:
			{
					array->size+=4;
				...
				break;
			case ServerAssignmentAnswer_EXPERIMENTALRESULT:
			{
				array->size+=4;
				...
				break;
			case ServerAssignmentAnswer_AUTHSESSIONSTATE:
			{
					array->size+=4;
				...
				break;
			case ServerAssignmentAnswer_ORIGINHOST:
			{
					array->size+=4;
				...
				break;
			case ServerAssignmentAnswer_ORIGINREALM:
			{
					array->size+=4;
				...
				break;
			case ServerAssignmentAnswer_USERNAME:
			{
					array->size+=4;
				...
				break;
			case ServerAssignmentAnswer_OCSUPPORTEDFEATURES:
			{
				array->size+=4;
				...
				break;
			case ServerAssignmentAnswer_OCOLR:
			{
				array->size+=4;
				...
				break;
			case ServerAssignmentAnswer_LOAD:
			{
				if( mLoadGrpList->size() <= 0) {
					break;
				}
				...
				break;
			case ServerAssignmentAnswer_SUPPORTEDFEATURES:
			{
				if( mSupportedFeaturesGrpList->size() <= 0) {
					break;
				}
				...
				break;
			case ServerAssignmentAnswer_USERDATA:
			{
					array->size+=4;
					lTempHtonlVar =  htonl(606);
					memcpy(array->byte+array->offset, &lTempHtonlVar, DIA_AVP_CODE_SIZE);
					array->offset += DIA_AVP_CODE_SIZE;
					array->size++;
				...
				break;
			case ServerAssignmentAnswer_CHARGINGINFORMATION:
			{
				array->size+=4;
				...
				break;
			case ServerAssignmentAnswer_ASSOCIATEDIDENTITIES:
			{
				array->size+=4;
				...
				break;
			case ServerAssignmentAnswer_LOOSEROUTEINDICATION:
			{
					array->size+=4;
					lTempHtonlVar =  htonl(638);
					memcpy(array->byte+array->offset, &lTempHtonlVar, DIA_AVP_CODE_SIZE);
					array->offset += DIA_AVP_CODE_SIZE;
					array->size++;
					array->byte[array->offset++] = 128;
					...
					if(!mLooseRouteIndication->mAvpDecodedFlag)
					{
						memcpy(array->byte+array->offset, &mspByteArray.get()->byte[mLooseRouteIndication->getAvpOffsetLen().first], mLooseRouteIndication->getAvpOffsetLen().second);
						array->offset += mLooseRouteIndication->getAvpOffsetLen().second;
						array->size += mLooseRouteIndication->getAvpOffsetLen().second;
						if(false == DiameterTLVUtil::avp_length_encode(lengthOffset, array)){
							TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_ENCODE,"Error encoding LooseRouteIndication");
							throw lTssDiaMsgException;
						}
						continue;
					}
					if((mLooseRouteIndication->Encode(array) < 0) || 
						(DiameterTLVUtil::avp_length_encode(lengthOffset, array) == false)){ 
						TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_ENCODE,"Error encoding LooseRouteIndication");
						throw lTssDiaMsgException;
					}
				...
				break;
			case ServerAssignmentAnswer_SCSCFRESTORATIONINFO:
			{
				if( mSCSCFRestorationInfoGrpList->size() <= 0) {
					break;
				}

				mSCSCFRestorationInfoGrpIterator = this->getSCSCFRestorationInfoGrpList()->begin();
				while(mSCSCFRestorationInfoGrpIterator != mSCSCFRestorationInfoGrpList->end())
				{
					lengthOffset = 0;
					array->size+=4;
					...
					if(((*mSCSCFRestorationInfoGrpIterator)->Encode(array) < 0) || 
							(DiameterTLVUtil::avp_length_encode(lengthOffset, array) == false)){ 
				...
				break;
			case ServerAssignmentAnswer_ASSOCIATEDREGISTEREDIDENTITIES:
			{
				array->size+=4;
					lTempHtonlVar =  htonl(647);
					memcpy(array->byte+array->offset, &lTempHtonlVar, DIA_AVP_CODE_SIZE);
					array->offset += DIA_AVP_CODE_SIZE;
				array->size++;
				array->byte[array->offset++] = 128;
				...
				if((this->getAssociatedRegisteredIdentities()->Encode(array) < 0) || 
						(DiameterTLVUtil::avp_length_encode(lengthOffset, array) == false)) {
					TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_ENCODE,"Error encoding AssociatedRegisteredIdentities");
					throw lTssDiaMsgException;
					}
				}
				break;
			case ServerAssignmentAnswer_SERVERNAME:
			{
					array->size+=4;
				...
				break;
			case ServerAssignmentAnswer_WILDCARDEDPUBLICIDENTITY:
			{
					array->size+=4;
				...
				break;
			case ServerAssignmentAnswer_PRIVILEDGEDSENDERINDICATION:
			{
					array->size+=4;
					...
					if(!mPriviledgedSenderIndication->mAvpDecodedFlag)
					{
						memcpy(array->byte+array->offset, &mspByteArray.get()->byte[mPriviledgedSenderIndication->getAvpOffsetLen().first], mPriviledgedSenderIndication->getAvpOffsetLen().second);
						array->offset += mPriviledgedSenderIndication->getAvpOffsetLen().second;
						array->size += mPriviledgedSenderIndication->getAvpOffsetLen().second;
						if(false == DiameterTLVUtil::avp_length_encode(lengthOffset, array)){
							TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_ENCODE,"Error encoding PriviledgedSenderIndication");
							throw lTssDiaMsgException;
						}
						continue;
					}
					if((mPriviledgedSenderIndication->Encode(array) < 0) || 
						(DiameterTLVUtil::avp_length_encode(lengthOffset, array) == false)){ 
						TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_ENCODE,"Error encoding PriviledgedSenderIndication");
						throw lTssDiaMsgException;
					}
				...
				break;
			case ServerAssignmentAnswer_ALLOWEDWAFWWSFIDENTITIES:
			{
				array->size+=4;
				...
				if((this->getAllowedWAFWWSFIdentities()->Encode(array) < 0) || 
						(DiameterTLVUtil::avp_length_encode(lengthOffset, array) == false)) {
					TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_ENCODE,"Error encoding AllowedWAFWWSFIdentities");
					throw lTssDiaMsgException;
					}
				}
				break;
			case ServerAssignmentAnswer_ERRORMESSAGE:
			{
					array->size+=4;
				...
				break;
			case ServerAssignmentAnswer_FAILEDAVP:
			{
				array->size+=4;
				...
				break;
			case ServerAssignmentAnswer_PROXYINFO:
			{
				if( mProxyInfoGrpList->size() <= 0) {
					break;
				}
				...
				break;
			case ServerAssignmentAnswer_ROUTERECORD:
			{
...
int ServerAssignmentAnswer::Decode(byteArray *array, INT16 lMLen, UINT16 *parsedBytes)
{
...
;	if(false == DiameterTLVUtil::decodeAvpCmdIntegerValue(array, mDiameterMsgHeader->mCmdCode, DIA_CMD_CODE_SIZE))
	...
	while( lMsgLen > 0)
	{
		...
		switch(lAvpCode)
		{
			...
			case 638:
				{
					if( NULL == this->setLooseRouteIndication())
					{
						mDecodeFlag = false;
						TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_DECODE,"Error decoding LooseRouteIndication");
						throw lTssDiaMsgException;
					}
					mLooseRouteIndication->setAvpCode(638);
					mLooseRouteIndication->setAvpVendorId(lVenderId);
					mLooseRouteIndication->SetDataType((DiameterDataType)7);
					mLooseRouteIndication->mspByteArray = mspByteArray;
					mLooseRouteIndication->mAvpDecodedFlag = false;
					mLooseRouteIndication->setAvpOffsetLen(array->offset,lAvpDataLen);
					array->offset+=lAvpDataLen;
			...
			case 639:
				{
					mTagsPresent[ServerAssignmentAnswer_SCSCFRESTORATIONINFO] = 1;
					SCSCFRestorationInfo *lSCSCFRestorationInfo = new SCSCFRestorationInfo();
					if( lSCSCFRestorationInfo == NULL ){
					mDecodeFlag = false;
						TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_DECODE,"Error decoding SCSCFRestorationInfo");
						throw lTssDiaMsgException;
					}
					lSCSCFRestorationInfo->setAvpCode(639);
					lSCSCFRestorationInfo->SetDataType((DiameterDataType)6);
					lSCSCFRestorationInfo->mspByteArray = mspByteArray;
					lSCSCFRestorationInfo->mAvpDecodedFlag = false;
					lSCSCFRestorationInfo->setAvpOffsetLen(array->offset,lAvpDataLen);
					array->offset+=lAvpDataLen;

					if(mSCSCFRestorationInfoGrpList == NULL)
						this->setSCSCFRestorationInfoGrpList();
					mSCSCFRestorationInfoGrpList->push_back(lSCSCFRestorationInfo);
					mBaseInterfaceList.push_back(lSCSCFRestorationInfo);
			...
			case 647:
				{
					mTagsPresent[ServerAssignmentAnswer_ASSOCIATEDREGISTEREDIDENTITIES] = 1;
						if( NULL == this->setAssociatedRegisteredIdentities()){
						mDecodeFlag = false;
						TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_DECODE,"Error decoding AssociatedRegisteredIdentities");
						throw lTssDiaMsgException;
						}
					mAssociatedRegisteredIdentitiesGrp->setAvpCode(647);
					mAssociatedRegisteredIdentitiesGrp->SetDataType((DiameterDataType)6);
					mAssociatedRegisteredIdentitiesGrp->mspByteArray = mspByteArray;
					mAssociatedRegisteredIdentitiesGrp->mAvpDecodedFlag = false;
					mAssociatedRegisteredIdentitiesGrp->setAvpOffsetLen(array->offset,lAvpDataLen);
					mBaseInterfaceList.push_back(mAssociatedRegisteredIdentitiesGrp);
					array->offset+=lAvpDataLen;
			...
			case 652:
				{
					if( NULL == this->setPriviledgedSenderIndication())
					{
						mDecodeFlag = false;
						TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_DECODE,"Error decoding PriviledgedSenderIndication");
						throw lTssDiaMsgException;
					}
					mPriviledgedSenderIndication->setAvpCode(652);
					mPriviledgedSenderIndication->setAvpVendorId(lVenderId);
					mPriviledgedSenderIndication->SetDataType((DiameterDataType)7);
					mPriviledgedSenderIndication->mspByteArray = mspByteArray;
					mPriviledgedSenderIndication->mAvpDecodedFlag = false;
					mPriviledgedSenderIndication->setAvpOffsetLen(array->offset,lAvpDataLen);
					array->offset+=lAvpDataLen;
			...
			case 656:
				{
					mTagsPresent[ServerAssignmentAnswer_ALLOWEDWAFWWSFIDENTITIES] = 1;
						if( NULL == this->setAllowedWAFWWSFIdentities()){
						mDecodeFlag = false;
						TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_DECODE,"Error decoding AllowedWAFWWSFIdentities");
						throw lTssDiaMsgException;
						}
					mAllowedWAFWWSFIdentitiesGrp->setAvpCode(656);
					mAllowedWAFWWSFIdentitiesGrp->SetDataType((DiameterDataType)6);
					mAllowedWAFWWSFIdentitiesGrp->mspByteArray = mspByteArray;
					mAllowedWAFWWSFIdentitiesGrp->mAvpDecodedFlag = false;
					mAllowedWAFWWSFIdentitiesGrp->setAvpOffsetLen(array->offset,lAvpDataLen);
					mBaseInterfaceList.push_back(mAllowedWAFWWSFIdentitiesGrp);
					array->offset+=lAvpDataLen;
...
DiameterBaseInterface* ServerAssignmentAnswer::Find(byteArray *array, UINT32 avpCode)
{
		...
		switch(avpCode)
		{
			...
			{
				if(mLooseRouteIndication != NULL)
					return mLooseRouteIndication->Find(array, avpCode);
				else
					return NULL;
			}
			case 602:
			...
			{
				if(mPriviledgedSenderIndication != NULL)
					return mPriviledgedSenderIndication->Find(array, avpCode);
				else
					return NULL;
			}
			case 281:
...
BOOLEAN ServerAssignmentAnswer::FindAll(byteArray *array, UINT32 avpCode, list<DiameterBaseInterface*>& pBaseInterface,BOOLEAN deepInspect)
{
		if(!mDecodeFlag && array != NULL)
			this->Decode(array);
		...
		switch(avpCode)
		{
			...
			case 638:
			{
				if(mLooseRouteIndication != NULL)
				{
					DiameterBaseInterface* inf =  mLooseRouteIndication->Find(this->mspByteArray.get(), avpCode);
					if(inf != NULL)
					{
						pBaseInterface.push_back(inf);
					}
			...
			case 639:
			{
				if(mSCSCFRestorationInfoGrpList != NULL)
				{
					for(auto itr = mSCSCFRestorationInfoGrpList->begin(); itr != mSCSCFRestorationInfoGrpList->end(); ++itr)
					{
						pBaseInterface.push_back(*itr);
					}
			...
			case 647:
			{
				if(mAssociatedRegisteredIdentitiesGrp != NULL)
				{
						pBaseInterface.push_back(mAssociatedRegisteredIdentitiesGrp);
				}
				if(deepInspect)
				{
					for(auto itr = mBaseInterfaceList.begin(); itr != mBaseInterfaceList.end(); ++itr)
			...
			case 652:
			{
				if(mPriviledgedSenderIndication != NULL)
				{
					DiameterBaseInterface* inf =  mPriviledgedSenderIndication->Find(this->mspByteArray.get(), avpCode);
					if(inf != NULL)
					{
						pBaseInterface.push_back(inf);
					}
			...
			case 656:
			{
				if(mAllowedWAFWWSFIdentitiesGrp != NULL)
				{
						pBaseInterface.push_back(mAllowedWAFWWSFIdentitiesGrp);
				}
				if(deepInspect)
				{
					for(auto itr = mBaseInterfaceList.begin(); itr != mBaseInterfaceList.end(); ++itr)
...
std::shared_ptr<byteArray> ServerAssignmentAnswer::GetByteArray()
{
	return mspByteArray;
}


BOOLEAN ServerAssignmentAnswer::FindAll(byteArray *array, std::map<int,UINT32> &avpCodeMap, int &size, int key, list<DiameterBaseInterface*>& pBaseInterface,BOOLEAN deepInspect)
{
		if(!mDecodeFlag && array != NULL)
			this->Decode(array);
		...
		switch(avpCode)
		{
			...
			case 647:
			{
				...
				if(deepInspect)
				{
					for(auto itr = mBaseInterfaceList.begin(); itr != mBaseInterfaceList.end(); ++itr)
					{
						mAssociatedRegisteredIdentitiesGrp->FindAll(this->mspByteArray.get(), avpCodeMap, size, (key+1),  pBaseInterface,deepInspect);
					}
				}
				}
				break;
			...
			case 656:
			{
				...
				if(deepInspect)
				{
					for(auto itr = mBaseInterfaceList.begin(); itr != mBaseInterfaceList.end(); ++itr)
					{
						mAllowedWAFWWSFIdentitiesGrp->FindAll(this->mspByteArray.get(), avpCodeMap, size, (key+1),  pBaseInterface,deepInspect);
					}
				}
				}
				break;
...
void ServerAssignmentAnswer::Display(UINT32 tab)
{
	...
	for(int i = 0; i < mTagsPresent.size() ; ++i)
	{
		...
		switch(i)
		{
			...
			case ServerAssignmentAnswer_USERDATA:
			{
				cout << setw(tempTab) << "" << "|"  << left <<  setw(12) << "AvpCode" << ":" << setw(15) << "606" << "|" << setw(12)  << "AvpType" << ":" << setw(15) << "SINGLE" << "|" << "\n"; 
			...
			case ServerAssignmentAnswer_LOOSEROUTEINDICATION:
			{
				cout << setw(tempTab) << "" << "|"  << left <<  setw(12) << "AvpCode" << ":" << setw(15) << "638" << "|" << setw(12)  << "AvpType" << ":" << setw(15) << "SINGLE" << "|" << "\n"; 
				cout << setw(tempTab) << "" << "|"  << left <<  setw(12) << "AvpName" << ":" << setw(44) << "LooseRouteIndication" << "\n"; 
				TEXT lAvpFlg[100];
				//Hardcoded value.;
				sprintf( lAvpFlg, "%s %s %s", ((DiameterTLVUtil::isVenderSpecific(128)==true)?"Vendor":""), ((DiameterTLVUtil::isMandatoryBitSet(128)==true)?"Mandatory":""), ((DiameterTLVUtil::isProtectedBitSet(128)==true)?"Protected":""));
				this->getLooseRouteIndication()->Display(tab+1);
			}
				break;
			case ServerAssignmentAnswer_SCSCFRESTORATIONINFO:
			{
				...
				mSCSCFRestorationInfoGrpIterator = this->getSCSCFRestorationInfoGrpList()->begin();
				while(mSCSCFRestorationInfoGrpIterator != this->getSCSCFRestorationInfoGrpList()->end()) {
					...
					//Hardcoded value.;
					sprintf( lAvpFlg, "%s %s %s", ((DiameterTLVUtil::isVenderSpecific(128)==true)?"Vendor":""), ((DiameterTLVUtil::isMandatoryBitSet(128)==true)?"Mandatory":""), ((DiameterTLVUtil::isProtectedBitSet(128)==true)?"Protected":""));
					(*mSCSCFRestorationInfoGrpIterator)->Display(tab+1);
					mSCSCFRestorationInfoGrpIterator++;
			...
			case ServerAssignmentAnswer_ASSOCIATEDREGISTEREDIDENTITIES:
			{
				cout << setw(tempTab) << "" << "|"  << left <<  setw(12) << "AvpCode" << ":" << setw(15) << "647" << "|" << setw(12)  << "AvpType" << ":" << setw(15) << "GROUPED" << "|" << "\n"; 
				cout << setw(tempTab) << "" << "|"  << left <<  setw(12) << "AvpName" << ":" << setw(44) << "AssociatedRegisteredIdentities" << "\n"; 
				TEXT lAvpFlg[100];
				//Hardcoded value.;
				sprintf( lAvpFlg, "%s %s %s", ((DiameterTLVUtil::isVenderSpecific(128)==true)?"Vendor":""), ((DiameterTLVUtil::isMandatoryBitSet(128)==true)?"Mandatory":""), ((DiameterTLVUtil::isProtectedBitSet(128)==true)?"Protected":""));
				this->getAssociatedRegisteredIdentities()->Display(tab+1);
			}
				break;
			case ServerAssignmentAnswer_SERVERNAME:
			{
			...
			case ServerAssignmentAnswer_PRIVILEDGEDSENDERINDICATION:
			{
				...
				cout << setw(tempTab) << "" << "|"  << left <<  setw(12) << "AvpName" << ":" << setw(44) << "PriviledgedSenderIndication" << "\n"; 
				TEXT lAvpFlg[100];
				//Hardcoded value.;
				sprintf( lAvpFlg, "%s %s %s", ((DiameterTLVUtil::isVenderSpecific(128)==true)?"Vendor":""), ((DiameterTLVUtil::isMandatoryBitSet(128)==true)?"Mandatory":""), ((DiameterTLVUtil::isProtectedBitSet(128)==true)?"Protected":""));
				this->getPriviledgedSenderIndication()->Display(tab+1);
			}
				break;
			case ServerAssignmentAnswer_ALLOWEDWAFWWSFIDENTITIES:
			{
				cout << setw(tempTab) << "" << "|"  << left <<  setw(12) << "AvpCode" << ":" << setw(15) << "656" << "|" << setw(12)  << "AvpType" << ":" << setw(15) << "GROUPED" << "|" << "\n"; 
				cout << setw(tempTab) << "" << "|"  << left <<  setw(12) << "AvpName" << ":" << setw(44) << "AllowedWAFWWSFIdentities" << "\n"; 
				TEXT lAvpFlg[100];
				//Hardcoded value.;
				sprintf( lAvpFlg, "%s %s %s", ((DiameterTLVUtil::isVenderSpecific(128)==true)?"Vendor":""), ((DiameterTLVUtil::isMandatoryBitSet(128)==true)?"Mandatory":""), ((DiameterTLVUtil::isProtectedBitSet(128)==true)?"Protected":""));
				this->getAllowedWAFWWSFIdentities()->Display(tab+1);
			}
				break;
			case ServerAssignmentAnswer_ERRORMESSAGE:
			{
...
BOOLEAN ServerAssignmentAnswer::GetMissingAvpInfo(TEXT *lMissingAvpInfo)
{
	bitset<27> lMissingAvpTag(mMandatoryAvpSetVal ^ mMandatoryAvpCheckVal);
	...
	for(unsigned int i = 0 ; i < lMissingAvpTag.size(); ++i)
	{
		...
		switch(i)
		{
			...
			case ServerAssignmentAnswer_USERDATA:
			{
				...
				{
					lFirstRec = false;
					sprintf(lMissingAvpInfo, "Missing Avps List: 606");
				}
				else
					sprintf(lMissingAvpInfo, "%s,606", lMissingAvpInfo);
			}
				break;
			case ServerAssignmentAnswer_CHARGINGINFORMATION:
			{
			...
			case ServerAssignmentAnswer_LOOSEROUTEINDICATION:
			{
				...
				{
					lFirstRec = false;
					sprintf(lMissingAvpInfo, "Missing Avps List: 638");
				}
				else
					sprintf(lMissingAvpInfo, "%s,638", lMissingAvpInfo);
			}
				break;
			case ServerAssignmentAnswer_SCSCFRESTORATIONINFO:
			{
			...
			case ServerAssignmentAnswer_ASSOCIATEDREGISTEREDIDENTITIES:
			{
				...
				{
					lFirstRec = false;
					sprintf(lMissingAvpInfo, "Missing Avps List: 647");
				}
				else
					sprintf(lMissingAvpInfo, "%s,647", lMissingAvpInfo);
			}
				break;
			case ServerAssignmentAnswer_SERVERNAME:
			{
