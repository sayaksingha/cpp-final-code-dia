...
OCOLR::~OCOLR()
{
	...
	if(mOCRegTimerExt != NULL)
		delete mOCRegTimerExt;
}
...
Unsigned32* OCOLR::setOCRegTimerExt(){
	if(mOCRegTimerExt == NULL)
	{
		mOCRegTimerExt = new Unsigned32();
		mTagsPresent[OCOLR_OCREGTIMEREXT] = 1;
		return mOCRegTimerExt;
	}
	return mOCRegTimerExt;
}

void OCOLR::resetOCRegTimerExt(){
	mTagsPresent[OCOLR_OCREGTIMEREXT] = 0;
	if(mOCRegTimerExt != NULL)
	{
		delete mOCRegTimerExt; 
		mOCRegTimerExt = NULL;
...
Unsigned32* OCOLR::getOCRegTimerExt(){
	if(NULL == mOCRegTimerExt) {
		TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "OCRegTimerExt is Null");
		throw lTssDiaMsgException;
	}
	if(!mOCRegTimerExt->mAvpDecodedFlag)
	{
			if(mspByteArray.get() == NULL)
			{
			mOCRegTimerExt->mAvpDecodedFlag = true;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "OCRegTimerExt is Null");
			throw lTssDiaMsgException;
			}
		mspByteArray.get()->offset = 0;
		mspByteArray.get()->offset = mOCRegTimerExt->getAvpOffsetLen().first;
		UINT16 lBytesDecoded = 0;
		if(mOCRegTimerExt->Decode(mspByteArray.get(), mOCRegTimerExt->getAvpOffsetLen().second, &lBytesDecoded)< 0)
		{
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_DECODE,"Error decoding OCRegTimerExt")     ;
			throw lTssDiaMsgException;
		}
		mOCRegTimerExt->mAvpDecodedFlag = true;
	}
	...
	for(unsigned int i = 0 ; i < mTagsPresent.size() ; ++i )
	{
		...
		switch(i)
		{
			...
			case OCOLR_OCVALIDITYDURATION:
			{
				...
				break;
			case OCOLR_OCREGTIMEREXT:
			{
					array->size+=4;
					lTempHtonlVar =  htonl(667);
					memcpy(array->byte+array->offset, &lTempHtonlVar, DIA_AVP_CODE_SIZE);
					array->offset += DIA_AVP_CODE_SIZE;
					array->size++;
					...
					if(!mOCRegTimerExt->mAvpDecodedFlag)
					{
						memcpy(array->byte+array->offset, &mspByteArray.get()->byte[mOCRegTimerExt->getAvpOffsetLen().first], mOCRegTimerExt->getAvpOffsetLen().second);
						array->offset += mOCRegTimerExt->getAvpOffsetLen().second;
						array->size += mOCRegTimerExt->getAvpOffsetLen().second;
						if(false == DiameterTLVUtil::avp_length_encode(lengthOffset, array)){
							TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_ENCODE,"Error encoding OCRegTimerExt");
							throw lTssDiaMsgException;
						}
						continue;
					}
					if((mOCRegTimerExt->Encode(array) < 0) || 
						(DiameterTLVUtil::avp_length_encode(lengthOffset, array) == false)){ 
						TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_ENCODE,"Error encoding OCRegTimerExt");
						throw lTssDiaMsgException;
					}
...
int OCOLR::Decode(byteArray *array, INT16 lMsgLen, UINT16 *parsedBytes)
{
	...
	while( lMsgLen > 0)
	{
		...
		switch(lAvpCode)
		{
			...
			case 667:
				{
					if( NULL == this->setOCRegTimerExt())
					{
						mDecodeFlag = false;
						TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_DECODE,"Error decoding OCRegTimerExt");
						throw lTssDiaMsgException;
					}
					mOCRegTimerExt->setAvpCode(667);
					mOCRegTimerExt->setAvpVendorId(lVenderId);
					mOCRegTimerExt->SetDataType((DiameterDataType)3);
					mOCRegTimerExt->mspByteArray = mspByteArray;
					mOCRegTimerExt->mAvpDecodedFlag = false;
					mOCRegTimerExt->setAvpOffsetLen(array->offset,lAvpDataLen);
					array->offset+=lAvpDataLen;
...
DiameterBaseInterface* OCOLR::Find(byteArray *array, UINT32 avpCode)
{
	if(!this->mAvpDecodedFlag)
	{
		...
		switch(avpCode)
		{
			...
			{
				if(mOCRegTimerExt != NULL)
					return mOCRegTimerExt->Find(array, avpCode);
				else
					return NULL;
			}
			default:
	...
	if(!this->mAvpDecodedFlag)
	{
		...
		switch(avpCode)
		{
			...
			case 667:
			{
				if(mOCRegTimerExt != NULL)
				{
					DiameterBaseInterface* inf =  mOCRegTimerExt->Find(this->mspByteArray.get(), avpCode);
					if(inf != NULL)
					{
						pBaseInterface.push_back(inf);
					}
...
void OCOLR::Display(UINT32 tab)
{
	...
	for(int i = 0; i < mTagsPresent.size() ; ++i)
	{
		...
		switch(i)
		{
			...
			case OCOLR_OCREGTIMEREXT:
			{
				cout << setw(tempTab) << "" << "|"  << left <<  setw(12) << "AvpCode" << ":" << setw(15) << "667" << "|" << setw(12)  << "AvpType" << ":" << setw(15) << "SINGLE" << "|" << "\n"; 
				cout << setw(tempTab) << "" << "|"  << left <<  setw(12) << "AvpName" << ":" << setw(44) << "OCRegTimerExt" << "\n"; 
				TEXT lAvpFlg[100];
				//Hardcoded value.;
				...
				this->getOCRegTimerExt()->Display(tab+1);
			}
				break;
			default:
				//printf("Unknown Tag received Tag:%d Offset:%d\n", mTagArray[lTemp], array->offset);
	...
	for(unsigned int i = 0 ; i < lMissingAvpTag.size(); ++i)
	{
		...
		switch(i)
		{
			...
			case OCOLR_OCREGTIMEREXT:
			{
				...
				{
					lFirstRec = false;
					sprintf(lMissingAvpInfo, "Missing Avps List: 667");
				}
				else
					sprintf(lMissingAvpInfo, "%s,667", lMissingAvpInfo);
			}
				break;
			default:
				break;
