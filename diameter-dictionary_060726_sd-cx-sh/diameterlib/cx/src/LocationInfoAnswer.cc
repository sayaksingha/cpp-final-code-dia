...
using namespace CX;
BOOLEAN LocationInfoAnswer::isPresent(int tag){
...
LocationInfoAnswer::LocationInfoAnswer()
{
	...
	mTagsPresent.reset();
	mMandatoryAvpCheckVal= bitset<20>(string("11100011001111111101"));
...
LocationInfoAnswer::~LocationInfoAnswer()
{
	...
	if(mServerName != NULL)
		delete mServerName;
	if(mServerCapabilitiesGrp != NULL)
		delete mServerCapabilitiesGrp;
...
OctetString* LocationInfoAnswer::setSessionId(){
	mMandatoryAvpSetVal [LocationInfoAnswer_SESSIONID] = 1;
	if(mSessionId == NULL)
	{
		mSessionId = new OctetString();
		mTagsPresent[LocationInfoAnswer_SESSIONID] = 1;
		return mSessionId;
	}
	return mSessionId;
}

void LocationInfoAnswer::resetSessionId(){
	mTagsPresent[LocationInfoAnswer_SESSIONID] = 0;
	if(mSessionId != NULL)
	{
		delete mSessionId; 
		mSessionId = NULL;
...
DRMP* LocationInfoAnswer::setDRMP(){
	if(mDRMP == NULL)
	{
		mDRMP = new DRMP();
		mTagsPresent[LocationInfoAnswer_DRMP] = 1;
		return mDRMP;
	}
	return mDRMP;
}

void LocationInfoAnswer::resetDRMP(){
	mTagsPresent[LocationInfoAnswer_DRMP] = 0;
	if(mDRMP != NULL)
	{
		delete mDRMP; 
		mDRMP = NULL;
...
Unsigned32* LocationInfoAnswer::setResultCode(){
	mMandatoryAvpSetVal [LocationInfoAnswer_RESULTCODE] = 1;
	if(mResultCode == NULL)
	{
		mResultCode = new Unsigned32();
		mTagsPresent[LocationInfoAnswer_RESULTCODE] = 1;
		return mResultCode;
	}
	return mResultCode;
}

void LocationInfoAnswer::resetResultCode(){
	mTagsPresent[LocationInfoAnswer_RESULTCODE] = 0;
	if(mResultCode != NULL)
	{
		delete mResultCode; 
		mResultCode = NULL;
...
AuthSessionState* LocationInfoAnswer::setAuthSessionState(){
	mMandatoryAvpSetVal [LocationInfoAnswer_AUTHSESSIONSTATE] = 1;
	if(mAuthSessionState == NULL)
	{
		mAuthSessionState = new AuthSessionState();
		mTagsPresent[LocationInfoAnswer_AUTHSESSIONSTATE] = 1;
		return mAuthSessionState;
	}
	return mAuthSessionState;
}

void LocationInfoAnswer::resetAuthSessionState(){
	mTagsPresent[LocationInfoAnswer_AUTHSESSIONSTATE] = 0;
	if(mAuthSessionState != NULL)
	{
		delete mAuthSessionState; 
		mAuthSessionState = NULL;
...
OctetString* LocationInfoAnswer::setOriginHost(){
	mMandatoryAvpSetVal [LocationInfoAnswer_ORIGINHOST] = 1;
	if(mOriginHost == NULL)
	{
		mOriginHost = new OctetString();
		mTagsPresent[LocationInfoAnswer_ORIGINHOST] = 1;
		return mOriginHost;
	}
	return mOriginHost;
}

void LocationInfoAnswer::resetOriginHost(){
	mTagsPresent[LocationInfoAnswer_ORIGINHOST] = 0;
	if(mOriginHost != NULL)
	{
		delete mOriginHost; 
		mOriginHost = NULL;
...
OctetString* LocationInfoAnswer::setOriginRealm(){
	mMandatoryAvpSetVal [LocationInfoAnswer_ORIGINREALM] = 1;
	if(mOriginRealm == NULL)
	{
		mOriginRealm = new OctetString();
		mTagsPresent[LocationInfoAnswer_ORIGINREALM] = 1;
		return mOriginRealm;
	}
	return mOriginRealm;
}

void LocationInfoAnswer::resetOriginRealm(){
	mTagsPresent[LocationInfoAnswer_ORIGINREALM] = 0;
	if(mOriginRealm != NULL)
	{
		delete mOriginRealm; 
		mOriginRealm = NULL;
...
OctetString* LocationInfoAnswer::setServerName(){
	mMandatoryAvpSetVal [LocationInfoAnswer_SERVERNAME] = 1;
	if(mServerName == NULL)
	{
		mServerName = new OctetString();
		mTagsPresent[LocationInfoAnswer_SERVERNAME] = 1;
		return mServerName;
	}
	return mServerName;
}

void LocationInfoAnswer::resetServerName(){
	mTagsPresent[LocationInfoAnswer_SERVERNAME] = 0;
	if(mServerName != NULL)
	{
		delete mServerName; 
		mServerName = NULL;
...
OctetString* LocationInfoAnswer::setWildcardedPublicIdentity(){
	if(mWildcardedPublicIdentity == NULL)
	{
		mWildcardedPublicIdentity = new OctetString();
		mTagsPresent[LocationInfoAnswer_WILDCARDEDPUBLICIDENTITY] = 1;
		return mWildcardedPublicIdentity;
	}
	return mWildcardedPublicIdentity;
}

void LocationInfoAnswer::resetWildcardedPublicIdentity(){
	mTagsPresent[LocationInfoAnswer_WILDCARDEDPUBLICIDENTITY] = 0;
	if(mWildcardedPublicIdentity != NULL)
	{
		delete mWildcardedPublicIdentity; 
		mWildcardedPublicIdentity = NULL;
...
Unsigned32* LocationInfoAnswer::setLIAFlags(){
	if(mLIAFlags == NULL)
	{
		mLIAFlags = new Unsigned32();
		mTagsPresent[LocationInfoAnswer_LIAFLAGS] = 1;
		return mLIAFlags;
	}
	return mLIAFlags;
}

void LocationInfoAnswer::resetLIAFlags(){
	mTagsPresent[LocationInfoAnswer_LIAFLAGS] = 0;
	if(mLIAFlags != NULL)
	{
		delete mLIAFlags; 
		mLIAFlags = NULL;
...
OctetString* LocationInfoAnswer::setErrorMessage(){
	if(mErrorMessage == NULL)
	{
		mErrorMessage = new OctetString();
		mTagsPresent[LocationInfoAnswer_ERRORMESSAGE] = 1;
		return mErrorMessage;
	}
	return mErrorMessage;
}

void LocationInfoAnswer::resetErrorMessage(){
	mTagsPresent[LocationInfoAnswer_ERRORMESSAGE] = 0;
	if(mErrorMessage != NULL)
	{
		delete mErrorMessage; 
		mErrorMessage = NULL;
...
OctetString* LocationInfoAnswer::getSessionId(){
	if(NULL == mSessionId) {
...
DRMP* LocationInfoAnswer::getDRMP(){
	if(NULL == mDRMP) {
...
CX::VendorSpecificApplicationId* LocationInfoAnswer::getVendorSpecificApplicationId()
{
	if(mVendorSpecificApplicationIdGrp == NULL)
	{
...
CX::VendorSpecificApplicationId* LocationInfoAnswer::setVendorSpecificApplicationId(){
	if(mVendorSpecificApplicationIdGrp == NULL)
		mVendorSpecificApplicationIdGrp = new VendorSpecificApplicationId();
	mTagsPresent[LocationInfoAnswer_VENDORSPECIFICAPPLICATIONID] = 1;
	mMandatoryAvpSetVal[LocationInfoAnswer_VENDORSPECIFICAPPLICATIONID] = 1;
	return mVendorSpecificApplicationIdGrp;
}

void LocationInfoAnswer::resetVendorSpecificApplicationId(){
	mTagsPresent[LocationInfoAnswer_VENDORSPECIFICAPPLICATIONID] = 0;
	if(mVendorSpecificApplicationIdGrp != NULL)
		delete mVendorSpecificApplicationIdGrp;
}

Unsigned32* LocationInfoAnswer::getResultCode(){
	if(NULL == mResultCode) {
...
ExperimentalResult* LocationInfoAnswer::getExperimentalResult()
{
	if(mExperimentalResultGrp == NULL)
	{
...
ExperimentalResult* LocationInfoAnswer::setExperimentalResult(){
	if(mExperimentalResultGrp == NULL)
		mExperimentalResultGrp = new ExperimentalResult();
	mTagsPresent[LocationInfoAnswer_EXPERIMENTALRESULT] = 1;
	mMandatoryAvpSetVal[LocationInfoAnswer_EXPERIMENTALRESULT] = 1;
	return mExperimentalResultGrp;
}

void LocationInfoAnswer::resetExperimentalResult(){
	mTagsPresent[LocationInfoAnswer_EXPERIMENTALRESULT] = 0;
	if(mExperimentalResultGrp != NULL)
		delete mExperimentalResultGrp;
}

AuthSessionState* LocationInfoAnswer::getAuthSessionState(){
...
OctetString* LocationInfoAnswer::getOriginHost(){
	if(NULL == mOriginHost) {
...
OctetString* LocationInfoAnswer::getOriginRealm(){
...
OCSupportedFeatures* LocationInfoAnswer::getOCSupportedFeatures()
{
	if(mOCSupportedFeaturesGrp == NULL)
	{
...
OCSupportedFeatures* LocationInfoAnswer::setOCSupportedFeatures(){
	if(mOCSupportedFeaturesGrp == NULL)
		mOCSupportedFeaturesGrp = new OCSupportedFeatures();
	mTagsPresent[LocationInfoAnswer_OCSUPPORTEDFEATURES] = 1;
	mMandatoryAvpSetVal[LocationInfoAnswer_OCSUPPORTEDFEATURES] = 1;
	return mOCSupportedFeaturesGrp;
}

void LocationInfoAnswer::resetOCSupportedFeatures(){
	mTagsPresent[LocationInfoAnswer_OCSUPPORTEDFEATURES] = 0;
	if(mOCSupportedFeaturesGrp != NULL)
		delete mOCSupportedFeaturesGrp;
}

OCOLR* LocationInfoAnswer::getOCOLR()
{
	if(mOCOLRGrp == NULL)
	{
...
OCOLR* LocationInfoAnswer::setOCOLR(){
	if(mOCOLRGrp == NULL)
		mOCOLRGrp = new OCOLR();
	mTagsPresent[LocationInfoAnswer_OCOLR] = 1;
	mMandatoryAvpSetVal[LocationInfoAnswer_OCOLR] = 1;
	return mOCOLRGrp;
}

void LocationInfoAnswer::resetOCOLR(){
	mTagsPresent[LocationInfoAnswer_OCOLR] = 0;
	if(mOCOLRGrp != NULL)
		delete mOCOLRGrp;
}

list<Load*>* LocationInfoAnswer::getLoadGrpList()
{
	if(mLoadGrpList == NULL)
	{
	...
	for(auto itr : *mLoadGrpList)
	{
		...
		{
			mTagsPresent[LocationInfoAnswer_LOAD] = 0;
			mLoadGrpList = NULL;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "Load is Null");
			throw lTssDiaMsgException;
		}
...
list<Load*>* LocationInfoAnswer::setLoadGrpList(){
	mTagsPresent[LocationInfoAnswer_LOAD] = 1;
	if(mLoadGrpList == NULL)
...
void LocationInfoAnswer::resetLoadGrpList(){
	mTagsPresent[LocationInfoAnswer_LOAD] = 0;
	if(mLoadGrpList != NULL)
		delete mLoadGrpList;
}

list<SupportedFeatures*>* LocationInfoAnswer::getSupportedFeaturesGrpList()
{
	if(mSupportedFeaturesGrpList == NULL)
	{
	...
	for(auto itr : *mSupportedFeaturesGrpList)
	{
		...
		{
			mTagsPresent[LocationInfoAnswer_SUPPORTEDFEATURES] = 0;
			mSupportedFeaturesGrpList = NULL;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "SupportedFeatures is Null");
			throw lTssDiaMsgException;
		}
...
list<SupportedFeatures*>* LocationInfoAnswer::setSupportedFeaturesGrpList(){
	mTagsPresent[LocationInfoAnswer_SUPPORTEDFEATURES] = 1;
	if(mSupportedFeaturesGrpList == NULL)
...
void LocationInfoAnswer::resetSupportedFeaturesGrpList(){
	mTagsPresent[LocationInfoAnswer_SUPPORTEDFEATURES] = 0;
	if(mSupportedFeaturesGrpList != NULL)
		delete mSupportedFeaturesGrpList;
}

OctetString* LocationInfoAnswer::getServerName(){
	if(NULL == mServerName) {
...
ServerCapabilities* LocationInfoAnswer::getServerCapabilities()
{
	if(mServerCapabilitiesGrp == NULL)
	{
		TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "ServerCapabilities is Null");
		throw lTssDiaMsgException;
	}
	if(!mServerCapabilitiesGrp->mAvpDecodedFlag)
	{
			if(mspByteArray.get() == NULL)
			{
			mServerCapabilitiesGrp->mAvpDecodedFlag = true;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "ServerCapabilities is Null");
			throw lTssDiaMsgException;
			}
		mspByteArray.get()->offset = 0;
		mspByteArray.get()->offset = mServerCapabilitiesGrp->getAvpOffsetLen().first;
		UINT16 lBytesDecoded = 0;
		if(mServerCapabilitiesGrp->Decode(mspByteArray.get(), mServerCapabilitiesGrp->getAvpOffsetLen().second, &lBytesDecoded)< 0)
		{
	...
	return mServerCapabilitiesGrp;
}

ServerCapabilities* LocationInfoAnswer::setServerCapabilities(){
	if(mServerCapabilitiesGrp == NULL)
		mServerCapabilitiesGrp = new ServerCapabilities();
	mTagsPresent[LocationInfoAnswer_SERVERCAPABILITIES] = 1;
	mMandatoryAvpSetVal[LocationInfoAnswer_SERVERCAPABILITIES] = 1;
	return mServerCapabilitiesGrp;
}

void LocationInfoAnswer::resetServerCapabilities(){
	mTagsPresent[LocationInfoAnswer_SERVERCAPABILITIES] = 0;
	if(mServerCapabilitiesGrp != NULL)
		delete mServerCapabilitiesGrp;
}

OctetString* LocationInfoAnswer::getWildcardedPublicIdentity(){
...
Unsigned32* LocationInfoAnswer::getLIAFlags(){
	if(NULL == mLIAFlags) {
		TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "LIAFlags is Null");
		throw lTssDiaMsgException;
	}
	if(!mLIAFlags->mAvpDecodedFlag)
	{
			if(mspByteArray.get() == NULL)
			{
			mLIAFlags->mAvpDecodedFlag = true;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "LIAFlags is Null");
			throw lTssDiaMsgException;
			}
		mspByteArray.get()->offset = 0;
		mspByteArray.get()->offset = mLIAFlags->getAvpOffsetLen().first;
		UINT16 lBytesDecoded = 0;
		if(mLIAFlags->Decode(mspByteArray.get(), mLIAFlags->getAvpOffsetLen().second, &lBytesDecoded)< 0)
		{
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_DECODE,"Error decoding LIAFlags")     ;
			throw lTssDiaMsgException;
		}
		mLIAFlags->mAvpDecodedFlag = true;
	}
...
OctetString* LocationInfoAnswer::getErrorMessage(){
...
FailedAVP* LocationInfoAnswer::getFailedAVP()
{
	if(mFailedAVPGrp == NULL)
	{
...
FailedAVP* LocationInfoAnswer::setFailedAVP(){
	if(mFailedAVPGrp == NULL)
		mFailedAVPGrp = new FailedAVP();
	mTagsPresent[LocationInfoAnswer_FAILEDAVP] = 1;
	mMandatoryAvpSetVal[LocationInfoAnswer_FAILEDAVP] = 1;
	return mFailedAVPGrp;
}

void LocationInfoAnswer::resetFailedAVP(){
	mTagsPresent[LocationInfoAnswer_FAILEDAVP] = 0;
	if(mFailedAVPGrp != NULL)
		delete mFailedAVPGrp;
}

list<CX::ProxyInfo*>* LocationInfoAnswer::getProxyInfoGrpList()
{
	if(mProxyInfoGrpList == NULL)
	{
	...
	for(auto itr : *mProxyInfoGrpList)
	{
		...
		{
			mTagsPresent[LocationInfoAnswer_PROXYINFO] = 0;
			mProxyInfoGrpList = NULL;
			TssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, "ProxyInfo is Null");
			throw lTssDiaMsgException;
		}
...
list<CX::ProxyInfo*>* LocationInfoAnswer::setProxyInfoGrpList(){
	mTagsPresent[LocationInfoAnswer_PROXYINFO] = 1;
	if(mProxyInfoGrpList == NULL)
		mProxyInfoGrpList = new list<CX::ProxyInfo*>;
	mMandatoryAvpSetVal[LocationInfoAnswer_PROXYINFO] = 1;
	mProxyInfoGrpList->clear();
	return mProxyInfoGrpList;
}

void LocationInfoAnswer::resetProxyInfoGrpList(){
	mTagsPresent[LocationInfoAnswer_PROXYINFO] = 0;
	if(mProxyInfoGrpList != NULL)
		delete mProxyInfoGrpList;
}

list<OctetString*>* LocationInfoAnswer::getRouteRecordList()
{
	if(mRouteRecordList == NULL)
	{
...
list<OctetString*>* LocationInfoAnswer::setRouteRecordList() {
	mTagsPresent[LocationInfoAnswer_ROUTERECORD] = 1;
	if(mRouteRecordList == NULL)
		mRouteRecordList = new list<OctetString*>;
	mMandatoryAvpSetVal [LocationInfoAnswer_ROUTERECORD] = 1;
	mRouteRecordList->clear();
	return mRouteRecordList;
}

void LocationInfoAnswer::resetRouteRecordList(){
	mTagsPresent[LocationInfoAnswer_ROUTERECORD] = 0;
	if(mRouteRecordList != NULL)
		delete mRouteRecordList;
}
...
int LocationInfoAnswer::Encode(byteArray *array)
{
	...
	for(unsigned int i = 0 ; i < mTagsPresent.size() ; ++i )
	{
		...
		switch(i)
		{
			case LocationInfoAnswer_SESSIONID:
			{
					array->size+=4;
			...
			case LocationInfoAnswer_DRMP:
			{
				...
				break;
			case LocationInfoAnswer_VENDORSPECIFICAPPLICATIONID:
			{
				array->size+=4;
				...
				break;
			case LocationInfoAnswer_RESULTCODE:
			{
					array->size+=4;
				...
				break;
			case LocationInfoAnswer_EXPERIMENTALRESULT:
			{
				array->size+=4;
				...
				break;
			case LocationInfoAnswer_AUTHSESSIONSTATE:
			{
					array->size+=4;
				...
				break;
			case LocationInfoAnswer_ORIGINHOST:
			{
					array->size+=4;
				...
				break;
			case LocationInfoAnswer_ORIGINREALM:
			{
					array->size+=4;
				...
				break;
			case LocationInfoAnswer_OCSUPPORTEDFEATURES:
			{
				array->size+=4;
			...
			case LocationInfoAnswer_LOAD:
			{
				...
				break;
			case LocationInfoAnswer_SUPPORTEDFEATURES:
			{
				if( mSupportedFeaturesGrpList->size() <= 0) {
					break;
				}
				...
				break;
			case LocationInfoAnswer_SERVERNAME:
			{
					array->size+=4;
				...
				break;
			case LocationInfoAnswer_SERVERCAPABILITIES:
			{
				array->size+=4;
					lTempHtonlVar =  htonl(603);
					memcpy(array->byte+array->offset, &lTempHtonlVar, DIA_AVP_CODE_SIZE);
					array->offset += DIA_AVP_CODE_SIZE;
				array->size++;
				array->byte[array->offset++] = 192;
				...
				if((this->getServerCapabilities()->Encode(array) < 0) || 
						(DiameterTLVUtil::avp_length_encode(lengthOffset, array) == false)) {
					TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_ENCODE,"Error encoding ServerCapabilities");
					throw lTssDiaMsgException;
					}
				}
				break;
			case LocationInfoAnswer_WILDCARDEDPUBLICIDENTITY:
			{
					array->size+=4;
				...
				break;
			case LocationInfoAnswer_LIAFLAGS:
			{
					array->size+=4;
					...
					if(!mLIAFlags->mAvpDecodedFlag)
					{
						memcpy(array->byte+array->offset, &mspByteArray.get()->byte[mLIAFlags->getAvpOffsetLen().first], mLIAFlags->getAvpOffsetLen().second);
						array->offset += mLIAFlags->getAvpOffsetLen().second;
						array->size += mLIAFlags->getAvpOffsetLen().second;
						if(false == DiameterTLVUtil::avp_length_encode(lengthOffset, array)){
							TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_ENCODE,"Error encoding LIAFlags");
							throw lTssDiaMsgException;
						}
						continue;
					}
					if((mLIAFlags->Encode(array) < 0) || 
						(DiameterTLVUtil::avp_length_encode(lengthOffset, array) == false)){ 
						TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_ENCODE,"Error encoding LIAFlags");
						throw lTssDiaMsgException;
					}
				...
				break;
			case LocationInfoAnswer_ERRORMESSAGE:
			{
					array->size+=4;
				...
				break;
			case LocationInfoAnswer_FAILEDAVP:
			{
				array->size+=4;
				...
				break;
			case LocationInfoAnswer_PROXYINFO:
			{
				if( mProxyInfoGrpList->size() <= 0) {
					break;
				}
				...
				break;
			case LocationInfoAnswer_ROUTERECORD:
			{
...
int LocationInfoAnswer::Decode(byteArray *array, INT16 lMLen, UINT16 *parsedBytes)
{
...
;	if(false == DiameterTLVUtil::decodeAvpCmdIntegerValue(array, mDiameterMsgHeader->mCmdCode, DIA_CMD_CODE_SIZE))
	...
	while( lMsgLen > 0)
	{
		...
		switch(lAvpCode)
		{
			...
			case 603:
				{
						mMandatoryAvpSetVal [LocationInfoAnswer_SERVERCAPABILITIES] = 1;
					mTagsPresent[LocationInfoAnswer_SERVERCAPABILITIES] = 1;
						if( NULL == this->setServerCapabilities()){
						mDecodeFlag = false;
						TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_DECODE,"Error decoding ServerCapabilities");
						throw lTssDiaMsgException;
						}
					mServerCapabilitiesGrp->setAvpCode(603);
					mServerCapabilitiesGrp->SetDataType((DiameterDataType)6);
					mServerCapabilitiesGrp->mspByteArray = mspByteArray;
					mServerCapabilitiesGrp->mAvpDecodedFlag = false;
					mServerCapabilitiesGrp->setAvpOffsetLen(array->offset,lAvpDataLen);
					mBaseInterfaceList.push_back(mServerCapabilitiesGrp);
					array->offset+=lAvpDataLen;
			...
			case 653:
				{
					if( NULL == this->setLIAFlags())
					{
						mDecodeFlag = false;
						TssDiameterMsgException lTssDiaMsgException(DIAMETER_ERR_DECODE,"Error decoding LIAFlags");
						throw lTssDiaMsgException;
					}
					mLIAFlags->setAvpCode(653);
					mLIAFlags->setAvpVendorId(lVenderId);
					mLIAFlags->SetDataType((DiameterDataType)3);
					mLIAFlags->mspByteArray = mspByteArray;
					mLIAFlags->mAvpDecodedFlag = false;
					mLIAFlags->setAvpOffsetLen(array->offset,lAvpDataLen);
					array->offset+=lAvpDataLen;
...
DiameterBaseInterface* LocationInfoAnswer::Find(byteArray *array, UINT32 avpCode)
{
		...
		switch(avpCode)
		{
			...
			{
				if(mLIAFlags != NULL)
					return mLIAFlags->Find(array, avpCode);
				else
					return NULL;
			}
			case 281:
...
BOOLEAN LocationInfoAnswer::FindAll(byteArray *array, UINT32 avpCode, list<DiameterBaseInterface*>& pBaseInterface,BOOLEAN deepInspect)
{
		if(!mDecodeFlag && array != NULL)
			this->Decode(array);
		...
		switch(avpCode)
		{
			...
			case 603:
			{
				if(mServerCapabilitiesGrp != NULL)
				{
						pBaseInterface.push_back(mServerCapabilitiesGrp);
				}
				if(deepInspect)
				{
					for(auto itr = mBaseInterfaceList.begin(); itr != mBaseInterfaceList.end(); ++itr)
			...
			case 653:
			{
				if(mLIAFlags != NULL)
				{
					DiameterBaseInterface* inf =  mLIAFlags->Find(this->mspByteArray.get(), avpCode);
					if(inf != NULL)
					{
						pBaseInterface.push_back(inf);
					}
...
std::shared_ptr<byteArray> LocationInfoAnswer::GetByteArray()
{
	return mspByteArray;
}


BOOLEAN LocationInfoAnswer::FindAll(byteArray *array, std::map<int,UINT32> &avpCodeMap, int &size, int key, list<DiameterBaseInterface*>& pBaseInterface,BOOLEAN deepInspect)
{
		if(!mDecodeFlag && array != NULL)
			this->Decode(array);
		...
		switch(avpCode)
		{
			...
			case 603:
			{
				...
				if(deepInspect)
				{
					for(auto itr = mBaseInterfaceList.begin(); itr != mBaseInterfaceList.end(); ++itr)
					{
						mServerCapabilitiesGrp->FindAll(this->mspByteArray.get(), avpCodeMap, size, (key+1),  pBaseInterface,deepInspect);
					}
				}
				}
				break;
...
void LocationInfoAnswer::Display(UINT32 tab)
{
	...
	for(int i = 0; i < mTagsPresent.size() ; ++i)
	{
		...
		switch(i)
		{
			...
			case LocationInfoAnswer_SERVERCAPABILITIES:
			{
				cout << setw(tempTab) << "" << "|"  << left <<  setw(12) << "AvpCode" << ":" << setw(15) << "603" << "|" << setw(12)  << "AvpType" << ":" << setw(15) << "GROUPED" << "|" << "\n"; 
				cout << setw(tempTab) << "" << "|"  << left <<  setw(12) << "AvpName" << ":" << setw(44) << "ServerCapabilities" << "\n"; 
				TEXT lAvpFlg[100];
				//Hardcoded value.;
				sprintf( lAvpFlg, "%s %s %s", ((DiameterTLVUtil::isVenderSpecific(192)==true)?"Vendor":""), ((DiameterTLVUtil::isMandatoryBitSet(192)==true)?"Mandatory":""), ((DiameterTLVUtil::isProtectedBitSet(192)==true)?"Protected":""));
				this->getServerCapabilities()->Display(tab+1);
			}
				break;
			case LocationInfoAnswer_WILDCARDEDPUBLICIDENTITY:
			{
			...
			case LocationInfoAnswer_LIAFLAGS:
			{
				cout << setw(tempTab) << "" << "|"  << left <<  setw(12) << "AvpCode" << ":" << setw(15) << "653" << "|" << setw(12)  << "AvpType" << ":" << setw(15) << "SINGLE" << "|" << "\n"; 
				cout << setw(tempTab) << "" << "|"  << left <<  setw(12) << "AvpName" << ":" << setw(44) << "LIAFlags" << "\n"; 
				TEXT lAvpFlg[100];
				//Hardcoded value.;
				sprintf( lAvpFlg, "%s %s %s", ((DiameterTLVUtil::isVenderSpecific(128)==true)?"Vendor":""), ((DiameterTLVUtil::isMandatoryBitSet(128)==true)?"Mandatory":""), ((DiameterTLVUtil::isProtectedBitSet(128)==true)?"Protected":""));
				this->getLIAFlags()->Display(tab+1);
			}
				break;
			case LocationInfoAnswer_ERRORMESSAGE:
			{
...
BOOLEAN LocationInfoAnswer::GetMissingAvpInfo(TEXT *lMissingAvpInfo)
{
	...
	for(unsigned int i = 0 ; i < lMissingAvpTag.size(); ++i)
	{
		...
		switch(i)
		{
			...
			case LocationInfoAnswer_SERVERCAPABILITIES:
			{
				...
				{
					lFirstRec = false;
					sprintf(lMissingAvpInfo, "Missing Avps List: 603");
				}
				else
					sprintf(lMissingAvpInfo, "%s,603", lMissingAvpInfo);
			}
				break;
			case LocationInfoAnswer_WILDCARDEDPUBLICIDENTITY:
			{
