...
using namespace CX;
BOOLEAN AssociatedRegisteredIdentities::isPresent(int tag){
...
list<OctetString*>* AssociatedRegisteredIdentities::getUserNameList()
{
	if(mUserNameList == NULL)
	{
...
list<OctetString*>* AssociatedRegisteredIdentities::setUserNameList() {
	mTagsPresent[AssociatedRegisteredIdentities_USERNAME] = 1;
	if(mUserNameList == NULL)
		mUserNameList = new list<OctetString*>;
	mMandatoryAvpSetVal [AssociatedRegisteredIdentities_USERNAME] = 1;
	mUserNameList->clear();
	return mUserNameList;
}

void AssociatedRegisteredIdentities::resetUserNameList(){
	mTagsPresent[AssociatedRegisteredIdentities_USERNAME] = 0;
	if(mUserNameList != NULL)
		delete mUserNameList;
}
...
int AssociatedRegisteredIdentities::Encode(byteArray *array)
{
	...
	for(unsigned int i = 0 ; i < mTagsPresent.size() ; ++i )
	{
		...
		switch(i)
		{
			case AssociatedRegisteredIdentities_USERNAME:
			{
...
int AssociatedRegisteredIdentities::Decode(byteArray *array, INT16 lMsgLen, UINT16 *parsedBytes)
{
	mDecodeFlag = true;
	while( lMsgLen > 0)
	{
...
DiameterBaseInterface* AssociatedRegisteredIdentities::Find(byteArray *array, UINT32 avpCode)
{
...
BOOLEAN AssociatedRegisteredIdentities::FindAll(byteArray *array, UINT32 avpCode, list<DiameterBaseInterface*>& pBaseInterface,BOOLEAN deepInspect)
{
	if(!this->mAvpDecodedFlag)
	{
		this->mspByteArray.get()->offset = 0;
...
std::shared_ptr<byteArray> AssociatedRegisteredIdentities::GetByteArray()
{
	return mspByteArray;
}


BOOLEAN AssociatedRegisteredIdentities::FindAll(byteArray *array, std::map<int,UINT32> &avpCodeMap, int &size, int key, list<DiameterBaseInterface*>& pBaseInterface,BOOLEAN deepInspect)
{
	if(!this->mAvpDecodedFlag)
	{
		this->mspByteArray.get()->offset = 0;
...
INT32 AssociatedRegisteredIdentities::setAvpCode(UINT32 avpCode)
{
	mAvpCode = avpCode;
	return 0;
}
UINT32 AssociatedRegisteredIdentities::getAvpCode()
{
	return mAvpCode;
}
INT32 AssociatedRegisteredIdentities::setAvpVendorId(UINT32 avpVendorId)
{
	mAvpVendorId = avpVendorId;
	return 0;
}
UINT32 AssociatedRegisteredIdentities::getAvpVendorId()
{
	return mAvpVendorId;
}
INT32 AssociatedRegisteredIdentities::setAvpOffsetLen(INT16 offset, INT16 length)
{
	mAvpOffsetLen = make_pair(offset,length);
	return 0;
}
pair<INT16,INT16> AssociatedRegisteredIdentities::getAvpOffsetLen()
{
...
void AssociatedRegisteredIdentities::Display(UINT32 tab)
{
	int tempTab = tab * 3; 
...
BOOLEAN AssociatedRegisteredIdentities::GetMissingAvpInfo(TEXT *lMissingAvpInfo)
{
