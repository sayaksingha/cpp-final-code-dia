...
BOOLEAN DiameterCreateSource::GenerateHeaderFile()
{
	...
	while(list != mAvpArgs.end())
	{
		...
		{
			fprintf(mHeaderFile, "\t\tlist<%s::%s*>*\tm%sGrpList;\n", to_upper(mInterface), list->avpname, list->avpname);
			fprintf(mHeaderFile, "\t\tlist<%s::%s*>::iterator\tm%sGrpIterator;\n", to_upper(mInterface), list->avpname, list->avpname);
		}
   ...
   fprintf(mHeaderFile, "\t\tBOOLEAN mAvpDecodedFlag = true;\n\n");
	...
	while(list != mAvpArgs.end())
	{
		...
		else
		{
			if(1 == list->multiple)
			{
				fprintf(mHeaderFile, "\t\tlist<%s::%s*>* get%sGrpList();\n\n", to_upper(mInterface), list->avpname, list->avpname);
				fprintf(mHeaderFile, "\t\tlist<%s::%s*>* set%sGrpList();\n\n", to_upper(mInterface), list->avpname, list->avpname);
...
mMessage.command, list->avpname, mMessage.command,to_upper(list->avpname),list->avpname, list->avpname);
		...
		else
		{
			if(1 == list->multiple)
			{
				fprintf(mSourceFile, "list<%s::%s*>* %s::get%sGrpList()\n", to_upper(mInterface), list->avpname, mMessage.command, list->avpname); 
            fprintf(mSourceFile, "{\n");
            fprintf(mSourceFile, "\tif(m%sGrpList == NULL)\n", list->avpname);
            fprintf(mSourceFile, "\t{\n");
            fprintf(mSourceFile, "\t\tTssDiameterMsgException lTssDiaMsgException(DIAMETER_OBJ_NULL, \"%s is Null\");\n",list->avpname);
            ...
            fprintf(mSourceFile, "}\n\n");
				...
				//fprintf(mSourceFile, "list<%s::%s*>* %s::set%sGrpList(){\n\tmTagsPresent[%s_%s] = 1;\n\tif(m%sGrpList == NULL)\n\t\tm%sGrpList = new list<%s::%s*>;\n", to_upper(mInterface), list->avpname, mMessage.command, list->avpname, mMessage.command,to_upper(list->avpname),list->avpname,list->avpname, to_upper(mInterface), list->avpname);
   

fprintf(mSourceFile,
    "list<%s::%s*>* %s::set%sGrpList(){\n",
    to_upper(mInterface), list->avpname,
    mMessage.command, list->avpname
);
...
fprintf(mSourceFile,
    "\t\tm%sGrpList = new list<%s::%s*>;\n",
    list->avpname, to_upper(mInterface), list->avpname
);
   ...
   if(strcmp(mInterface,"base") == 0)
   {
      ...
      for ( list1 =mDiameterBaseParam.begin(); list1 != mDiameterBaseParam.end(); ++list1) 
      {
         ...
         if(AVP == list1->second.avpGrp)
         {
            ...
            {
               fprintf(diaHeaderFPtr, "\tvirtual list<%s::%s*>*\tget%sGrpList();\n", to_upper(mInterface), list1->second.avpname, list1->second.avpname);
               fprintf(diaHeaderFPtr, "\tvirtual list<%s::%s*>*\tset%sGrpList();\n", to_upper(mInterface), list1->second.avpname, list1->second.avpname);
      ...
      for ( list1 =mDiameterBaseParam.begin(); list1 != mDiameterBaseParam.end(); ++list1) 
      {
         ...
         if(AVP == list1->second.avpGrp)
         {
            ...
            if(1 == list1->second.multiple)
            {
               fprintf(diaHeaderFPtr, "list<%s::%s*>* %s::get%sGrpList(){\n\tTssDiameterMsgException lTssDiaMsgException(DIAMETER_BASE_FUNC_CALLED_ERR);\n\tthrow lTssDiaMsgException;\n\treturn NULL;\n}\n\n", to_upper(mInterface), list1->second.avpname, classname.c_str(), list1->second.avpname );
               fprintf(diaHeaderFPtr, "list<%s::%s*>* %s::set%sGrpList(){\n\tTssDiameterMsgException lTssDiaMsgException(DIAMETER_BASE_FUNC_CALLED_ERR);\n\tthrow lTssDiaMsgException;\n\treturn NULL;\n}\n\n", to_upper(mInterface), list1->second.avpname, classname.c_str(), list1->second.avpname);
