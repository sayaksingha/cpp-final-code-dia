//static const char *id = " @(#) $Id: DiameterCreateSource.cc,v 1.1.2.1 2025/01/29 15:33:27 mram Exp $";
// @(#) $Id: DiameterCreateSource.cc,v 1.1.2.1 2025/01/29 15:33:27 mram Exp $
//----------------------------------------------------------------------
// NAME
		...
		if(AVP == list->avpGrp)
		{
			if ( list->multiple == 0 )
			{
				...
				sprintf(lTemp, "m%s", list->avpname);
				 //Abhishek if (((list->avpFlags&0x40)?true:false))
...
BOOLEAN DiameterCreateSource::generateSourceForDecode()
{
   ...
   fprintf(mSourceFile, "\t\t}\n");
	...
	while(list != mAvpArgs.end())
	{
		...
		fprintf(mSourceFile, "\t\t\t\t{\n");
		/* Updated with below code By Abhishek Chhabra to enhance the Flags functionality. Case:
		 *  If recieved as set then send as set, if receieved as unset then send
		 *  as unset.
		 if (((list->avpFlags&0x40)?true:false))
		 fprintf(mSourceFile, "\t\t\t\t\t\tmMandatoryAvpSetVal [%s_%s] = 1;\n", mMessage.command, to_upper(list->avpname));
		 */

      //Added By Abhishek Chhabra --- Start
      if( list->multiple != false)
      {
      /*fprintf(mSourceFile, "\t\t\t\t\t\tDiameterTLVUtil::isMandatoryBitSet(lAvpFlag) == 1 ? \n");
      fprintf(mSourceFile, "\t\t\t\t\t\t\tmMandatoryAvpSetVal [%s_%s] = 1 : \n", mMessage.command, to_upper(list->avpname));
      fprintf(mSourceFile, "\t\t\t\t\t\t\tmMandatoryAvpSetVal [%s_%s] = 0;\n", mMessage.command, to_upper(list->avpname));*/
      }
      //End
		...
		if( list->avpGrp == AVP) {
            ...
            */
            ...
            fprintf(mSourceFile, "\t\t\t\t\t\t\tmMandatoryAvpSetVal [%s_%s] = 0;\n", mMessage.command, to_upper(list->avpname));
				...
				fprintf(mSourceFile, "\t\t\t\t\tm%sGrp->setAvpVendorId(lVenderId);\n", list->avpname);
            fprintf(mSourceFile, "\t\t\t\t\t\tDiameterTLVUtil::isMandatoryBitSet(lAvpFlag) == 1 ? \n");
            fprintf(mSourceFile, "\t\t\t\t\t\t\tmMandatoryAvpSetVal [%s_%s] = 1 : \n", mMessage.command, to_upper(list->avpname));
            fprintf(mSourceFile, "\t\t\t\t\t\t\tmMandatoryAvpSetVal [%s_%s] = 0;\n", mMessage.command, to_upper(list->avpname));
				//End
	...
	while(list != mAvpArgs.end())
	{
		...
		if( list->avpGrp == AVP){
			if( list->multiple == false) {
				 ...
				 *   *  If recieved as set then send as set, if receieved as unset then send
				 *    *  as unset.
				 fprintf(mSourceFile, "\t\t\t\t\tarray->byte[array->offset++] = %d;\n", mAvp.avpFlags); //ToTest
				 */
				//Start
				fprintf(mSourceFile, "\t\t\t\t\tif(m%s->getAvpVendorId())\n", list->avpname );
				fprintf(mSourceFile, "\t\t\t\t\t{\n");
				fprintf(mSourceFile, "\t\t\t\t\t\tarray->byte[array->offset++] = mMandatoryAvpSetVal [%s_%s] == 1\n", mMessage.command, to_upper(list->avpname));
				fprintf(mSourceFile, "\t\t\t\t\t\t\t? 192\n");
				fprintf(mSourceFile, "\t\t\t\t\t\t\t: 128;\n");
				fprintf(mSourceFile, "\t\t\t\t\t}\n");
				fprintf(mSourceFile, "\t\t\t\t\telse\n");
				fprintf(mSourceFile, "\t\t\t\t\t{\n");
				fprintf(mSourceFile, "\t\t\t\t\t\tarray->byte[array->offset++] = mMandatoryAvpSetVal [%s_%s] == 1\n", mMessage.command, to_upper(list->avpname));
				fprintf(mSourceFile, "\t\t\t\t\t\t\t? 64\n");
				fprintf(mSourceFile, "\t\t\t\t\t\t\t: 0;\n");
				fprintf(mSourceFile, "\t\t\t\t\t}\n");
				//End
          ...
          else if(strcasecmp(mAvp.type, "Time") != 0)
          {
            ...
            else
            {
               ...
               fprintf(mSourceFile, "\t\t\t\t\t\t}\n");
				 ...
				 */
				//Start
				fprintf(mSourceFile, "\t\t\t\t\tif((*m%sIterator)->getAvpVendorId())\n", list->avpname );
				fprintf(mSourceFile, "\t\t\t\t\t{\n");
				fprintf(mSourceFile, "\t\t\t\t\t\tarray->byte[array->offset++] = mMandatoryAvpSetVal [%s_%s] == 1\n", mMessage.command, to_upper(list->avpname));
				fprintf(mSourceFile, "\t\t\t\t\t\t\t? 192\n");
				fprintf(mSourceFile, "\t\t\t\t\t\t\t: 128;\n");
		...
		else 
		{
			if( list->multiple == true) {
				 ...
				 */
				 ...
				 //fprintf(mSourceFile, "\t\t\t\t\tarray->byte[array->offset++] = %d;\n", mAvp.avpFlags);
				/// won't work as it is grp iterator 
				fprintf(mSourceFile, "\t\t\t\t\tif((*m%sGrpIterator)->getAvpVendorId())\n", list->avpname );
				//fprintf(mSourceFile, "\t\t\t\t\tif(%d == 128)\n", mAvp.avpFlags );
				fprintf(mSourceFile, "\t\t\t\t\t{\n");
				fprintf(mSourceFile, "\t\t\t\t\t\tarray->byte[array->offset++] = mMandatoryAvpSetVal [%s_%s] == 1\n", mMessage.command, to_upper(list->avpname));
				fprintf(mSourceFile, "\t\t\t\t\t\t\t? 192\n");
				fprintf(mSourceFile, "\t\t\t\t\t\t\t: 128;\n");
				...
				fprintf(mSourceFile, "\t\t\t\tarray->size++;\n");
//Abhishek				fprintf(mSourceFile, "\t\t\t\tarray->byte[array->offset++] = %d;\n", mAvp.avpFlags);
//				//Start
				fprintf(mSourceFile, "\t\t\t\t\tif((this->get%s()->getAvpVendorId()))\n", list->avpname );
				fprintf(mSourceFile, "\t\t\t\t\t{\n");
				fprintf(mSourceFile, "\t\t\t\t\t\tarray->byte[array->offset++] = mMandatoryAvpSetVal [%s_%s] == 1\n", mMessage.command, to_upper(list->avpname));
				fprintf(mSourceFile, "\t\t\t\t\t\t\t? 192\n");
				fprintf(mSourceFile, "\t\t\t\t\t\t\t: 128;\n");
