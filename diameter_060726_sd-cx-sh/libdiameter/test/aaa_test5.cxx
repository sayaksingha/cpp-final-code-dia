...
/*                                                                        */
/* END_COPYRIGHT                                                          */
/* $Id: aaa_test5.cxx,v 1.1.4.1 2017/11/22 00:35:10 shijuc Exp $ */
/* 
...
DiameterMsgParserWidgetChecked   parser;
  ...
  diameter_identity_t *host = originHost.GetAvp((char *)"Origin-Host");
  if (host) {
      std::cout << "Origin-Host = " << *host << std::endl;
  }

  diameter_identity_t *realm = originRealm.GetAvp((char *)"Origin-Realm");
  if (realm) {
      std::cout << "Origin-Host = " << *realm << std::endl;
  }

  diameter_address_t *address = hostIp.GetAvp((char *)"Host-IP-Address", 0);
  for (int ndx=1; address; ndx++) {
	  ...
	  std::cout << "Host-IP-Address = " 
		        << addr.get_host_addr()
		        << std::endl;
      address = hostIp.GetAvp((char *)"Host-IP-Address", ndx);
  }

  diameter_unsigned32_t *vid = vendorId.GetAvp((char *)"Vendor-Id");
  if (vid) {
      std::cout << "Vendor-Id = " << *vid << std::endl;
  }

  diameter_utf8string_t *prod = product.GetAvp((char *)"Product-Name");
  if (prod) {
      std::cout << "Product-Name = " << *prod << std::endl;
  }

  diameter_unsigned32_t *oState = originState.GetAvp((char *)"Origin-State-Id");
  if (oState) {
      std::cout << "Origin-State-Id = " << *oState << std::endl;
  }

  diameter_unsigned32_t *svid = supportedVendorId.GetAvp
      ((char *)"Supported-Vendor-Id", 0);
  for (int ndx=1; svid; ndx++) {
      std::cout << "Supported-Vendor-Id = " << *svid << std::endl;
      svid = supportedVendorId.GetAvp((char *)"Supported-Vendor-Id", ndx);
  }

  diameter_unsigned32_t *fware = firmware.GetAvp((char *)"Firmware-Revision");
  if (fware) {
      std::cout << "Firmware-Revision = " << *fware << std::endl;
  }

  diameter_unsigned32_t *authId = authAppId.GetAvp
      ((char *)"Auth-Application-Id", 0);
  for (int ndx=1; authId; ndx++) {
      std::cout << "Auth-Application-Id = " << *authId << std::endl;
      authId = authAppId.GetAvp((char *)"Auth-Application-Id", ndx);
  }
  ...
  for (int ndx=1; acctId; ndx++) {
      ...
      acctId = acctAppId.GetAvp((char *)"Acct-Application-Id", ndx);
  }

  diameter_grouped_t *grouped = vendorSpecificId.GetAvp
      ((char *)"Vendor-Specific-Application-Id", 0);
  ...
  for (int ndx=1; grouped; ndx++) {
      ...
      diameter_unsigned32_t *uint32 = gVendorId.GetAvp((char *)"Vendor-Id", 0);
      
      for (int p=1; uint32; p++) {
          uint32 = gVendorId.GetAvp((char *)"Vendor-Id", p);
          if (uint32) {
              std::cout << "Vendor-Id = " << *uint32 << std::endl;
          }
      }
        
      uint32 = gAuthId.GetAvp((char *)"Auth-Application-Id");
      if (uint32) {
          std::cout << "Auth-Application-Id = " << *uint32 << std::endl;
      }

      uint32 = gAcctId.GetAvp((char *)"Acct-Application-Id");
      if (uint32) {
...
stest(unsigned char *buf, int size)
{
  ...
  DiameterIdentityAvpWidget originHost((char *)"Origin-Host");
  DiameterIdentityAvpWidget originRealm((char *)"Origin-Realm");
  DiameterAddressAvpWidget  hostIp((char *)"Host-IP-Address");
  DiameterUInt32AvpWidget   vendorId((char *)"Vendor-Id");
  DiameterUtf8AvpWidget     product((char *)"Product-Name");
  DiameterUInt32AvpWidget   originStateId((char *)"Origin-State-Id");
