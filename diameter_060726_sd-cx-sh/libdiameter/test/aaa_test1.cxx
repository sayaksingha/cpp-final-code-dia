...
/*                                                                        */
/* END_COPYRIGHT                                                          */
/* $Id: aaa_test1.cxx,v 1.1.4.1 2017/11/22 00:35:10 shijuc Exp $ */
/* 
...
//  std::vector<AAAUInt8> rawBody(buf , buf+size);
	
//  pp.setRecvData(rawBody);
  ...
  ACE_OS::memcpy ( msg->rawbyteArray.byte , buf , size);

  msg->rawbyteArray.size = size;

  for ( int i = 0 ; i < size ; i ++)
	  std::cout << std::hex << " "<<  (int) buf[i] ;
	
  // msg.rawBody = pp.getRecvData( );
  // std::vector<AAAUInt8>::iterator it;


  //for (  it = msg.rawBody.begin() ; it < msg.rawBody.end(); it ++)
