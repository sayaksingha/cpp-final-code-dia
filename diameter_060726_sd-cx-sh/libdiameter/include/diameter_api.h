// @(#) $Id: diameter_api.h,v 1.1.4.1.2.1.2.2 2024/05/15 13:11:22 mram Exp $
//---------------------------------------------------------------
// NAME : diameter_api.h
//
// COPYRIGHT
...
typedef std::unique_ptr<DiameterApplication> AAAApplicationHandle;
...
class DIAMETERBASEPROTOCOL_EXPORT AAAApplicationCore 
{
	public:
				...
				AAA_Task &task) 
		{
			if (! m_Handle.get()) 
			{
				m_Handle = std::unique_ptr<DiameterApplication>
	...
	private:
		std::unique_ptr<DiameterApplication> m_Handle;
};
...
class DIAMETERBASEPROTOCOL_EXPORT AAAMessageControl 
{
	public:
		...
		{
			std::unique_ptr<DiameterMsg> newMsg(new DiameterMsg);
	...
	public AAAAccountingSession
{
	...
	public:
		AAAAccountingClientSession(AAAApplicationCore &appCore,
			...
			m_LastPayload(NULL) {
				m_SubSession = std::unique_ptr<DiameterClientAcctSubSession<LocalCollector> >
	...
	private:
		...
		std::unique_ptr<DiameterClientAcctSubSession<LocalCollector> >
			m_SubSession;
	...
	public AAAAccountingSession
{
		...
		public:
			...
			virtual void StoreRecord(AAAAvpContainerList &avpList,
				...
				if (m_Session) 
				{
					...
					msg.hdr.mMsgLen = 0;
					msg.hdr.mCmdFlg.rBit = DIAMETER_FLAG_SET;
					msg.hdr.mCmdFlg.pBit = DIAMETER_FLAG_CLR;
					msg.hdr.mCmdFlg.eBit = DIAMETER_FLAG_CLR;
					msg.hdr.mCmdCode = DIAMETER_MSGCODE_ACCOUNTING;
					msg.hdr.mApplId = m_Session->GetApplicationId();
	...
	public:
		AAAAccountingServerSession(AAAApplicationCore &appCore,
				diameter_unsigned32_t id) :
			AAAAccountingSession(appCore, id) {
				m_Session = std::unique_ptr<DiameterServerAcctSession<LocalStorage> >
	...
	protected:
		...
		std::unique_ptr<DiameterServerAcctSession<LocalStorage> >
			m_Session;
};
