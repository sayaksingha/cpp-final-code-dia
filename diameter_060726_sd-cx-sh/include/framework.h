// @(#) $Id: framework.h,v 1.1.4.1.2.1.2.7 2024/05/15 13:04:57 mram Exp $
//---------------------------------------------------------------
// NAME : framework.h
//
// COPYRIGHT
...
extern TEXT          gLogText[];
extern Log gLog;

#define ACTIVE_OLD_LOGIC 0
  ...
  class AAA_QueueJob : public AAA_Job
{
 ...
 : AAA_Job(d, name), numPriority(numPriority), maxWeight(maxWeight), total(0), cond(std::unique_ptr< ACE_Condition<LOCK> >
      (new ACE_Condition<LOCK>(lock))), signaled(false)

 {
   ...
   //size.reserve(numPriority*maxWeight);
   //std::fill(size.begin(), size.end(), 0);
   for(unsigned temp = 0; temp < maxNumPriority*maxMaxWeight; temp++)
	size[temp] = 0;
 }
 ...
 { 
	 ...
	 { 
		 if (!blocking)
		 {
			 FRAMEWORK_LOG(LM_DEBUG, "%T |JOB_QUEUE| Enqueue is blocked.\n");
			 return 0;
		 }
		 cond->wait(); 
	 }
	 ...
	 //FRAMEWORK_LOG(LM_DEBUG, "%T |JOB_QUEUE| Enqueue() ---- 3  total:%d index:%d size:%d[%d].\n", total,dIndex,size[dIndex],size.size());
	 ...
	 if (size[dIndex] < weight)
		 indexQueue[pIndex].push_back(dIndex);

//AAA_LOG

	 //if ( size[dIndex] / 1000 )
		// std::cout << "|JOB| SIZE " <<  size[dIndex] << std::endl;
	 ...
	 do 
	 {
		 ...
		 do 
		 {
			 ...
			 for (unsigned p=0; p<numPriority; p++)
			 {
				 ...
				 if (size[dIndex] >= (dIndex % maxWeight) + 1)
			 ...
			 if (!found)
			 {
				 FRAMEWORK_LOG(LM_ERROR, "%T |JOB_QUEUE| BUG (dequeue error).\n");
				 return 0;
			 }
 ...
 { 
	 ...
	 for (unsigned i=0; i<numPriority*maxWeight; dataQueue[i++].clear(),size[i]=0);
	 total=0;
	 cond->signal();
 }
 ...
 void AdjustIndexQueue(unsigned pIndex, unsigned dIndex)
 {
   ...
   for (i=iQueue.begin(); i!=iQueue.end(); )
	 ...
	 i = (++j > size[dIndex]) ? iQueue.erase(i) : i++;
       else
	 i++;
     }
 }
 ...
 std::unique_ptr< ACE_Condition<LOCK> > cond;

 int size[maxNumPriority*maxMaxWeight];;
...
class AAA_GroupedJob : public AAA_QueueJob<AAA_Job*, ACE_Thread_Mutex>
{
	public:
		...
		int Serve()
		{
			...
			m_BusyMutex.acquire();
			m_busy--;
			int enqueue = Enqueue(job, priority ? true : false);
			m_BusyMutex.release();
			return enqueue;
		}
	...
	private:
		...
		AAA_GroupedJob(AAA_Job &parent, 
			...
			: AAA_QueueJob<AAA_Job*, ACE_Thread_Mutex>(d, name, 
					numPriority, maxWeight), 
			parent(parent), policy(policy), running(false), blocking(blocking),m_busy(0)
	{
  ...
  AAA_GroupedJob(AAA_JobData* d, char *name=0,
    ...
    : AAA_QueueJob<AAA_Job*, ACE_Thread_Mutex>(d, name,
					       numPriority, maxWeight), 
    parent(*this), policy(policy), running(false), blocking(blocking),m_busy(0)
  {}
			...
			: handle
			  (AAA_JobHandle<AAA_GroupedJob>
			   (AAA_GroupedJob::Create((AAA_JobData*)this, name, policy, true,
						   numPriority, maxWeight))),
			  cond(std::unique_ptr< ACE_Condition<ACE_Mutex> >
...
private:
  ...
  int svc()
  {
	  ...
	  if (!reactor())
	  {
		  std::unique_ptr<ACE_Reactor> r(new ACE_Reactor);
		  reactor(r.get());
		  mutex.release();
		  cond->signal();
  ...
  /// Used for synchronization among threads.
  std::unique_ptr< ACE_Condition<ACE_Mutex> > cond;
...
template <class ARG> class AAA_Action 
{
	public:
		...
		virtual void operator()(ARG& lArgs, std::unique_ptr<Diameter_IO_Base> m_IO, std::unique_ptr<DiameterMsg> m_Msg){}; // for CER
      virtual void operator()(ARG& lArg, std::unique_ptr<DiameterMsg> m_Msg, std::unique_ptr<Diameter_IO_Base> m_IO) {}; // for Gx,Rx,Sy
	protected:
		virtual ~AAA_Action() {}
		AAA_Action() {}
};
...
public:
  ...
  virtual void UpdateStateOnSessSync(AAA_State rState) //throw(AAA_Error)
  { 
    state = rState;
    //running = true;
  }
  ...
  void Event(AAA_Event ev)
  {
    ...
    //printf("---StateMachine[%s] cannot accept event %d at state %d. nextState %d\n",
          //Name().c_str(), ev, state, entry->NextState());
    ...
    // Execute the action for the current StateTableEntry.
		...
		{
			//printf("TBR ==== Exception \n");
		}
		
  }
  void Event(AAA_Event ev, std::unique_ptr<DiameterMsg> m_Msg, std::unique_ptr<Diameter_IO_Base> m_IO)
  {
     // Throw an exception if state machine is not started.
     if (!running)
     {
     ...
     try
     {
        //entry->Action()(actionArg, m_Msg, m_IO);
        entry->Action()(actionArg, std::move(m_Msg), std::move(m_IO));
     }
     catch(...)
     {
        //printf("TBR ==== Exception \n");
  ...
  //This method is added by Abhishek Chhabra to sending The NACK in case of Recieving CER on established Connection.
  void Event(AAA_Event ev, std::unique_ptr<Diameter_IO_Base> m_IO, std::unique_ptr<DiameterMsg> m_Msg)
   {
      // Throw an exception if state machine is not started.
      if (!running)
      {
      ...
      // Search state table for StateTableEntry containing the event
         //FRAMEWORK_LOG
           // (LM_ERROR,
             //"StateMachine[%s] current event %d at state %d.\n",
             //Name().c_str(), ev, state);
      // defined in the current state.
      if (!stateTable.FindStateTableEntry(state, ev, entry))
      {
      ...
      // Change the state.
      int lCurrentState = state; 
      
      //printf("Overload StateMachine[%s] event %d at state %d. NextState %d\n",
        //    Name().c_str(), ev, state, entry->NextState() );          
      ...
      {
         //entry->Action()(actionArg, m_Msg, m_IO);
         if (ev == 4 && (lCurrentState == 6 || lCurrentState == 0 || lCurrentState == 1 
            || lCurrentState == 2 || lCurrentState == 3 || lCurrentState == 4 || lCurrentState == 5)) // 4 = DIAMETER_PEER_EV_R_CONN_CER ... 6 = DIAMETER_PEER_ST_R_OPEN
            entry->Action()(actionArg, std::move(m_IO), std::move(m_Msg));
         else
            entry->Action()(actionArg);
...
public:
		     ...
		     ACE_UINT32 usec=0, int type=0)
  {
    ...
    AAA_MutexScopeLock guard(mTimerMutex); 
...
ACE_SINGLETON_TEMPLATE_INSTANTIATE(ACE_Singleton, AAALogMsg, ACE_Recursive_Thread_Mutex);
//ACE_EXPORT_SINGLETON_DECLARE(ACE_Singleton, AAALogMsg, ACE_Recursive_Thread_Mutex);
class TssLogGenerator {
   public:

      //static Log *mLog;

       //Log* TssLogGenerator::mLog = new Log;
      template<typename... Args>
         static void log( int err , char * format, Args... args)
         {
            char lLogText[300];
            //snprintf( lLogText, 300, format+3, args...); AB updated to below because of: global-buffer-overflow 
            snprintf( lLogText, 300, format, args...);
            switch(err)
            {
               case LM_CRITICAL:
               case LM_SHUTDOWN:
               case LM_STARTUP:
               case LM_EMERGENCY:
                  gLog.GenerateLog(GSYS01, lLogText);
                  //mLog->GenerateLog(DRA01, lLogText);
                  break;
               case LM_ERROR:
                  gLog.GenerateLog(GSYS02, 1, lLogText);
                  break;
               case LM_INFO:
               case LM_NOTICE:
                  gLog.GenerateLog(GSYS03, 2, lLogText);
                  break;
               case LM_DEBUG:
               case LM_WARNING:
               case LM_ALERT:
                  gLog.GenerateLog(GSYS04, 3, lLogText);
                  break;
               case LM_TRACE:
                  gLog.GenerateLog(GSYS05, 4, lLogText);
                  break;
            }
         }
 
          static void log( int err , char * format )
      {
         char lLogText[300];
         //snprintf( lLogText, 300, format+3 ); AB updated to below because of: global-buffer-overflow 
         snprintf( lLogText, 300, format);
        //mLog->GenerateLog(101,"check check check");
#if ACTIVE_OLD_LOGIC 
	 switch(err)
         {
            case LM_CRITICAL:
           ...
           //Changed by sayak singha 23/02/2026
	    switch(err)
            {
              case LM_SHUTDOWN:
              case LM_STARTUP:
              case LM_EMERGENCY:
                gLog.GenerateLog(GSYS01, lLogText,(UINT8)LOG_EMERG);
                break;
                //mLog->GenerateLog(DRA01, lLogText);
              case LM_CRITICAL:
                gLog.GenerateLog(GSYS01, lLogText,(UINT8)LOG_CRIT);
                //mLog->GenerateLog(DRA01, lLogText);
                break;
              case LM_ERROR:
                  gLog.GenerateLog(GSYS02, 1, lLogText,LOG_ERR);
                  break;
              case LM_INFO:
              case LM_NOTICE:
                  gLog.GenerateLog(GSYS03, 2, lLogText,LOG_INFO);
                  break;
              case LM_DEBUG:
              case LM_WARNING:
              case LM_ALERT:
                  gLog.GenerateLog(GSYS04, 3, lLogText,LOG_WARNING);
                  break;
              case LM_TRACE:
                  gLog.GenerateLog(GSYS05, 4, lLogText,LOG_DEBUG );
                  break;
            }
#endif
      }
...
#if 0
template <class O_ID, class L_HH>
class HH_Store: private std::map<O_ID,L_HH>
{
public :
      virtual ~HH_Store()
      {
         //std::cout<<"TBC Destructou HH_Store\n";
			//TBC----------add logs
      }

      virtual int Add(O_ID ndx, L_HH data)
      {
         if(std::map<O_ID,L_HH>::insert(std::pair<O_ID,L_HH>(ndx, data)).second==false)
            return -1;
         else
            return 0;
      }

      virtual bool Lookup(O_ID ndx,L_HH &data)
      {
         typename std::map<O_ID,L_HH>::iterator i;
         i = std::map<O_ID,L_HH>::find(ndx);
         if (i != std::map<O_ID,L_HH>::end())
         {
            data = i->second;
            //O_ID key = i->first;
            //std::cout<<data<<'\t'<<key<<std::endl;
            //std::cout<<std::map<O_ID,L_HH>::size()<<std::endl;
            return (true);
         }
         std::cout<<std::map<O_ID,L_HH>::size()<<std::endl;
         return (false);
...
{
   public:
      void Enqueue(std::unique_ptr<ARG> a) {
         m_Queue.Enqueue(a.release());
      }
      std::unique_ptr<ARG> Dequeue() {
         std::unique_ptr<ARG> arg(m_Queue.Dequeue());
         return arg;
      }
		unsigned Size()
		{
